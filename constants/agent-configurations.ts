...
const DEPARTMENT_CONFIGS: DepartmentAgentConfig[] = [
  // 0: Cross-Department
  {
    departmentId: 0,
...
// ============================================================
// AUTO-GENERATED DEPARTMENT CONFIGURATIONS (24-80+)
// For departments not manually configured, we generate configs
// based on the department name and common patterns.
// ============================================================

interface GeneratedDeptInfo {
  id: number;
  name: string;
  slug: string;
  color: string;
...
const GENERATED_DEPARTMENTS: GeneratedDeptInfo[] = [
...
const ALL_DEPARTMENT_CONFIGS: DepartmentAgentConfig[] = [
  ...DEPARTMENT_CONFIGS,
  ...GENERATED_DEPARTMENTS.map((dept): DepartmentAgentConfig => ({
...
export function getAllDepartmentConfigs(): DepartmentAgentConfig[] {
  return ALL_DEPARTMENT_CONFIGS;
}

// Helper to get config by department ID
export function getDepartmentConfig(departmentId: number): DepartmentAgentConfig | undefined {
  return ALL_DEPARTMENT_CONFIGS.find(c => c.departmentId === departmentId);
}

// Helper to get config by department slug
export function getDepartmentConfigBySlug(slug: string): DepartmentAgentConfig | undefined {
  return DEPARTMENT_CONFIGS.find(c => c.departmentSlug === slug);
}

// Helper to get system prompt for a specific agent
export function getAgentSystemPrompt(departmentId: number, level: string, agentTitle: string): string {
  const config = getDepartmentConfig(departmentId);
  if (!config) return `You are ${agentTitle}, an AI agent providing specialized services.`;
...
export function getAgentTools(departmentId: number, level: string): AgentToolConfig[] {
  const config = getDepartmentConfig(departmentId);
  if (!config) return [];
...
export function getAgentCapabilities(departmentId: number, level: string): string[] {
  const config = getDepartmentConfig(departmentId);
  if (!config) return config?.capabilities || [];
...
export function getAllAgentTools(): AgentToolConfig[] {
  const allTools: AgentToolConfig[] = [];
