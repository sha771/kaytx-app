...
// Forward declaration of logger to be initialized later to avoid circular dependencies
let logger: any;
const getLogger = () => {
  if (!logger) {
    // Dynamic import to avoid circular dependency at top-level
    const { logger: prodLogger } = require('./production-logger');
    logger = prodLogger;
  }
  return logger;
};
...
export function loadUnifiedConfig(): UnifiedConfig {
  ...
  const validEnv = ['development', 'staging', 'production'].includes(nodeEnv) 
    ? nodeEnv as 'development' | 'staging' | 'production'
    : 'development';
  const isProduction = validEnv === 'production';
  ...
  const getEnvOrGenerate = (key: string, length: number = 32): string => {
    ...
    if (isProduction) {
      throw new Error(`CRITICAL: Missing required environment variable: ${key}. GENERATION IS DISABLED IN PRODUCTION TO PREVENT DATA LOSS.`);
    }
    const generated = crypto.randomBytes(length).toString('hex');
    
    // Explicit warning about generated keys in development
    console.warn(`[Config] WARNING: ${key} not set. Using a temporary generated value. THIS WILL CHANGE ON NEXT RESTART.`);
  ...
  const config: any = {
    ...
    security: {
      jwt: {
        secret: isProduction ? requireEnv('JWT_SECRET') : getEnvOrGenerate('JWT_SECRET'),
        expiration: process.env.JWT_EXPIRATION || '24h',
        refreshExpiration: process.env.JWT_REFRESH_EXPIRATION || '7d',
      },
      encryption: {
        key: isProduction ? requireEnv('ENCRYPTION_KEY') : getEnvOrGenerate('ENCRYPTION_KEY'),
  ...
  try {
    ...
    getLogger().error('Configuration validation failed', error instanceof Error ? error : undefined);
