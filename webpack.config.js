...
module.exports = async function (env, argv) {
  ...
  config.resolve.alias = {
    ...
    'react-native-web/dist/exports/findNodeHandle': path.resolve(__dirname, 'lib/react-native-web-exports.ts'),
    'react-native-web/dist/exports/processColor': path.resolve(__dirname, 'lib/react-native-web-exports.ts'),
    'react-native-web/dist/exports/render': path.resolve(__dirname, 'lib/react-native-web-exports.ts'),
    'react-native-web/dist/modules/getStyleSheet': path.resolve(__dirname, 'lib/getStyleSheet-stub.ts'),
  };
  ...
  // Fix react-native-web NativeEventEmitter missing export
  config.plugins.push(
    new NormalModuleReplacementPlugin(
      /react-native-web[\/\\]dist[\/\\]exports[\/\\]NativeEventEmitter/,
      path.resolve(__dirname, 'lib/NativeEventEmitter-stub.ts')
    )
  );

  // Fix other missing react-native-web exports
  ...
  for (const exportName of missingExports) {
    config.plugins.push(
      new NormalModuleReplacementPlugin(
        new RegExp(`react-native-web[\\\\/]dist[\\\\/]exports[\\\\/]${exportName}`),
        path.resolve(__dirname, 'lib/react-native-web-exports.ts')
      )
    );
  }

  // Fix react-native-web getStyleSheet missing module
  config.plugins.push(
    new NormalModuleReplacementPlugin(
      /react-native-web[\/\\]dist[\/\\]modules[\/\\]getStyleSheet/,
      path.resolve(__dirname, 'lib/getStyleSheet-stub.ts')
    )
  );
  ...
  if (config.devServer) {
    ...
    // Ensure historyApiFallback is enabled for Expo Router SPA routing
    cleanDevServer.historyApiFallback = {
      rewrites: [
        { from: /^\/.*$/, to: '/index.html' }
      ]
    };
