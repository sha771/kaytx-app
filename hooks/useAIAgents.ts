...
export function useAIAgents() {
  ...
  const {
    ...
    () => (apiClient as any).aiAgents?.list?.query() ?? Promise.resolve([]),
    [],
    { immediate: true }
  );
  ...
  const createAgent = useCallback(async (data: CreateAgentData) => {
    try {
      const result = await apiCall(() => 
        (apiClient as any).aiAgents?.create?.mutate(data)
      );
  ...
  const updateAgent = useCallback(async (id: string, data: Partial<CreateAgentData>) => {
    try {
      const result = await apiCall(() => 
        (apiClient as any).aiAgents?.update?.mutate({ id, ...data })
      );
  ...
  const deleteAgent = useCallback(async (id: string) => {
    try {
      const result = await apiCall(() => 
        (apiClient as any).aiAgents?.delete?.mutate({ id })
      );
  ...
  const getAgent = useCallback(async (id: string) => {
    try {
      const result = await apiCall(() => 
        (apiClient as any).aiAgents?.get?.query({ id })
      );
...
// ============================================
// UNIFIED A2A AGENT INTEGRATION EXPORTS
// NOTE: Backend service exports moved to stub file
// to prevent Node.js module bundling issues
// ============================================

// Export stubs (safe for web)
export {
...
// Re-export from constants (safe for web)
export {
  type AIAgent,
  type AgentType,
  type AgentStatus,
