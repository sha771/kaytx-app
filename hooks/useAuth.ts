...
export function useAuth() {
  ...
  const initializeAuth = useCallback(async () => {
    try {
      ...
      const result = await apiCall(() => (apiClient as any).auth?.me?.query());
  ...
  const login = useCallback(async (credentials: LoginCredentials) => {
    try {
      ...
      const result = await apiCall(() => 
        (apiClient as any).auth?.login?.mutate(credentials)
      );
      ...
      if (result.data) {
        ...
        // Get user details
        const userResult = await apiCall(() => (apiClient as any).auth?.me?.query());
  ...
  const register = useCallback(async (data: RegisterData) => {
    try {
      ...
      const result = await apiCall(() => 
        (apiClient as any).auth?.register?.mutate(data)
      );
  ...
  const logout = useCallback(async () => {
    try {
      // Call backend logout
      await apiCall(() => (apiClient as any).auth?.logout?.mutate());
    } catch (error) {
  ...
  const verifyEmail = useCallback(async (token: string, verificationCode?: string) => {
    try {
      const result = await apiCall(() => 
        (apiClient as any).auth?.verifyEmail?.mutate({ token, verificationCode })
      );
  ...
  const resetPassword = useCallback(async (email: string) => {
    try {
      const result = await apiCall(() => 
        (apiClient as any).auth?.resetPassword?.mutate({ email })
      );
  ...
  const refreshToken = useCallback(async () => {
    try {
      ...
      const result = await apiCall(() => 
        (apiClient as any).auth?.refreshToken?.mutate({ refreshToken })
      );
  ...
  const getMFAStatus = useCallback(async () => {
    try {
      const result = await apiCall(() => (apiClient as any).auth?.mfa?.status?.query());
  ...
  const setupMFA = useCallback(async () => {
    try {
      const result = await apiCall(() => (apiClient as any).auth?.mfa?.setup?.mutate());
      return { success: true, data: result.data };
    } catch (error) {
      if (error instanceof APIError) {
        return { success: false, error: error.message };
  ...
  const enableMFA = useCallback(async (totp: string) => {
    try {
      const result = await apiCall(() => (apiClient as any).auth?.mfa?.enable?.mutate({ totp }));
  ...
  const disableMFA = useCallback(async (totp: string) => {
    try {
      const result = await apiCall(() => (apiClient as any).auth?.mfa?.disable?.mutate({ totp }));
  ...
  const startSSOFlow = useCallback(async (provider: 'oidc' | 'saml', orgSlug: string) => {
    try {
      if (provider === 'oidc') {
        const result = await apiCall(() => 
          (apiClient as any).auth?.sso?.oidcStart?.mutate({ orgSlug })
        );
        return { success: true, data: result.data };
      } else {
        const result = await apiCall(() => 
          (apiClient as any).auth?.sso?.samlStart?.mutate({ orgSlug })
        );
        return { success: true, data: result.data };
      }
    } catch (error) {
    ...
    try {
      ...
      if (provider === 'oidc') {
        result = await apiCall(() => 
          (apiClient as any).auth?.sso?.oidcCallback?.mutate(params)
        );
      } else {
        result = await apiCall(() => 
          (apiClient as any).auth?.sso?.samlCallback?.mutate({ orgSlug, ...params })
        );
      }
