...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'53',icon:CheckCircle2,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.4%',icon:Target,color:'#1A237E'}];
  const capabilities = ['Customer Data Analysis','Pattern Identification','Actionable Insights'];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#1A237E20' }]}><Bot size={48} color="#1A237E" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Customer Insights Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Analyzes customer data, identifies patterns, and provides actionable customer insights.</Text></View>
      ...
      <AgentFeatures agentId="customer-insights" agentName="AI Customer Insights Agent" />
    </ScrollView>
  );
}
