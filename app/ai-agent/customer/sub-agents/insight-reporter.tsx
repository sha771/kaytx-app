...
import { Activity, ChartBarBig, CircleCheck, Zap, ChevronLeft, ArrowRight, FileText, Lightbulb } from 'lucide-react-native';
...
const ACCENT = '#5856D6';
const AGENT_NAME = 'AI Insight Reporter';
const AGENT_TITLE = 'Executive Insight Intelligence';
const AGENT_DESC = 'Generates executive-level insight reports, dashboards, and actionable recommendations from analyzed feedback data. Transforms raw survey responses into strategic business intelligence for decision-makers.';
const PARENT_NAME = 'AI Feedback & Survey';
const PARENT_ROUTE = '/ai-agent/customer-experience/feedback-survey';

const METRICS = [
  { label: 'Reports Today', value: '89', color: ACCENT },
  { label: 'Actionability', value: '94%', color: '#00C853' },
  { label: 'Avg Speed', value: '1.2s', color: '#007AFF' },
  { label: 'Exec Rating', value: '4.9/5', color: '#5856D6' },
];

const RECENT_TASKS = [
  { action: 'Generated Q4 executive insight report with 12 key findings', time: '3m ago' },
  { action: 'Created real-time NPS dashboard for product leadership', time: '10m ago' },
  { action: 'Delivered churn-risk insight brief to CS team', time: '20m ago' },
  { action: 'Auto-compiled weekly feedback digest for 5 departments', time: '35m ago' },
  { action: 'Surfaced 3 high-impact improvement opportunities from CSAT data', time: '50m ago' },
  { action: 'Built comparative benchmark report against industry NPS', time: '1h ago' },
];
...
const CAPABILITIES = [
  ...
  'Trend Visualization & Storytelling',
  'Cross-Department Insight Sharing',
  'Benchmark Comparison Reports',
  'Automated Weekly/Monthly Digests',
  'Priority-Impact Matrix Creation',
  'Stakeholder-Specific Formatting',
  'Data-Driven Decision Support',
];

const PERFORMANCE_BARS = [
  { label: 'Insight Relevance', value: 94, color: '#00C853' },
  { label: 'Actionability Score', value: 92, color: ACCENT },
  { label: 'Report Quality', value: 96, color: '#007AFF' },
  { label: 'Stakeholder Satisfaction', value: 98, color: '#5856D6' },
  { label: 'Delivery Speed', value: 91, color: '#FF9500' },
];

const A2A_ENDPOINTS = [
  '/consult/insight-reporter',
  '/insights/generate-report',
  '/insights/create-dashboard',
  '/insights/deliver-recommendations',
  '/insights/compile-digest',
  '/insights/benchmark-compare',
];
        ...
        <View style={[styles.section, { backgroundColor: colors.card }]}>
          ...
          <TouchableOpacity onPress={() => router.push(PARENT_ROUTE)} style={[styles.parentCard, { backgroundColor: colors.background, borderBottomColor: colors.border }]}>
            ...
            <View style={styles.parentInfo}>
              ...
              <Text style={[styles.parentDesc, { color: colors.text + '70' }]}>Main Agent · Feedback & Survey</Text>
            </View>
