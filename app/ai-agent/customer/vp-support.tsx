...
import { Headphones, Activity, Star, CheckCircle2, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'1300',icon:CheckCircle2,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.9%',icon:Target,color:'#E65100'}];
  ...
  const activities = [{time:'3 min ago',text:'Resolved 28 customer escalations',icon:CheckCircle2},{time:'6 min ago',text:'Updated satisfaction survey results',icon:Clock},{time:'9 min ago',text:'Launched new retention campaign',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E6510020' }]}><Headphones size={48} color="#E65100" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Customer Support</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Customer Support AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Customer Experience division.</Text></View>
      ...
      <AgentFeatures agentId="vp-support" agentName="VP Customer Support" />
    </ScrollView>
  );
}
