...
import { Activity, MessageCircle, Clock, Target, Zap, ArrowRight, Briefcase, Star, CircleCheckBig, TrendingUp, BarChart3 } from 'lucide-react-native';
...
export default function customerSupportPage() {
  ...
  const capabilities = ["FAQ Response","Troubleshooting","Live Chat","Ticket Creation","Knowledge Access","Multi-channel Support"];
  const responsibilities = ["Respond to frequently asked questions","Guide customers through troubleshooting","Handle live chat interactions","Create and route support tickets","Access knowledge base for solutions","Provide multi-channel support"];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#007AFF20' }]}><MessageCircle size={48} color="#007AFF" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Customer Support Agent"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>A2A Endpoints</Text>{['/consult/customer-support','/customer-support/execute','/customer-support/analyze'].map((ep,i)=>(<View key={i} style={styles.endpointRow}><Zap size={14} color="#8B5CF6" /><Text style={[styles.endpointText, { color: theme.colors.textSecondary }]}>{ep}</Text></View>))}</View>
      <AgentFeatures agentId="customer-support" agentName="AI Customer Support Agent" />
