...
import { Activity, Phone, Clock, Target, Zap, ArrowRight, Briefcase, Star, CircleCheckBig, TrendingUp, BarChart3 } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function receptionistPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'1.4K',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.2%',icon:Target,color:'#34C759'}];
  const capabilities = ["Call Routing","Visitor Management","Appointment Scheduling","Greeting Automation","Directory Services","Message Taking"];
  const responsibilities = ["Route incoming calls to departments","Log and manage visitors","Schedule appointments and meetings","Automate greeting and check-in","Provide directory and information services","Take and route messages"];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#34C75920' }]}><Phone size={48} color="#34C759" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Receptionist"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Customer Experience</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Star size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>VP / Executive</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Receptionist - Leading departmental strategy, managing cross-functional initiatives, and driving performance optimization within the Customer Experience division.</Text></View>
