...
import { Eye, Activity, Star, Users, CircleCheckBig, Clock, ArrowRight, Zap, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, FileText, Shield, UserCheck, Lock } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function VPPrivacyPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Data Subjects Protected', value: '2.4M', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.7%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.3s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.7%', icon: Zap, color: '#C62828' },
  ];

  const capabilities = ['Privacy Program Management', 'Data Protection', 'Privacy Compliance', 'Consent Management', 'Privacy Impact Assessment', 'Data Classification', 'Regulatory Compliance', 'Privacy Innovation'];

  const responsibilities = [
    'Lead enterprise privacy program and strategy',
    'Ensure data protection across all systems and processes',
    'Manage privacy compliance with GDPR, CCPA, and other regulations',
    'Oversee consent management and data subject rights',
    'Conduct privacy impact assessments for new initiatives',
    'Implement and enforce data classification standards',
    'Ensure regulatory compliance for privacy requirements',
    'Drive privacy innovation and best practices'
  ];

  const activities = [
    { time: '4 min ago', text: 'Processed 150 data subject requests', icon: UserCheck },
    { time: '20 min ago', text: 'Completed privacy impact assessment', icon: Shield },
    { time: '1 hour ago', text: 'Updated consent management system', icon: FileText },
    { time: '3 hours ago', text: 'Reviewed data classification policies', icon: Lock },
  ];

  const subAgents = [
    { name: 'AI Privacy Impact Assessor', route: '/ai-agent/security/sub-agents/privacy-impact-assessor', desc: 'Privacy impact assessment & analysis', icon: Shield },
    { name: 'AI Data Classification Enforcer', route: '/ai-agent/security/sub-agents/data-classification-enforcer', desc: 'Data classification enforcement & monitoring', icon: Lock },
    { name: 'AI Consent Manager', route: '/ai-agent/security/sub-agents/consent-manager', desc: 'Consent management & tracking', icon: UserCheck },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/vp/privacy/status', description: 'Get privacy status' },
    { method: 'POST', endpoint: '/api/v1/security/vp/privacy/assess', description: 'Assess privacy impact' },
    { method: 'GET', endpoint: '/api/v1/security/vp/privacy/consents', description: 'Get consent records' },
    { method: 'POST', endpoint: '/api/v1/security/vp/privacy/classify', description: 'Classify data' },
  ];

  const performanceMetrics = [
    { label: 'Data Subjects', value: '2.4M', trend: '+15%' },
    { label: 'Requests Processed', value: '45,230', trend: '+28%' },
    { label: 'Compliance Rate', value: '99%', trend: '+3%' },
    { label: 'PIA Completed', value: '234', trend: '+42%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI VP Privacy leads enterprise privacy program, data protection, and privacy compliance. It oversees 3 specialized sub-agents for comprehensive privacy management coverage.
        </Text>
      </View>
      ...
      <AgentFeatures agentId="vp-privacy" agentName="AI VP Privacy" />
