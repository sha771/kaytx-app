...
import { LayoutDashboard, Activity, Star, Users, CircleCheckBig, Clock, ArrowRight, Zap, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, Shield, Target, Lock, FileCheck } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function SecurityArchitectPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Systems Secured', value: '156', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.6%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.5s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.6%', icon: Zap, color: '#C62828' },
  ];

  const capabilities = ['Security Architecture', 'Threat Modeling', 'Security Design', 'Control Mapping', 'Security Standards', 'Architecture Review', 'Security Patterns', 'Zero Trust Architecture'];

  const responsibilities = [
    'Design and maintain enterprise security architecture',
    'Conduct threat modeling for systems and applications',
    'Create security design specifications and standards',
    'Map security controls to frameworks and requirements',
    'Develop and enforce security architecture standards',
    'Review and approve security designs for new systems',
    'Implement security patterns and best practices',
    'Design and implement zero trust architecture'
  ];

  const activities = [
    { time: '3 min ago', text: 'Completed threat model for payment system', icon: Target },
    { time: '18 min ago', text: 'Reviewed security design for new app', icon: FileCheck },
    { time: '1 hour ago', text: 'Updated security architecture blueprint', icon: LayoutDashboard },
    { time: '3 hours ago', text: 'Mapped controls to NIST framework', icon: Lock },
  ];

  const subAgents = [
    { name: 'AI Security Design Reviewer', route: '/ai-agent/security/sub-agents/security-design-reviewer', desc: 'Security design review & validation', icon: FileCheck },
    { name: 'AI Threat Modeler', route: '/ai-agent/security/sub-agents/threat-modeler', desc: 'Threat modeling & analysis', icon: Target },
    { name: 'AI Control Mapper', route: '/ai-agent/security/sub-agents/control-mapper', desc: 'Security control mapping & documentation', icon: Lock },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/architect/systems', description: 'Get secured systems' },
    { method: 'POST', endpoint: '/api/v1/security/architect/threat-model', description: 'Create threat model' },
    { method: 'GET', endpoint: '/api/v1/security/architect/designs', description: 'Get designs' },
    { method: 'POST', endpoint: '/api/v1/security/architect/review', description: 'Review security design' },
  ];

  const performanceMetrics = [
    { label: 'Systems Secured', value: '156', trend: '+22%' },
    { label: 'Designs Reviewed', value: '234', trend: '+18%' },
    { label: 'Threat Models', value: '89', trend: '+35%' },
    { label: 'Controls Mapped', value: '456', trend: '+28%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#C6282818', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#C6282825' }]}>
          <LayoutDashboard size={56} color="#C62828" />
        </View>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#C6282822' }]}><Star size={12} color="#C62828" /><Text style={[styles.badgeText, { color: '#C62828' }]}>Architect</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Security Architect designs enterprise security architecture, conducts threat modeling, and ensures secure system design. It oversees 3 specialized sub-agents for comprehensive security architecture coverage.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#C6282812' }]} onPress={() => router.push('/ai-agent/security')}>
            <LayoutDashboard size={20} color="#C62828" />
      ...
      <AgentFeatures agentId="security-architect" agentName="AI Security Architect" />
