...
import { Target, Activity, Users, Clock, Target as TargetIcon, Zap, ArrowRight, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, Bug, FileText, AlertTriangle, Shield } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function AiPenetrationTesterPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Vulnerabilities Found', value: '1,234', icon: Bug, color: '#34C759' },
    { label: 'Uptime', value: '99.5%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.3s', icon: Clock, color: '#FF9500' },
    { label: 'Tests Completed', value: '456', icon: TargetIcon, color: '#C62828' },
  ];

  const capabilities = ['Penetration Testing', 'Vulnerability Assessment', 'Exploit Research', 'Security Testing', 'Red Team Operations', 'Remediation Guidance', 'Security Audits', 'Code Review'];

  const responsibilities = [
    'Conduct penetration testing and vulnerability assessments',
    'Research and identify exploits',
    'Report vulnerabilities and provide remediation advice',
    'Perform security testing and red team operations',
  ...
  const activities = [
    { time: '2 min ago', text: 'Identified critical vulnerability', icon: Bug },
    { time: '15 min ago', text: 'Completed penetration test', icon: TargetIcon },
    { time: '1 hour ago', text: 'Researched new exploit', icon: AlertTriangle },
    { time: '3 hours ago', text: 'Generated vulnerability report', icon: FileText },
  ];

  const subAgents = [
    { name: 'AI Exploit Researcher', route: '/ai-agent/security/sub-agents/exploit-researcher', desc: 'Exploit research & development', icon: Bug },
    { name: 'AI Vulnerability Reporter', route: '/ai-agent/security/sub-agents/vulnerability-reporter', desc: 'Vulnerability reporting & tracking', icon: FileText },
    { name: 'AI Remediation Advisor', route: '/ai-agent/security/sub-agents/remediation-advisor', desc: 'Remediation guidance & support', icon: Shield },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/pentest/vulns', description: 'Get vulnerabilities' },
    { method: 'POST', endpoint: '/api/v1/security/pentest/scan', description: 'Start penetration test' },
    { method: 'GET', endpoint: '/api/v1/security/pentest/reports', description: 'Get reports' },
    { method: 'POST', endpoint: '/api/v1/security/pentest/remediate', description: 'Get remediation' },
  ];

  const performanceMetrics = [
    { label: 'Vulns Found', value: '1,234', trend: '+42%' },
    { label: 'Tests Completed', value: '456', trend: '+28%' },
    { label: 'Critical Issues', value: '89', trend: '-15%' },
    { label: 'Remediation Rate', value: '95%', trend: '+12%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#C6282818', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#C6282822' }]}><Users size={12} color="#C62828" /><Text style={[styles.badgeText, { color: '#C62828' }]}>Tester</Text></View>
          <View style={[styles.badge, { backgroundColor: '#FF950022' }]}><TargetIcon size={12} color="#FF9500" /><Text style={[styles.badgeText, { color: '#FF9500' }]}>{subAgents.length} Sub-Agents</Text></View>
        </View>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Penetration Tester conducts security testing, vulnerability assessments, and red team operations. It oversees 3 specialized sub-agents for comprehensive penetration testing coverage.
        </Text>
      </View>
      ...
      <AgentFeatures agentId="ai-penetration-tester" agentName="AI Penetration Tester" />
