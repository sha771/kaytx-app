...
import { FileCheck, Activity, Users, Clock, Target, Zap, ArrowRight, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, ClipboardList, UserCheck, Search, FileText } from 'lucide-react-native';
...
export default function AiSecurityCompliancePage() {
  ...
  const stats = [
    { label: 'Audits Completed', value: '234', icon: FileCheck, color: '#34C759' },
  ...
  const capabilities = ['Compliance Management', 'Framework Mapping', 'Evidence Gathering', 'Audit Support', 'Regulatory Compliance', 'Policy Enforcement', 'Risk Assessment', 'Control Testing'];

  const responsibilities = [
    'Manage security compliance programs',
    'Map controls to compliance frameworks',
    'Gather evidence for audits',
    'Support audit activities and liaison',
  ...
  const activities = [
    { time: '2 min ago', text: 'Completed framework mapping', icon: ClipboardList },
    { time: '15 min ago', text: 'Gathered audit evidence', icon: FileText },
    { time: '1 hour ago', text: 'Conducted compliance assessment', icon: FileCheck },
    { time: '3 hours ago', text: 'Supported audit review', icon: UserCheck },
  ];

  const subAgents = [
    { name: 'AI Framework Mapper', route: '/ai-agent/security/sub-agents/framework-mapper', desc: 'Compliance framework mapping', icon: ClipboardList },
    { name: 'AI Evidence Gatherer', route: '/ai-agent/security/sub-agents/evidence-gatherer', desc: 'Audit evidence collection', icon: FileText },
    { name: 'AI Audit Liaison', route: '/ai-agent/security/sub-agents/audit-liaison', desc: 'Audit coordination & support', icon: UserCheck },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/compliance/status', description: 'Get compliance status' },
    { method: 'POST', endpoint: '/api/v1/security/compliance/assess', description: 'Assess compliance' },
    { method: 'GET', endpoint: '/api/v1/security/compliance/frameworks', description: 'Get frameworks' },
    { method: 'POST', endpoint: '/api/v1/security/compliance/evidence', description: 'Gather evidence' },
  ];

  const performanceMetrics = [
    { label: 'Audits Completed', value: '234', trend: '+28%' },
    { label: 'Frameworks Mapped', value: '45', trend: '+35%' },
    { label: 'Compliance Rate', value: '98.5%', trend: '+5%' },
    { label: 'Evidence Items', value: '12,456', trend: '+42%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#C6282818', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#C6282825' }]}>
          <FileCheck size={56} color="#C62828" />
        </View>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#C6282822' }]}><Users size={12} color="#C62828" /><Text style={[styles.badgeText, { color: '#C62828' }]}>Specialist</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Security Compliance Specialist manages compliance programs, maps controls to frameworks, and supports audits. It oversees 3 specialized sub-agents for comprehensive compliance coverage.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#C6282812' }]} onPress={() => router.push('/ai-agent/security')}>
            <FileCheck size={20} color="#C62828" />
      ...
      <AgentFeatures agentId="ai-security-compliance" agentName="AI Security Compliance Specialist" />
