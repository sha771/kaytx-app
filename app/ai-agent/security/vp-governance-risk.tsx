...
import { Scale, Activity, Star, Users, CircleCheckBig, Clock, ArrowRight, Zap, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, FileText, Target, ClipboardList, AlertTriangle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function VPGoveranceRiskPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Risks Managed', value: '847', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.5%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.4s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.5%', icon: Zap, color: '#C62828' },
  ];

  const capabilities = ['Governance Framework', 'Risk Management', 'Policy Development', 'Compliance Oversight', 'Risk Assessment', 'Regulatory Alignment', 'Enterprise Risk', 'Control Framework'];

  const responsibilities = [
    'Develop and maintain security governance frameworks',
    'Lead enterprise risk management programs',
    'Create and enforce security policies and standards',
    'Oversee compliance with security regulations',
    'Conduct enterprise risk assessments',
    'Ensure regulatory alignment across all operations',
    'Manage enterprise risk register and reporting',
    'Design and implement control frameworks'
  ];

  const activities = [
    { time: '5 min ago', text: 'Updated governance framework v3.2', icon: Scale },
    { time: '25 min ago', text: 'Reviewed enterprise risk register', icon: ClipboardList },
    { time: '1 hour ago', text: 'Approved new security policy', icon: FileText },
    { time: '3 hours ago', text: 'Conducted risk assessment for new initiative', icon: Target },
  ];

  const subAgents = [
    { name: 'AI Risk Register Manager', route: '/ai-agent/security/sub-agents/risk-register-manager', desc: 'Risk register management & tracking', icon: ClipboardList },
    { name: 'AI Policy Drafter', route: '/ai-agent/security/sub-agents/policy-drafter', desc: 'Security policy drafting & maintenance', icon: FileText },
    { name: 'AI Risk Appetite Monitor', route: '/ai-agent/security/sub-agents/risk-appetite-monitor', desc: 'Risk appetite monitoring & reporting', icon: AlertTriangle },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/vp/governance/status', description: 'Get governance status' },
    { method: 'POST', endpoint: '/api/v1/security/vp/governance/risk', description: 'Assess risk' },
    { method: 'GET', endpoint: '/api/v1/security/vp/governance/policies', description: 'Get policies' },
    { method: 'POST', endpoint: '/api/v1/security/vp/governance/policy', description: 'Create policy' },
  ];

  const performanceMetrics = [
    { label: 'Risks Managed', value: '847', trend: '+22%' },
    { label: 'Policies Updated', value: '156', trend: '+18%' },
    { label: 'Compliance Rate', value: '99%', trend: '+5%' },
    { label: 'Controls Implemented', value: '234', trend: '+35%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI VP Governance & Risk leads security governance, enterprise risk management, and policy development. It oversees 3 specialized sub-agents for comprehensive governance and risk coverage.
        </Text>
      </View>
      ...
      <AgentFeatures agentId="vp-governance-risk" agentName="AI VP Governance & Risk" />
