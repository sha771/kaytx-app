...
import { UserCheck, Activity, Users, Clock, Target, Zap, ArrowRight, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, Shield, Key, UsersRound, AlertTriangle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function AiIdentityManagerPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Access Reviews', value: '12,456', icon: UserCheck, color: '#34C759' },
    { label: 'Uptime', value: '99.5%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.3s', icon: Clock, color: '#FF9500' },
    { label: 'Compliance', value: '98.5%', icon: Target, color: '#C62828' },
  ];

  const capabilities = ['Identity Management', 'Access Reviews', 'Role Modeling', 'Privilege Monitoring', 'IAM Operations', 'Access Control', 'User Provisioning', 'MFA Management'];

  const responsibilities = [
    'Manage identity and access management',
    'Conduct access reviews and certifications',
    'Model and optimize role-based access',
    'Monitor privilege escalation risks',
    'Ensure IAM compliance',
    'Manage user provisioning and de-provisioning',
    'Implement and monitor MFA',
    'Report on access metrics'
  ];

  const activities = [
    { time: '2 min ago', text: 'Completed access review', icon: UserCheck },
    { time: '15 min ago', text: 'Updated role permissions', icon: UsersRound },
    { time: '1 hour ago', text: 'Detected privilege escalation', icon: AlertTriangle },
    { time: '3 hours ago', text: 'Generated access report', icon: Key },
  ];

  const subAgents = [
    { name: 'AI Access Reviewer', route: '/ai-agent/security/sub-agents/access-reviewer', desc: 'Access review & certification', icon: UserCheck },
    { name: 'AI Role Modeler', route: '/ai-agent/security/sub-agents/role-modeler', desc: 'Role modeling & optimization', icon: UsersRound },
    { name: 'AI Privilege Escalation Monitor', route: '/ai-agent/security/sub-agents/privilege-escalation-monitor', desc: 'Privilege escalation detection', icon: AlertTriangle },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/identity/access', description: 'Get access status' },
    { method: 'POST', endpoint: '/api/v1/security/identity/review', description: 'Start access review' },
    { method: 'GET', endpoint: '/api/v1/security/identity/roles', description: 'Get role models' },
    { method: 'POST', endpoint: '/api/v1/security/identity/monitor', description: 'Monitor privileges' },
  ];

  const performanceMetrics = [
    { label: 'Access Reviews', value: '12,456', trend: '+35%' },
    { label: 'Roles Modeled', value: '234', trend: '+28%' },
    { label: 'Alerts Triggered', value: '89', trend: '-15%' },
    { label: 'Compliance Rate', value: '98.5%', trend: '+5%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#C6282818', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#C6282825' }]}>
          <UserCheck size={56} color="#C62828" />
        </View>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#C6282822' }]}><Users size={12} color="#C62828" /><Text style={[styles.badgeText, { color: '#C62828' }]}>Manager</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Identity Manager manages identity and access, conducts access reviews, and monitors privilege escalation. It oversees 3 specialized sub-agents for comprehensive IAM coverage.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#C6282812' }]} onPress={() => router.push('/ai-agent/security')}>
            <UserCheck size={20} color="#C62828" />
      ...
      <AgentFeatures agentId="ai-identity-manager" agentName="AI Identity Manager" />
