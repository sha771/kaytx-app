...
import { ShieldAlert, Activity, Star, Users, CircleCheckBig, Clock, ArrowRight, Zap, ChevronRight, TrendingUp, BarChart3, MessageSquare, Calendar, Settings, Eye, Target, FileText, Lock } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function VPCybersecurityPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Cyber Threats Blocked', value: '28,900', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.2s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.9%', icon: Zap, color: '#C62828' },
  ];

  const capabilities = ['Cybersecurity Strategy', 'Threat Intelligence', 'Cyber Risk Management', 'Security Architecture', 'Vulnerability Management', 'Threat Landscape Analysis', 'Security Innovation', 'Cross-functional Leadership'];

  const responsibilities = [
    'Develop and execute enterprise cybersecurity strategy',
    'Lead threat intelligence and cyber threat analysis',
    'Manage enterprise cyber risk assessment and mitigation',
    'Oversee security architecture and design reviews',
    'Drive vulnerability management programs',
    'Analyze threat landscape and emerging risks',
    'Innovate security technologies and practices',
    'Lead cross-functional cybersecurity initiatives'
  ];

  const activities = [
    { time: '2 min ago', text: 'Blocked advanced persistent threat campaign', icon: ShieldAlert },
    { time: '15 min ago', text: 'Updated cybersecurity strategy for Q3', icon: Target },
    { time: '1 hour ago', text: 'Reviewed threat intelligence feeds', icon: Eye },
    { time: '3 hours ago', text: 'Approved security architecture changes', icon: Lock },
  ];

  const subAgents = [
    { name: 'AI Threat Landscape Monitor', route: '/ai-agent/security/sub-agents/threat-landscape-monitor', desc: 'Threat landscape monitoring & analysis', icon: Eye },
    { name: 'AI Cyber Risk Quantifier', route: '/ai-agent/security/sub-agents/cyber-risk-quantifier', desc: 'Cyber risk quantification & metrics', icon: Target },
    { name: 'AI Security Architecture Reviewer', route: '/ai-agent/security/sub-agents/security-architecture-reviewer', desc: 'Security architecture review & assessment', icon: Lock },
  ];

  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/security/vp/cyber/status', description: 'Get cybersecurity status' },
    { method: 'POST', endpoint: '/api/v1/security/vp/cyber/threat', description: 'Analyze threat' },
    { method: 'GET', endpoint: '/api/v1/security/vp/cyber/risks', description: 'Get risk assessment' },
    { method: 'POST', endpoint: '/api/v1/security/vp/cyber/architect', description: 'Review architecture' },
  ];

  const performanceMetrics = [
    { label: 'Threats Blocked', value: '28,900', trend: '+35%' },
    { label: 'Risk Reduction', value: '78%', trend: '+22%' },
    { label: 'Architecture Reviews', value: '156', trend: '+18%' },
    { label: 'Vulnerabilities Patched', value: '2,340', trend: '+45%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI VP Cybersecurity leads enterprise cybersecurity strategy, threat intelligence, and cyber risk management. It oversees 3 specialized sub-agents for comprehensive cybersecurity coverage.
        </Text>
      </View>
      ...
      <AgentFeatures agentId="vp-cybersecurity" agentName="AI VP Cybersecurity" />
