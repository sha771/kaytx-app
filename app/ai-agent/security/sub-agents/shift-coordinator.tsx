...
export default function ShiftCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const colors = theme.colors;
  const [activeTab, setActiveTab] = useState('overview');
  const [autoScheduling, setAutoScheduling] = useState(true);
  const [handoffEnforcement, setHandoffEnforcement] = useState(true);
  const [overtimeAlerts, setOvertimeAlerts] = useState(true);

  const stats = [
    { label: 'Shifts Managed', value: '847', change: '+23', icon: Calendar, color: '#EF4444', trend: 'up' },
    { label: 'Handoff Accuracy', value: '99.1%', change: '+0.8%', icon: CheckCircle2, color: '#10B981', trend: 'up' },
    { label: 'Coverage Rate', value: '100%', change: '+0%', icon: Users, color: '#F59E0B', trend: 'up' },
    { label: 'Overtime Hours', value: '12 hrs', change: '-4 hrs', icon: Timer, color: '#8B5CF6', trend: 'down' },
  ];

  const kpis = [
    { label: 'Shift Coverage', value: '100%', target: '100%', status: 'meeting', icon: Users },
    { label: 'Handoff Completion', value: '99.5%', target: '98%', status: 'exceeding', icon: RefreshCw },
    { label: 'Analyst Availability', value: '97.8%', target: '95%', status: 'exceeding', icon: UserCheck },
    { label: 'Schedule Accuracy', value: '98.2%', target: '95%', status: 'exceeding', icon: Clock },
  ];

  const capabilities = [
    'Shift Scheduling', 'Resource Allocation', 'Handoff Management', 'Coverage Optimization',
    'Overtime Tracking', 'On-Call Coordination', 'Analyst Assignment', 'Workload Balancing',
    'Time Zone Management', 'Break Scheduling', 'Emergency Coverage', 'Skill-Based Routing',
    'Availability Tracking', 'Shift Swap Management', 'Performance Monitoring'
  ];

  const responsibilities = [
    'Create and manage SOC analyst shift schedules',
    'Allocate resources based on alert volume predictions',
    'Manage shift handoff procedures and documentation',
    'Optimize coverage to ensure 24/7 operations',
    'Track and manage overtime to prevent burnout',
    'Coordinate on-call rotations for after-hours support',
    'Assign analysts to shifts based on skills and expertise',
    'Balance workload across all shift teams',
    'Manage time zone differences for global SOC operations',
    'Schedule breaks to maintain analyst alertness',
    'Arrange emergency coverage for unexpected absences',
    'Route incidents to analysts with appropriate skills',
    'Track analyst availability and time-off requests',
    'Process shift swap requests between analysts',
    'Monitor shift performance metrics and report issues'
  ];

  const activities = [
    { action: 'Completed handoff', target: 'Day shift → Night shift', time: '30 mins ago', icon: RefreshCw },
    { action: 'Approved swap', target: 'Analyst J. Smith', time: '1 hour ago', icon: UserCheck },
    { action: 'Updated schedule', target: 'Week 48 coverage', time: '2 hours ago', icon: Calendar },
    { action: 'Assigned coverage', target: 'Emergency absence', time: '3 hours ago', icon: Users },
    { action: 'Tracked overtime', target: 'Weekly report', time: '4 hours ago', icon: Timer },
    { action: 'Balanced workload', target: 'Alert queue distribution', time: '5 hours ago', icon: Gauge },
  ];

  const quickActions = [
    { label: 'View Schedule', icon: Calendar, color: '#EF4444' },
    { label: 'Manage Handoff', icon: RefreshCw, color: '#F59E0B' },
    { label: 'Assign Analyst', icon: UserCheck, color: '#10B981' },
    { label: 'Request Swap', icon: Users, color: '#8B5CF6' },
    { label: 'Track Overtime', icon: Timer, color: '#3B82F6' },
    { label: 'Emergency Cover', icon: AlertTriangle, color: '#DC2626' },
    { label: 'View Coverage', icon: Gauge, color: '#EC4899' },
    { label: 'Notify Team', icon: Bell, color: '#10B981' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.hero}>
        ...
        <Text style={[styles.heroTitle, { color: colors.text }]}>AI Shift Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: colors.textSecondary }]}>
          SOC shift scheduling and coordination specialist
        </Text>
      ...
      <View style={styles.tabContainer}>
        {['overview', 'shifts', 'capabilities', 'settings'].map((tab) => (
          <TouchableOpacity
            key={tab}
      ...
      <View style={[styles.section, { backgroundColor: colors.card }]}>
        ...
        <Text style={[styles.overviewText, { color: colors.textSecondary }]}>
          The AI Shift Coordinator manages SOC analyst scheduling, shift handoffs, and resource allocation to ensure 
          continuous 24/7 security monitoring. This agent optimizes coverage, balances workloads, and coordinates 
          on-call rotations for seamless security operations.
        </Text>
      </View>
      </>)}
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: colors.card }]}>
          ...
          {[
            { endpoint: '/consult/shift-coordinator', desc: 'Consult on shift coordination' },
            { endpoint: '/shift-coordinator/schedule', desc: 'Manage shift schedules' },
            { endpoint: '/shift-coordinator/handoff', desc: 'Process shift handoff' },
            { endpoint: '/shift-coordinator/coverage', desc: 'Check coverage status' },
          ].map((item, index) => (
        ...
        <View style={[styles.section, { backgroundColor: colors.card }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              ...
              <Text style={[styles.featureToggleDesc, { color: colors.textSecondary }]}>Automatically optimize shift schedules</Text>
            </View>
            <Switch value={autoScheduling} onValueChange={setAutoScheduling} trackColor={{ false: '#767577', true: '#DC2626' }} />
          </View>

          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={[styles.featureToggleTitle, { color: colors.text }]}>Handoff Enforcement</Text>
              <Text style={[styles.featureToggleDesc, { color: colors.textSecondary }]}>Enforce handoff documentation</Text>
            </View>
            <Switch value={handoffEnforcement} onValueChange={setHandoffEnforcement} trackColor={{ false: '#767577', true: '#DC2626' }} />
          </View>

          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={[styles.featureToggleTitle, { color: colors.text }]}>Overtime Alerts</Text>
              <Text style={[styles.featureToggleDesc, { color: colors.textSecondary }]}>Alert on overtime threshold breaches</Text>
            </View>
            <Switch value={overtimeAlerts} onValueChange={setOvertimeAlerts} trackColor={{ false: '#767577', true: '#DC2626' }} />
          </View>
        </View>
      )}

      {/* Agent Features */}
      <AgentFeatures agentId="shift-coordinator" agentName="AI Shift Coordinator" />
