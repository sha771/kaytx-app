...
export default function AgentPage() {
  ...
  const stats = [{label:'Identities',value:'12K+',icon:Users,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Provision',value:'<2min',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.7%',icon:Target,color:'#581C84'}];
  const capabilities = ['IAM', 'SSO', 'MFA', 'RBAC', 'PAM', 'Identity Governance'];
  const responsibilities = ['Identity lifecycle management & provisioning','Access control policy enforcement','Multi-factor authentication deployment','Role-based access control management','Privileged access monitoring & rotation','Identity governance & compliance reporting'];
  const activities = [{time:'3 min ago',text:'Provisioned 45 new user accounts',icon:Users},{time:'20 min ago',text:'Rotated privileged access credentials',icon:Key},{time:'1 hour ago',text:'Enforced MFA for finance department',icon:Lock}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#581C8420' }]}><Key size={48} color="#581C84" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Identity Manager</Text>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#581C8422' }]}><Star size={12} color="#581C84" /><Text style={[styles.badgeText, { color: '#581C84' }]}>Specialist</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Identity & Access Manager AI controls user identities, access permissions, and authentication policies across the enterprise, ensuring zero-trust security and compliance.</Text></View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Capabilities</Text><View style={styles.tagsContainer}>{capabilities.map((cap,i)=>(<View key={i} style={[styles.tag, { backgroundColor: '#581C8418' }]}><Text style={[styles.tagText, { color: '#581C84' }]}>{cap}</Text></View>))}</View></View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Key Responsibilities</Text>{responsibilities.map((item,i)=>(<View key={i} style={styles.responsibilityRow}><ArrowRight size={14} color="#581C84" /><Text style={[styles.responsibilityText, { color: theme.colors.textSecondary }]}>{item}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Recent Activity</Text>{activities.map((act,i)=>(<View key={i} style={styles.activityRow}><View style={[styles.activityIcon, { backgroundColor: '#581C8415' }]}><act.icon size={14} color="#581C84" /></View><View style={styles.activityContent}><Text style={[styles.activityText, { color: theme.colors.text }]}>{act.text}</Text><Text style={[styles.activityTime, { color: theme.colors.textSecondary }]}>{act.time}</Text></View></View>))}</View>
      <AgentFeatures agentId="identity-manager-1" agentName="Identity Manager" />
    </ScrollView>
  );
}
