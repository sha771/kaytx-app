...
import {
  Cloud, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight,
  Users, MessageSquare, ChartBarBig, TrendingUp, AlertTriangle,
  Shield, ChevronRight, Zap as Lightning, Globe, Calculator
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const SUB_AGENTS = [
  { id: 'event-simulator', name: 'AI Event Simulator', icon: Lightning, desc: 'Simulates catastrophic events including hurricanes, earthquakes, and floods' },
  { id: 'exposure-aggregator', name: 'AI Exposure Aggregator', icon: Globe, desc: 'Aggregates and manages exposure data across geographic zones and lines' },
  { id: 'loss-estimator', name: 'AI Loss Estimator', icon: Calculator, desc: 'Estimates catastrophe losses using probabilistic models and event footprints' },
];

const QUICK_ACTIONS = [
  { label: 'Cat Models', icon: Cloud },
  { label: 'Team Chat', icon: MessageSquare },
  { label: 'Exposure', icon: Globe },
  { label: 'Loss Est.', icon: Calculator },
];

const METRICS = [
  { label: 'Events Simulated', value: '3,994', change: '+25%', trend: 'up' },
  { label: 'Model Accuracy', value: '98.0%', change: '+1.5%', trend: 'up' },
  { label: 'Exposure Value', value: '$4.2B', change: '+8%', trend: 'up' },
  { label: 'PML 1-in-100', value: '$312M', change: '-5%', trend: 'up' },
];

export default function CatastropheModelerPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks', value: '3,994', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '1.7s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.0%', icon: Target, color: '#004D40' }
  ];

  const capabilities = [
    'Cat Modeling', 'Scenario Analysis', 'Exposure Management',
    'Reinsurance Optimization', 'GIS Mapping', 'Climate Risk',
    'Event Simulation', 'Exposure Aggregation', 'Loss Estimation',
    'Probabilistic Modeling', 'Hazard Mapping', 'Capital Planning'
  ];

  const responsibilities = [
    'Catastrophe loss modeling, simulation, and probabilistic analysis',
    'Scenario analysis and stress testing for extreme events',
    'Exposure data management, quality assurance, and aggregation',
    'Reinsurance optimization using catastrophe model outputs',
    'GIS-based risk mapping and geographic concentration analysis',
    'Climate change risk assessment and long-term trend modeling',
    'Event simulation for hurricanes, earthquakes, floods, and wildfires',
    'Exposure aggregation across geographic zones and business lines',
    'Loss estimation using probabilistic models and event footprints',
    'Capital planning and solvency assessment using cat model outputs'
  ];

  const activities = [
    { time: '3 min ago', text: 'Ran hurricane season simulations: 10K events across Gulf states', icon: Cloud },
    { time: '6 min ago', text: 'Updated exposure database with Q4 portfolio changes', icon: Globe },
    { time: '9 min ago', text: 'Optimized reinsurance structure: reduced PML by $45M', icon: ChartBarBig },
    { time: '14 min ago', text: 'Simulated M7.5 earthquake on New Madrid fault: $1.2B loss', icon: Lightning },
    { time: '28 min ago', text: 'Estimated wildfire losses for California portfolio: $89M', icon: Calculator },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}>
          <Cloud size={48} color="#004D40" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Catastrophe Modeler</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Catastrophe Modeling & Exposure Management</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#004D4022' }]}><Star size={12} color="#004D40" /><Text style={[styles.badgeText, { color: '#004D40' }]}>Modeler</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Catastrophe Modeler leads catastrophe modeling, exposure management, and loss estimation across the Insurance division. This agent simulates extreme events, manages exposure data, and optimizes reinsurance structures. It orchestrates sub-agents for event simulation, exposure aggregation, and loss estimation.</Text></View>
      ...
      <AgentFeatures agentId="catastrophe-modeler" agentName="AI Catastrophe Modeler" />
