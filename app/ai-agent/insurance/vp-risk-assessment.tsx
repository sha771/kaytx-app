...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3,581',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.7%',icon:Target,color:'#004D40'}];
  const capabilities = ['Risk Strategy','Assessment Framework','Regulatory Relations','Capital Planning','Emerging Risk','Reporting'];
  const responsibilities = ['Risk assessment strategy & framework','Risk evaluation methodology development','Regulatory relationship management','Capital planning & allocation support','Emerging risk identification & monitoring','Risk reporting to senior leadership'];
  const activities = [{time:'3 min ago',text:'Presented risk assessment to board',icon: CircleCheckBig},{time:'6 min ago',text:'Updated risk evaluation methodology',icon:Clock},{time:'9 min ago',text:'Reviewed emerging risk landscape',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}><Target size={48} color="#004D40" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Risk Assessment</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Risk Assessment</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#004D4022' }]}><Star size={12} color="#004D40" /><Text style={[styles.badgeText, { color: '#004D40' }]}>VP Level</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Risk Assessment AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Insurance division.</Text></View>
      ...
      <AgentFeatures agentId="vp-risk-assessment" agentName="VP Risk Assessment" />
    </ScrollView>
  );
}
