...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, Database, FileText, Shield, ShieldCheck, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Exception Evaluation","Risk Assessment","Authority Verification","Approval Workflow","Documentation Requirements","Precedent Analysis","Escalation Management","Audit Trail Maintenance"];
const RESPONSIBILITIES = ["Evaluate underwriting exception requests against guidelines","Assess risk implications of proposed exceptions","Verify authority levels for exception approvals","Manage exception approval workflows and documentation","Analyze precedent cases for consistent exception handling","Maintain audit trails for all exception decisions"];
const METRICS = [{"label":"Exceptions Reviewed","value":"127","change":"+8","trend":"up"},{"label":"Approval Rate","value":"68%","change":"+4%","trend":"up"},{"label":"Avg Decision Time","value":"2.4 hrs","change":"-18%","trend":"up"},{"label":"Audit Compliance","value":"100%","change":"0%","trend":"up"}];

export default function ExceptionApproverPage() {
  ...
  const activities = [{"time":"4 min ago","text":"Approved exception for high-value commercial property","icon":"ShieldCheck"},{"time":"11 min ago","text":"Denied exception request due to concentration risk","icon":"AlertTriangle"},{"time":"18 min ago","text":"Updated exception precedent database","icon":"Database"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <ShieldCheck size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Exception Approver"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Exception Approver - Sub-agent supporting AI Underwriting Manager. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="exception-approver" agentName="AI Exception Approver" />
