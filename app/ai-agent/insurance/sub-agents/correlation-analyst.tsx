...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, Eye, FileText, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Correlation Analysis","Dependency Modeling","Copula Analysis","Tail Dependency","Diversification Assessment","Correlation Monitoring","Regime Detection","Correlation Forecasting"];
const RESPONSIBILITIES = ["Analyze correlations between risk factors and lines of business","Model dependencies using copulas and multivariate distributions","Assess tail dependencies for extreme event risk aggregation","Evaluate diversification benefits across the insurance portfolio","Monitor correlation stability and detect regime changes","Forecast correlation dynamics for forward-looking risk models"];
const METRICS = [{"label":"Correlations Tracked","value":"342","change":"+28","trend":"up"},{"label":"Tail Dependency","value":"0.42","change":"-0.04","trend":"up"},{"label":"Diversification Benefit","value":"18%","change":"+2%","trend":"up"},{"label":"Regime Alerts","value":"1","change":"-2","trend":"up"}];

export default function CorrelationAnalystPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '342', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"5 min ago","text":"Updated correlation matrix for multi-line risk model","icon":"BarChart3"},{"time":"13 min ago","text":"Assessed tail dependency between property and cat lines","icon":"AlertTriangle"},{"time":"21 min ago","text":"Detected correlation regime shift in credit markets","icon":"Eye"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF572220' }]}>
          <BarChart3 size={56} color="#FF5722" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Correlation Analyst"}</Text>
      ...
      <AgentFeatures agentId="correlation-analyst" agentName="AI Correlation Analyst" />
