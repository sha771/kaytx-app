...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, PieChart, RefreshCw, Shield, Target, TrendingUp, UserCheck, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Risk Profiling","Customer Risk Assessment","Demographic Analysis","Exposure Profiling","Risk Categorization","Portfolio Segmentation","Predictive Profiling","Profile Monitoring"];
const RESPONSIBILITIES = ["Create comprehensive risk profiles for insurance customers","Assess customer risk based on demographic and exposure factors","Categorize customers into risk segments for underwriting guidance","Develop predictive risk profiles using behavioral and historical data","Monitor customer risk profiles for changes and updates","Segment customer portfolio by risk characteristics for analytics"];
const METRICS = [{"label":"Profiles Created","value":"18,421","change":"+847","trend":"up"},{"label":"Segmentation Accuracy","value":"96.4%","change":"+1.2%","trend":"up"},{"label":"Update Frequency","value":"Daily","change":"On Track","trend":"up"},{"label":"Predictive Power","value":"0.91","change":"+0.03","trend":"up"}];
...
export default function RiskProfilerPage() {
  ...
  const activities = [{"time":"5 min ago","text":"Created risk profiles for 42 new commercial accounts","icon":"UserCheck"},{"time":"13 min ago","text":"Updated 127 customer profiles with new exposure data","icon":"RefreshCw"},{"time":"21 min ago","text":"Segmented portfolio into 8 risk categories","icon":"PieChart"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <UserCheck size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Risk Profiler"}</Text>
      ...
      <AgentFeatures agentId="risk-profiler" agentName="AI Risk Profiler" />
