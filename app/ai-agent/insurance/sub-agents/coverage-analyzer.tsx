...
const CAPABILITIES = ["Coverage Analysis","Form Comparison","Limit Assessment","Deductive Evaluation","Endorsement Review","Gap Identification","Policy Interpretation","Regulatory Compliance"];
const RESPONSIBILITIES = ["Analyze coverage options and policy form differences","Compare coverage forms and endorsements for client needs","Assess limit adequacy and recommend coverage levels","Evaluate deductible structures and their risk implications","Review endorsements for coverage modifications and extensions","Identify coverage gaps and recommend solutions"];
const METRICS = [{"label":"Analyses Completed","value":"3,421","change":"+187","trend":"up"},{"label":"Gaps Identified","value":"847","change":"+42","trend":"up"},{"label":"Recommendations","value":"2,847","change":"+142","trend":"up"},{"label":"Client Satisfaction","value":"4.7/5","change":"+0.2","trend":"up"}];

export default function CoverageAnalyzerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '3,421', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"5 min ago","text":"Completed coverage analysis for 12 commercial accounts","icon":"Eye"},{"time":"13 min ago","text":"Identified coverage gaps in 4 renewal policies","icon":"AlertTriangle"},{"time":"21 min ago","text":"Recommended enhanced limits for 7 policies","icon":"TrendingUp"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <Eye size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Coverage Analyzer"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI Underwriter</Text>
