...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Shield, SquareCheck, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Standards Enforcement","Assessment Quality Control","Methodology Validation","Peer Review Coordination","Documentation Compliance","Process Standardization","Best Practice Promotion","Audit Readiness"];
const RESPONSIBILITIES = ["Enforce risk assessment standards across all business units","Conduct quality control reviews on risk assessment outputs","Validate risk assessment methodologies and assumptions","Coordinate peer review processes for complex assessments","Ensure documentation compliance with assessment standards","Promote risk assessment best practices and process improvements"];
const METRICS = [{"label":"Standards Met","value":"98.7%","change":"+1.2%","trend":"up"},{"label":"QC Reviews","value":"234","change":"+28","trend":"up"},{"label":"Methodology Flags","value":"3","change":"-5","trend":"up"},{"label":"Audit Score","value":"97.4%","change":"+0.8%","trend":"up"}];

export default function AssessmentStandardsEnforcerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '98.7', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"6 min ago","text":"Completed QC review on 12 risk assessments","icon":"SquareCheck"},{"time":"14 min ago","text":"Flagged methodology deviation in credit risk assessment","icon":"AlertTriangle"},{"time":"22 min ago","text":"Updated assessment documentation templates","icon":"FileText"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}>
          <SquareCheck size={56} color="#BF360C" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Assessment Standards Enforcer"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI VP Risk Assessment</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#BF360C22' }]}><Briefcase size={12} color="#BF360C" /><Text style={[styles.badgeText, { color: '#BF360C' }]}>Specialist</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Assessment Standards Enforcer - Sub-agent supporting AI VP Risk Assessment. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>A2A Endpoints</Text>{['/consult/assessment-standards-enforcer','/assessment-standards-enforcer/execute','/assessment-standards-enforcer/analyze'].map((endpoint,i)=>(<View key={i} style={styles.endpointRow}><Zap size={14} color="#8B5CF6" /><Text style={[styles.endpointText, { color: theme.colors.textSecondary }]}>{endpoint}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/insurance/vp-risk-assessment')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><Shield size={24} color="#004D40" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>{"AI VP Risk Assessment"}</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent · Insurance Dept</Text></View><ChevronRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="assessment-standards-enforcer" agentName="AI Assessment Standards Enforcer" />
