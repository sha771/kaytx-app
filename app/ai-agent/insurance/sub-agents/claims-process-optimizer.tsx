...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Process Automation","Workflow Optimization","Bottleneck Identification","Cycle Time Reduction","Straight-Through Processing","Resource Allocation","Quality Assurance","Performance Benchmarking"];
const RESPONSIBILITIES = ["Optimize claims processing workflows for speed and accuracy","Identify and eliminate bottlenecks in claims handling processes","Increase straight-through processing rates through automation","Reduce claims cycle time while maintaining quality standards","Allocate claims resources based on volume and complexity forecasts","Benchmark claims performance against industry standards"];
const METRICS = [{"label":"STP Rate","value":"72%","change":"+8%","trend":"up"},{"label":"Cycle Time","value":"3.8 days","change":"-22%","trend":"up"},{"label":"Efficiency Gain","value":"34%","change":"+6%","trend":"up"},{"label":"Cost per Claim","value":"$142","change":"-$18","trend":"up"}];

export default function ClaimsProcessOptimizerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '72', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"2 min ago","text":"Automated FNOL intake for 89% of new claims","icon":"Zap"},{"time":"9 min ago","text":"Reduced auto claims cycle time by 1.2 days","icon":"Clock"},{"time":"16 min ago","text":"Identified bottleneck in commercial claims assignment","icon":"AlertTriangle"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <Zap size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Claims Process Optimizer"}</Text>
      ...
      <AgentFeatures agentId="claims-process-optimizer" agentName="AI Claims Process Optimizer" />
