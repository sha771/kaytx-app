...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, PieChart, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Customer Segmentation","Risk Segmentation","Profitability Analysis","Lifetime Value Modeling","Churn Prediction","Segment Migration","Targeting Strategy","Segment Analytics"];
const RESPONSIBILITIES = ["Segment customer base by risk, profitability, and lifetime value","Analyze segment performance and migration patterns","Model customer lifetime value across risk segments","Predict churn risk and develop retention strategies by segment","Develop targeting strategies for marketing and underwriting","Generate segment analytics and reporting for business intelligence"];
const METRICS = [{"label":"Segments Defined","value":"24","change":"+3","trend":"up"},{"label":"LTV Accuracy","value":"92.4%","change":"+1.8%","trend":"up"},{"label":"Churn Prediction","value":"88%","change":"+4%","trend":"up"},{"label":"Segment Stability","value":"94%","change":"+2%","trend":"up"}];

export default function SegmentationAnalystPage() {
  ...
  const activities = [{"time":"6 min ago","text":"Defined 3 new micro-segments for young driver cohort","icon":"PieChart"},{"time":"14 min ago","text":"Analyzed segment migration for Q3 portfolio review","icon":"BarChart3"},{"time":"22 min ago","text":"Predicted churn risk for 847 renewal policies","icon":"TrendingUp"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <PieChart size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Segmentation Analyst"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Segmentation Analyst - Sub-agent supporting AI Customer Risk Analyst. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="segmentation-analyst" agentName="AI Segmentation Analyst" />
