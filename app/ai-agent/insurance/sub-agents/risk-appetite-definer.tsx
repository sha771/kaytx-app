...
const CAPABILITIES = ["Risk Appetite Calibration","Tolerance Threshold Setting","Appetite Statement Drafting","Quantitative Risk Limits","Risk Capacity Analysis","Stress Test Integration","Appetite Monitoring","Regulatory Alignment"];
const RESPONSIBILITIES = ["Define and calibrate organizational risk appetite across all risk categories","Set quantitative and qualitative risk tolerance thresholds","Draft and maintain risk appetite statements for board approval","Integrate risk appetite with stress testing and capital planning","Monitor risk appetite utilization and trigger breach alerts","Align risk appetite frameworks with regulatory expectations"];
const METRICS = [{"label":"Appetite Statements","value":"12","change":"+2","trend":"up"},{"label":"Threshold Breaches","value":"0","change":"0","trend":"up"},{"label":"Utilization","value":"72%","change":"-4%","trend":"up"},{"label":"Board Approvals","value":"8","change":"+1","trend":"up"}];

export default function RiskAppetiteDefinerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '12', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"5 min ago","text":"Updated risk appetite thresholds for market risk","icon":"Gauge"},{"time":"12 min ago","text":"Calibrated operational risk tolerance levels","icon":"Shield"},{"time":"25 min ago","text":"Drafted revised risk appetite statement","icon":"FileText"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <Gauge size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Risk Appetite Definer"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Risk Appetite Definer - Sub-agent supporting AI Chief Risk Officer. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="risk-appetite-definer" agentName="AI Risk Appetite Definer" />
