...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Shield, Target, TrendingUp, UserPlus, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Claims Assignment","Skill-Based Routing","Workload Balancing","Priority Classification","Adjuster Matching","Geographic Assignment","Specialty Routing","Capacity Management"];
const RESPONSIBILITIES = ["Assign incoming claims to appropriate adjusters based on skills and workload","Route claims by priority, complexity, and geographic requirements","Balance workload across claims team to optimize throughput","Classify claims by priority for appropriate handling speed","Match adjuster specialties to claim types for best outcomes","Manage claims team capacity and forecast assignment needs"];
const METRICS = [{"label":"Claims Assigned","value":"4,821","change":"+247","trend":"up"},{"label":"Match Accuracy","value":"94.7%","change":"+2.1%","trend":"up"},{"label":"Assignment Time","value":"8 min","change":"-34%","trend":"up"},{"label":"Workload Balance","value":"92%","change":"+4%","trend":"up"}];

export default function ClaimsAssignerPage() {
  ...
  const activities = [{"time":"2 min ago","text":"Assigned 14 new auto claims to specialty team","icon":"UserPlus"},{"time":"9 min ago","text":"Rebalanced workload after adjuster availability change","icon":"Users"},{"time":"16 min ago","text":"Routed 3 catastrophe claims to CAT team","icon":"AlertTriangle"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF572220' }]}>
          <UserPlus size={56} color="#FF5722" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Claims Assigner"}</Text>
      ...
      <AgentFeatures agentId="claims-assigner" agentName="AI Claims Assigner" />
