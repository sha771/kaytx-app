...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, Database, FileText, Search, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Risk Evaluation","Underwriting Analysis","Exposure Assessment","Loss History Review","Hazard Identification","Risk Classification","Decision Support","Data Enrichment"];
const RESPONSIBILITIES = ["Evaluate insurance risks for underwriting decision support","Analyze exposures and assess risk quality for submissions","Review loss history and claims data for risk assessment","Identify hazards and risk factors for underwriting consideration","Classify risks according to company underwriting guidelines","Enrich risk data with external sources for comprehensive evaluation"];
const METRICS = [{"label":"Risks Evaluated","value":"4,821","change":"+247","trend":"up"},{"label":"Evaluation Time","value":"12 min","change":"-28%","trend":"up"},{"label":"Data Enrichment","value":"94%","change":"+3%","trend":"up"},{"label":"Decision Accuracy","value":"97.8%","change":"+0.6%","trend":"up"}];

export default function RiskEvaluatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '4,821', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"4 min ago","text":"Evaluated 14 commercial property risk submissions","icon":"Search"},{"time":"12 min ago","text":"Enriched risk data with external hazard scores","icon":"Database"},{"time":"20 min ago","text":"Classified 8 risks per underwriting guidelines","icon":"Shield"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <Search size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Risk Evaluator"}</Text>
      ...
      <AgentFeatures agentId="risk-evaluator" agentName="AI Risk Evaluator" />
