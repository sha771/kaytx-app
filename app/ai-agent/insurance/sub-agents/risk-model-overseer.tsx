...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Brain, Briefcase, ChevronRight, CircleCheckBig, ClipboardList, Clock, FileText, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Model Governance","Validation Oversight","Model Inventory Management","Performance Monitoring","Regulatory Compliance","Model Risk Framework","Version Control","Documentation Standards"];
const RESPONSIBILITIES = ["Oversee all risk models across the enterprise model inventory","Manage model validation schedules and independent review processes","Monitor model performance and trigger recalibration when needed","Ensure regulatory compliance for all risk models (SR 11-7, etc.)","Develop and maintain model risk management framework","Control model versions and documentation standards"];
const METRICS = [{"label":"Models Governed","value":"47","change":"+3","trend":"up"},{"label":"Validation Rate","value":"100%","change":"0%","trend":"up"},{"label":"Performance Alerts","value":"2","change":"-4","trend":"up"},{"label":"Compliance Score","value":"99.1%","change":"+0.3%","trend":"up"}];

export default function RiskModelOverseerPage() {
  ...
  const activities = [{"time":"3 min ago","text":"Completed annual validation for catastrophe model","icon":"Brain"},{"time":"12 min ago","text":"Flagged performance drift in reserving model","icon":"AlertTriangle"},{"time":"20 min ago","text":"Updated model inventory with 3 new entries","icon":"ClipboardList"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}>
          <Brain size={56} color="#BF360C" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Risk Model Overseer"}</Text>
      ...
      <AgentFeatures agentId="risk-model-overseer" agentName="AI Risk Model Overseer" />
