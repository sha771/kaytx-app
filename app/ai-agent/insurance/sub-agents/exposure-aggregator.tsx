...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, DollarSign, FileText, Globe, MapPin, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Exposure Aggregation","Geographic Concentration","Cresta Zone Analysis","Portfolio Overlay","Location Intelligence","Replacement Cost Estimation","Occupancy Classification","Construction Classification"];
const RESPONSIBILITIES = ["Aggregate exposure data across the insurance portfolio by peril zone","Analyze geographic concentration and CRESTA zone exposure","Overlay portfolio exposure with catastrophe hazard maps","Provide location intelligence for underwriting and risk management","Estimate replacement costs for property exposure values","Classify occupancy and construction types for vulnerability assessment"];
const METRICS = [{"label":"Locations Managed","value":"847K","change":"+42K","trend":"up"},{"label":"Concentration Alerts","value":"3","change":"-2","trend":"up"},{"label":"Data Quality","value":"97.4%","change":"+1.2%","trend":"up"},{"label":"Coverage Ratio","value":"94%","change":"+2%","trend":"up"}];

export default function ExposureAggregatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '847K', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"5 min ago","text":"Aggregated exposure for 42K new property locations","icon":"Globe"},{"time":"13 min ago","text":"Analyzed CRESTA zone concentration for hurricane peril","icon":"MapPin"},{"time":"21 min ago","text":"Updated replacement cost estimates for commercial book","icon":"DollarSign"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF572220' }]}>
          <Globe size={56} color="#FF5722" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Exposure Aggregator"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Exposure Aggregator - Sub-agent supporting AI Catastrophe Modeler. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="exposure-aggregator" agentName="AI Exposure Aggregator" />
