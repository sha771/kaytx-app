...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Handshake, Search, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Treaty Negotiation","Terms Development","Pricing Review","Counterparty Assessment","Contract Drafting","Renewal Negotiation","Market Intelligence","Broker Coordination"];
const RESPONSIBILITIES = ["Negotiate reinsurance treaty terms with counterparties and brokers","Develop treaty terms and conditions aligned with risk transfer goals","Review reinsurance pricing and structure for optimal risk/return","Assess counterparty creditworthiness and security requirements","Draft reinsurance contract language and documentation","Manage treaty renewal negotiations and placement timelines"];
const METRICS = [{"label":"Treaties Negotiated","value":"14","change":"+2","trend":"up"},{"label":"Placement Rate","value":"100%","change":"0%","trend":"up"},{"label":"Terms Improvement","value":"+8%","change":"+2%","trend":"up"},{"label":"Counterparty Rating","value":"A+","change":"Stable","trend":"up"}];

export default function TreatyNegotiatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '14', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"5 min ago","text":"Negotiated proportional treaty renewal with Munich Re","icon":"Handshake"},{"time":"13 min ago","text":"Developed terms for new catastrophe excess of loss treaty","icon":"FileText"},{"time":"21 min ago","text":"Reviewed broker market intelligence for Q4 placement","icon":"Search"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <Handshake size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Treaty Negotiator"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Treaty Negotiator - Sub-agent supporting AI Reinsurance Specialist. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="treaty-negotiator" agentName="AI Treaty Negotiator" />
