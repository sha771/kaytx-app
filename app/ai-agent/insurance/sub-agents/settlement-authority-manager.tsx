...
import {
  Activity, ArrowRight, ArrowUp, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Scale, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Authority Level Management","Approval Workflow Design","Settlement Authorization","Escalation Protocols","Financial Limit Controls","Audit Compliance","Delegation Framework","Settlement Tracking"];
const RESPONSIBILITIES = ["Manage settlement authority levels across claims organization","Design and maintain approval workflows for settlement decisions","Authorize settlements within delegated authority limits","Establish escalation protocols for complex or high-value claims","Monitor financial limit controls and authority utilization","Ensure audit compliance for all settlement authority delegations"];
const METRICS = [{"label":"Authority Utilization","value":"84%","change":"+3%","trend":"up"},{"label":"Approval Cycle","value":"2.1 hrs","change":"-18%","trend":"up"},{"label":"Escalation Rate","value":"4.2%","change":"-1.1%","trend":"up"},{"label":"Audit Compliance","value":"100%","change":"0%","trend":"up"}];

export default function SettlementAuthorityManagerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '84', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"5 min ago","text":"Approved $450K commercial settlement within authority","icon":"Scale"},{"time":"11 min ago","text":"Escalated $1.2M D&O claim to VP Claims","icon":"ArrowUp"},{"time":"19 min ago","text":"Updated authority delegation matrix for Q4","icon":"FileText"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <Scale size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Settlement Authority Manager"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Settlement Authority Manager - Sub-agent supporting AI VP Claims. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>A2A Endpoints</Text>{['/consult/settlement-authority-manager','/settlement-authority-manager/execute','/settlement-authority-manager/analyze'].map((endpoint,i)=>(<View key={i} style={styles.endpointRow}><Zap size={14} color="#8B5CF6" /><Text style={[styles.endpointText, { color: theme.colors.textSecondary }]}>{endpoint}</Text></View>))}</View>
      ...
      <AgentFeatures agentId="settlement-authority-manager" agentName="AI Settlement Authority Manager" />
