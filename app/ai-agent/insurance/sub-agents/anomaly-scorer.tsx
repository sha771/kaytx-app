...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Gauge, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Anomaly Scoring","Statistical Analysis","Outlier Detection","ML-Based Scoring","Threshold Calibration","Real-Time Scoring","Severity Classification","Score Explainability"];
const RESPONSIBILITIES = ["Score claim anomalies using statistical and ML models","Detect outliers and measure deviations from baselines","Calibrate scoring thresholds to optimize detection rates","Provide real-time scoring at FNOL and throughout claim lifecycle","Classify anomaly severity for investigation prioritization","Generate explainable scores for regulatory and audit requirements"];
const METRICS = [{"label":"Claims Scored","value":"4,567","change":"+234","trend":"up"},{"label":"High Anomaly","value":"3.2%","change":"-0.5%","trend":"up"},{"label":"FPR","value":"8%","change":"-3%","trend":"up"},{"label":"AUC","value":"0.94","change":"+0.02","trend":"up"}];

export default function AnomalyScorerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '4,567', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"2 min ago","text":"Scored 28 new claims at FNOL with anomaly detection","icon":"Activity"},{"time":"9 min ago","text":"Recalibrated scoring thresholds for Q4","icon":"Gauge"},{"time":"16 min ago","text":"Generated explainability report for SIU referral","icon":"FileText"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <Activity size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Anomaly Scorer"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI Fraud Detection Agent</Text>
