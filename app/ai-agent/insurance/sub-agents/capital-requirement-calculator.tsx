...
import {
  Activity, ArrowRight, BarChart3, Briefcase, Calculator, ChevronRight, CircleCheckBig, Clock, FileText, Landmark, PieChart, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Capital Calculation","Solvency Assessment","RBC Computation","Economic Capital","Regulatory Capital","Capital Allocation","Target Capital","Capital Optimization"];
const RESPONSIBILITIES = ["Calculate capital requirements under regulatory and economic frameworks","Assess solvency positions and capital adequacy ratios","Compute Risk-Based Capital (RBC) charges by risk category","Determine economic capital needs using internal risk models","Allocate capital across business units and risk categories","Optimize capital structure and identify surplus positions"];
const METRICS = [{"label":"Capital Calculated","value":"$842M","change":"+$24M","trend":"up"},{"label":"Solvency Ratio","value":"142%","change":"+4%","trend":"up"},{"label":"RBC Ratio","value":"342%","change":"+18%","trend":"up"},{"label":"Economic Capital","value":"$612M","change":"+$18M","trend":"up"}];

export default function CapitalRequirementCalculatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '842M', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"3 min ago","text":"Calculated Q3 capital requirements under Solvency II","icon":"Landmark"},{"time":"10 min ago","text":"Allocated capital across 6 business units","icon":"PieChart"},{"time":"18 min ago","text":"Identified $42M capital optimization opportunity","icon":"TrendingUp"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF572220' }]}>
          <Landmark size={56} color="#FF5722" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Capital Requirement Calculator"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI Risk Modeler</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Capital Requirement Calculator - Sub-agent supporting AI Risk Modeler. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>A2A Endpoints</Text>{['/consult/capital-requirement-calculator','/capital-requirement-calculator/execute','/capital-requirement-calculator/analyze'].map((endpoint,i)=>(<View key={i} style={styles.endpointRow}><Zap size={14} color="#8B5CF6" /><Text style={[styles.endpointText, { color: theme.colors.textSecondary }]}>{endpoint}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/insurance/risk-modeler')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><Shield size={24} color="#004D40" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>{"AI Risk Modeler"}</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent · Insurance Dept</Text></View><ChevronRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="capital-requirement-calculator" agentName="AI Capital Requirement Calculator" />
