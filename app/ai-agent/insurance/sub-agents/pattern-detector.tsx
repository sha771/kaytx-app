...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, Search, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Pattern Detection","Network Analysis","Behavioral Baselines","Claim Clustering","Provider Analysis","Geographic Patterns","Temporal Analysis","Link Analysis"];
const RESPONSIBILITIES = ["Detect fraudulent patterns across claims and policy data","Analyze networks of claimants, providers, and attorneys","Establish behavioral baselines and detect deviations","Cluster suspicious claims for coordinated fraud investigation","Analyze provider billing patterns for medical fraud indicators","Identify geographic and temporal fraud patterns"];
const METRICS = [{"label":"Patterns Detected","value":"127","change":"+14","trend":"up"},{"label":"Networks Identified","value":"8","change":"+2","trend":"up"},{"label":"Detection Rate","value":"94.2%","change":"+1.4%","trend":"up"},{"label":"False Positive","value":"6%","change":"-2%","trend":"up"}];
...
export default function PatternDetectorPage() {
  ...
  const activities = [{"time":"3 min ago","text":"Detected suspicious provider billing network in FL","icon":"Search"},{"time":"10 min ago","text":"Clustered 14 related claims for SIU investigation","icon":"Users"},{"time":"18 min ago","text":"Updated behavioral baselines for auto claims","icon":"BarChart3"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <Search size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Pattern Detector"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Pattern Detector - Sub-agent supporting AI Fraud Detection Agent. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="pattern-detector" agentName="AI Pattern Detector" />
