...
import {
  Activity, ArrowRight, BarChart3, Briefcase, Calculator, ChevronRight, CircleCheckBig, Clock, DollarSign, FileText, RotateCcw, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Recovery Tracking","Cash Collection","Account Reconciliation","Outstanding Monitoring","Collection Forecasting","Dispute Resolution","Commutation Analysis","Run-Off Management"];
const RESPONSIBILITIES = ["Track reinsurance recoveries from notification to cash collection","Monitor outstanding recoveries and escalate overdue collections","Reconcile reinsurance accounts with counterparty statements","Forecast recovery cash flows for financial planning","Resolve recovery disputes with reinsurers and brokers","Analyze commutation opportunities and manage run-off portfolios"];
const METRICS = [{"label":"Recoveries Tracked","value":"$142M","change":"+$8M","trend":"up"},{"label":"Collection Rate","value":"97.4%","change":"+1.2%","trend":"up"},{"label":"Avg Collection Time","value":"42 days","change":"-8","trend":"up"},{"label":"Disputes Open","value":"3","change":"-2","trend":"up"}];

export default function RecoveriesTrackerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '142M', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"4 min ago","text":"Tracked $4.2M in catastrophe recoveries from Q3 events","icon":"RotateCcw"},{"time":"12 min ago","text":"Collected $1.8M from proportional treaty settlement","icon":"DollarSign"},{"time":"20 min ago","text":"Reconciled account with Swiss Re for Q3 bordereaux","icon":"Calculator"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <RotateCcw size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Recoveries Tracker"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI Reinsurance Specialist</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Recoveries Tracker - Sub-agent supporting AI Reinsurance Specialist. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
