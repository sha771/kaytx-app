...
import {
  Activity, AlertTriangle, ArrowRight, BarChart3, Briefcase, Calendar, CalendarClock, ChevronRight, CircleCheckBig, Clock, FileText, Mail, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Renewal Tracking","Expiration Monitoring","Auto-Renewal Processing","Renewal Pricing","Retention Analysis","Notice Generation","Lapse Prevention","Renewal Forecasting"];
const RESPONSIBILITIES = ["Track policy renewal dates and monitor expiration schedules","Process auto-renewals according to policy terms and conditions","Coordinate renewal pricing with underwriting and actuarial teams","Analyze retention rates and identify lapse risk policies","Generate renewal notices and communications to policyholders","Forecast renewal volumes and revenue projections"];
const METRICS = [{"label":"Renewals Tracked","value":"18,421","change":"+847","trend":"up"},{"label":"Retention Rate","value":"91.4%","change":"+1.2%","trend":"up"},{"label":"Auto-Renewals","value":"72%","change":"+4%","trend":"up"},{"label":"Lapse Prevention","value":"94%","change":"+2%","trend":"up"}];

export default function RenewalTrackerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '18,421', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"6 min ago","text":"Processed 84 auto-renewals for Q4 expiration batch","icon":"CalendarClock"},{"time":"14 min ago","text":"Flagged 23 policies at high lapse risk","icon":"AlertTriangle"},{"time":"22 min ago","text":"Generated renewal notices for 156 policyholders","icon":"Mail"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <CalendarClock size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Renewal Tracker"}</Text>
      ...
      <AgentFeatures agentId="renewal-tracker" agentName="AI Renewal Tracker" />
