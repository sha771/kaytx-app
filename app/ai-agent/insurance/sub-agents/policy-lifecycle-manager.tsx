...
import {
  Activity, ArrowRight, BarChart3, Briefcase, ChevronRight, CircleCheckBig, Clock, FileText, RefreshCw, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Policy Lifecycle Management","Issuance Tracking","Amendment Processing","Cancellation Management","Reinstatement Processing","Status Monitoring","Compliance Verification","Automated Workflows"];
const RESPONSIBILITIES = ["Manage complete policy lifecycle from issuance to expiration","Track policy amendments, endorsements, and modifications","Process policy cancellations and reinstatements according to rules","Monitor policy status and trigger automated lifecycle events","Verify compliance requirements at each lifecycle stage","Design and operate automated policy lifecycle workflows"];
const METRICS = [{"label":"Policies Managed","value":"24,847","change":"+1,247","trend":"up"},{"label":"Lifecycle Events","value":"8,421","change":"+342","trend":"up"},{"label":"Automation Rate","value":"82%","change":"+6%","trend":"up"},{"label":"Compliance Rate","value":"99.7%","change":"+0.2%","trend":"up"}];

export default function PolicyLifecycleManagerPage() {
  ...
  const activities = [{"time":"4 min ago","text":"Processed 47 policy amendments automatically","icon":"RefreshCw"},{"time":"11 min ago","text":"Managed 12 policy cancellations per compliance rules","icon":"FileText"},{"time":"19 min ago","text":"Reinstated 3 policies after payment verification","icon":"CircleCheckBig"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E64A1920' }]}>
          <RefreshCw size={56} color="#E64A19" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Policy Lifecycle Manager"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI Policy Manager</Text>
