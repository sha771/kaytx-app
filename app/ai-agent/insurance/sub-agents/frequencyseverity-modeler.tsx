...
import {
  Activity, ArrowRight, BarChart3, Briefcase, Calculator, ChevronRight, CircleCheckBig, Clock, FileText, Shield, Target, TrendingUp, Users, Zap
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const CAPABILITIES = ["Frequency Modeling","Severity Modeling","Distribution Fitting","GLM Analysis","Trend Projection","Credibility Blending","Simulation Modeling","Predictive Analytics"];
const RESPONSIBILITIES = ["Model claim frequency and severity distributions by line of business","Fit statistical distributions to loss data for pricing and reserving","Perform GLM analysis for rate indication and segmentation","Project frequency and severity trends for future periods","Blend credibility between company and industry experience","Develop simulation models for capital and reinsurance decisions"];
const METRICS = [{"label":"Models Updated","value":"18","change":"+2","trend":"up"},{"label":"Fit Quality","value":"0.97","change":"+0.02","trend":"up"},{"label":"Projection Accuracy","value":"94.2%","change":"+1.4%","trend":"up"},{"label":"Simulations Run","value":"1.2M","change":"+200K","trend":"up"}];

export default function FrequencyseverityModelerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '18', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"6 min ago","text":"Updated frequency model for auto bodily injury","icon":"BarChart3"},{"time":"14 min ago","text":"Fit new severity distribution for property line","icon":"Calculator"},{"time":"22 min ago","text":"Ran 500K simulation iterations for capital model","icon":"Zap"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <BarChart3 size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Frequency/Severity Modeler"}</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent · AI Actuary Analyst</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Frequency/Severity Modeler - Sub-agent supporting AI Actuary Analyst. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
