...
const CAPABILITIES = ["Loss Development Tracking","Chain Ladder Analysis","Development Factor Calculation","IBNR Estimation","Triangle Management","Aging Analysis","Trend Identification","Reserve Indication"];
const RESPONSIBILITIES = ["Track loss development patterns across accident years","Perform chain ladder analyses and calculate development factors","Estimate incurred but not reported (IBNR) reserves","Manage loss development triangles and data quality","Analyze aging patterns and identify development trends","Generate reserve indications based on development patterns"];
const METRICS = [{"label":"Triangles Managed","value":"24","change":"+2","trend":"up"},{"label":"IBNR Estimate","value":"$42M","change":"+$2.1M","trend":"up"},{"label":"Development Accuracy","value":"96.8%","change":"+1.2%","trend":"up"},{"label":"Data Quality","value":"99.4%","change":"+0.2%","trend":"up"}];

export default function LossDevelopmentTrackerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [
    { label: 'Tasks', value: '24', icon: CircleCheckBig, color: '#34C759' },
  ...
  const activities = [{"time":"4 min ago","text":"Updated loss development triangles for Q3","icon":"TrendingUp"},{"time":"12 min ago","text":"Calculated new development factors for workers comp","icon":"Calculator"},{"time":"20 min ago","text":"Generated IBNR estimate for commercial lines","icon":"BarChart3"}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#FF704320' }]}>
          <TrendingUp size={56} color="#FF7043" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{"AI Loss Development Tracker"}</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Loss Development Tracker - Sub-agent supporting AI Actuary Analyst. Part of the Kaytx AI Workforce Insurance hierarchy providing specialized enterprise capabilities for risk management, underwriting, claims, and actuarial operations.</Text></View>
      ...
      <AgentFeatures agentId="loss-development-tracker" agentName="AI Loss Development Tracker" />
