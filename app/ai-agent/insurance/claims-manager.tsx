...
import { FolderOpen, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'4,332',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.7%',icon:Target,color:'#004D40'}];
  const capabilities = ['Claims Operations','Team Management','Quality Assurance','Vendor Management','Reserving','Reporting'];
  const responsibilities = ['Claims operations management & oversight','Claims team leadership & development','Quality assurance & audit programs','Vendor & vendor network management','Claims reserving & financial oversight','Management reporting & analytics'];
  const activities = [{time:'3 min ago',text:'Managed team processing 200 claims',icon: CircleCheckBig},{time:'6 min ago',text:'Reduced average claim cycle by 8 days',icon:Clock},{time:'9 min ago',text:'Published monthly claims report',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}><FolderOpen size={48} color="#004D40" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Claims Manager</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Claims Management</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Claims Manager AI coordinates team operations, streamlines workflows, and ensures quality delivery within the Insurance department.</Text></View>
      ...
      <AgentFeatures agentId="claims-manager" agentName="Claims Manager" />
    </ScrollView>
  );
}
