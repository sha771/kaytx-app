...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3,469',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.0s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.3%',icon:Target,color:'#004D40'}];
  const capabilities = ['Claims Investigation','Damage Assessment','Settlement','Policy Interpretation','Fraud Detection','Documentation'];
  const responsibilities = ['Claims investigation & evaluation','Damage assessment & estimation','Settlement negotiation & processing','Policy interpretation & coverage analysis','Fraud detection & referral','Claims documentation & reporting'];
  const activities = [{time:'3 min ago',text:'Processed 25 claims today',icon: CircleCheckBig},{time:'6 min ago',text:'Investigated 3 suspicious claims',icon:Clock},{time:'9 min ago',text:'Settled total loss vehicle claim',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Claims Adjuster</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Claims Processing</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Claims Adjuster AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Insurance department.</Text></View>
      ...
      <AgentFeatures agentId="claims-adjuster" agentName="Claims Adjuster" />
    </ScrollView>
  );
}
