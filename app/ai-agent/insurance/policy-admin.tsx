...
import {
  FileText, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap,
  Users, MessageSquare, ChartBarBig, TrendingUp, AlertTriangle,
  Shield, ChevronRight, Stamp, ClipboardCheck, CheckCircle
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const SUB_AGENTS = [
  { id: 'policy-issuer', name: 'AI Policy Issuer', icon: Stamp, desc: 'Issues new policies with automated documentation, rating, and binding' },
  { id: 'document-generator', name: 'AI Document Generator', icon: FileText, desc: 'Generates policy documents, certificates, and coverage summaries' },
  { id: 'compliance-checker', name: 'AI Compliance Checker', icon: CheckCircle, desc: 'Validates policy compliance against regulatory requirements and forms' },
];

const QUICK_ACTIONS = [
  { label: 'Issue Policy', icon: Stamp },
...
const METRICS = [
  { label: 'Policies Issued', value: '2,460', change: '+7.5%', trend: 'up' },
  { label: 'Processing Time', value: '0.5s', change: '-30%', trend: 'up' },
  { label: 'Compliance Rate', value: '100.0%', change: '+0%', trend: 'up' },
  { label: 'Endorsements', value: '85/day', change: '+12%', trend: 'up' },
];
...
export default function PolicyAdminPage() {
  ...
  const stats = [
    { label: 'Tasks', value: '2,460', icon: CircleCheckBig, color: '#34C759' },
  ...
  const capabilities = [
    'Policy Issuance', 'Endorsement Processing', 'Renewal Processing',
    'Data Management', 'Compliance Validation', 'Customer Service',
    'Document Generation', 'Certificate Issuance', 'Regulatory Checking',
    'Policy Binding', 'Form Management', 'Audit Trail'
  ];

  const responsibilities = [
    'Policy issuance, documentation, and automated binding across all lines',
    'Endorsement processing, mid-term adjustments, and coverage modifications',
    'Policy renewal processing, retention optimization, and automated workflows',
    'Policy data management, accuracy assurance, and data governance',
    'Regulatory compliance for policy documents and form filings',
    'Policy-related customer service and inquiry resolution',
    'Document generation for policies, certificates, and coverage summaries',
    'Certificate of insurance issuance and verification',
    'Regulatory compliance checking against state and federal requirements',
    'Audit trail maintenance and policy version control'
  ];

  const activities = [
    { time: '3 min ago', text: 'Issued 120 new policies across auto and property lines', icon: Stamp },
    { time: '6 min ago', text: 'Processed 85 endorsements for mid-term coverage changes', icon: FileText },
    { time: '9 min ago', text: 'Managed 200 renewal cycles with 94% retention rate', icon: Clock },
    { time: '14 min ago', text: 'Generated 50 certificates of insurance for commercial clients', icon: FileText },
    { time: '28 min ago', text: 'Validated compliance for 15 policy form updates across 8 states', icon: CheckCircle },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}>
          <FileText size={48} color="#004D40" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Policy Administrator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Policy Operations & Documentation</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#004D4022' }]}><Star size={12} color="#004D40" /><Text style={[styles.badgeText, { color: '#004D40' }]}>Admin</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Policy Administrator manages policy operations, documentation, and compliance across the Insurance division. This agent handles policy issuance, endorsement processing, renewal management, and regulatory compliance. It orchestrates sub-agents for policy issuance, document generation, and compliance checking.</Text></View>
      ...
      <AgentFeatures agentId="policy-admin" agentName="AI Policy Administrator" />
