...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'1,509',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.8%',icon:Target,color:'#004D40'}];
  const capabilities = ['Fraud Detection','Pattern Analysis','SIU Support','Data Mining','Investigation','Recovery'];
  const responsibilities = ['Fraud detection & alert management','Pattern analysis & anomaly detection','Special investigations unit support','Data mining for fraud indicators','Claim investigation & evidence gathering','Fraud recovery & prevention programs'];
  const activities = [{time:'3 min ago',text:'Identified $450K in suspicious claims',icon: CircleCheckBig},{time:'6 min ago',text:'Analyzed fraud pattern trends',icon:Clock},{time:'9 min ago',text:'Referred 5 cases to SIU',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}><Search size={48} color="#004D40" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Fraud Detection Specialist</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Special Investigations</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Fraud Detection Specialist AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Insurance department.</Text></View>
      ...
      <AgentFeatures agentId="fraud-detector" agentName="Fraud Detection Specialist" />
    </ScrollView>
  );
}
