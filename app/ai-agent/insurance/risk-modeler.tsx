...
import {
  ChartBarBig, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap,
  Users, MessageSquare, TrendingUp, AlertTriangle,
  Shield, ChevronRight, GitBranch, Network, Landmark
} from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const SUB_AGENTS = [
  { id: 'scenario-builder', name: 'AI Scenario Builder', icon: GitBranch, desc: 'Constructs stress test scenarios and what-if analysis for risk assessment' },
  { id: 'correlation-analyst', name: 'AI Correlation Analyst', icon: Network, desc: 'Analyzes risk factor correlations and dependency structures across portfolios' },
  { id: 'capital-requirement-calculator', name: 'AI Capital Requirement Calculator', icon: Landmark, desc: 'Calculates regulatory and economic capital requirements using risk models' },
];

const QUICK_ACTIONS = [
  { label: 'Model Library', icon: ChartBarBig },
...
const METRICS = [
  { label: 'Models Calibrated', value: '5,183', change: '+11%', trend: 'up' },
  { label: 'Backtest Pass', value: '98.5%', change: '+0.8%', trend: 'up' },
  { label: 'Scenarios Run', value: '847', change: '+32%', trend: 'up' },
  { label: 'Capital Coverage', value: '1.42x', change: '+0.05', trend: 'up' },
];

export default function RiskModelerPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks', value: '5,183', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '1.3s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.5%', icon: Target, color: '#004D40' }
  ];

  const capabilities = [
    'Risk Modeling', 'Statistical Analysis', 'Stochastic Models',
    'Model Validation', 'Data Engineering', 'Reporting',
    'Scenario Construction', 'Correlation Analysis', 'Capital Calculation',
    'Stress Testing', 'Monte Carlo Simulation', 'Model Governance'
  ];

  const responsibilities = [
    'Risk model development, calibration, and production deployment',
    'Statistical analysis, hypothesis testing, and distribution fitting',
    'Stochastic process modeling and Monte Carlo simulation',
    'Model validation, backtesting, and performance monitoring',
    'Data engineering for risk model inputs and feature engineering',
    'Risk model reporting, documentation, and model governance',
    'Scenario construction for stress testing and what-if analysis',
    'Correlation analysis and dependency structure modeling across risk factors',
    'Capital requirement calculation for regulatory and economic capital',
    'Model governance, version control, and audit trail maintenance'
  ];

  const activities = [
    { time: '3 min ago', text: 'Calibrated severity & frequency models for property line', icon: ChartBarBig },
    { time: '6 min ago', text: 'Validated 5 production models with 98.5% backtest pass rate', icon: CircleCheckBig },
    { time: '9 min ago', text: 'Published model performance report for Q4 review', icon: Activity },
    { time: '14 min ago', text: 'Ran 50 stress scenarios for Solvency II compliance', icon: GitBranch },
    { time: '28 min ago', text: 'Calculated capital requirements: $89M across all lines', icon: Landmark },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}>
          <ChartBarBig size={48} color="#004D40" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Risk Modeler</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Risk Modeling & Quantitative Analysis</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Risk Modeler leads quantitative risk modeling, statistical analysis, and stochastic simulation across the Insurance division. This agent develops and validates risk models, runs stress scenarios, and calculates capital requirements. It orchestrates sub-agents for scenario building, correlation analysis, and capital requirement calculation.</Text></View>
      ...
      <AgentFeatures agentId="risk-modeler" agentName="AI Risk Modeler" />
