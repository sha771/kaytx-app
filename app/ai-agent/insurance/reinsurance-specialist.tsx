...
import { Layers, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'3,672',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.4s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.2%',icon:Target,color:'#004D40'}];
  const capabilities = ['Treaty Management','Facultative Placement','Reinsurance Accounting','Claim Recovery','Broker Relations','Modeling'];
  const responsibilities = ['Treaty reinsurance management & administration','Facultative reinsurance placement','Reinsurance accounting & reporting','Claim recovery from reinsurers','Broker relationship management','Reinsurance program modeling & optimization'];
  const activities = [{time:'3 min ago',text:'Placed $50M facultative cover',icon: CircleCheckBig},{time:'6 min ago',text:'Recovered $2.3M in reinsurance claims',icon:Clock},{time:'9 min ago',text:'Renewed treaty reinsurance program',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#004D4020' }]}><Layers size={48} color="#004D40" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Reinsurance Specialist</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Reinsurance</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Reinsurance Specialist AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Insurance department.</Text></View>
      ...
      <AgentFeatures agentId="reinsurance-specialist" agentName="Reinsurance Specialist" />
    </ScrollView>
  );
}
