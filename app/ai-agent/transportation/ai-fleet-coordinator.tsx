...
import {
  Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Truck, Zap, UserCheck, MapPin
...
const METRICS = [
  { label: 'Dispatch Rate', value: '97.2%', change: '+2.4%', trend: 'up' },
  { label: 'Deadhead Rate', value: '8.4%', change: '-3.1%', trend: 'up' },
  { label: 'HOS Compliance', value: '99.8%', change: '+0.2%', trend: 'up' },
  { label: 'Active Vehicles', value: '1,421', change: '+87', trend: 'up' },
];

export default function aiFleetCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks', value: '2,800', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.7%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.2s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.1%', icon: Target, color: '#26A69A' }
  ];

  const capabilities = [
    'Fleet Coordination', 'Dispatch Optimization', 'Driver Assignment', 'Vehicle Tracking', 'Load Matching', 'GPS Monitoring', 'Geofencing', 'Driver Management', 'Hours of Service', 'ELD Compliance', 'Vehicle Utilization', 'Deadhead Reduction', 'Fleet Communication', 'Break Management', 'Fuel Card Management', 'Yard Coordination'
  ];

  const responsibilities = [
    'Fleet dispatch optimization and load-to-vehicle matching',
    'Driver-to-vehicle and driver-to-route assignment coordination',
    'Real-time vehicle tracking and location monitoring',
    'Hours of service compliance and ELD management',
    'Deadhead reduction and vehicle utilization improvement',
    'Fleet communication and driver coordination',
    'Geofencing and arrival/departure tracking',
    'Fuel card management and expense monitoring',
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Fleet Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Fleet Dispatch & Vehicle Coordination</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Fleet Coordinator manages fleet dispatch & vehicle coordination across the transportation and logistics network. This agent ensures optimal performance through its specialized sub-agents: Dispatch Optimizer, Driver Assignment Agent, Vehicle Tracker.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary, marginBottom: 12 }]}>
          Direct reports executing specialized functions under AI Fleet Coordinator direction.
        </Text>
      ...
      <AgentFeatures agentId="ai-fleet-coordinator" agentName="AI Fleet Coordinator" />
