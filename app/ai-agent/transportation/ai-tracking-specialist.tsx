...
import {
  Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, MapPin, Eye
...
const METRICS = [
  { label: 'Tracking Accuracy', value: '99.6%', change: '+0.3%', trend: 'up' },
  { label: 'ETA Accuracy', value: '96.4%', change: '+2.8%', trend: 'up' },
  { label: 'Exception Response', value: '1.2 min', change: '-35%', trend: 'up' },
  { label: 'Active Shipments', value: '14,821', change: '+1,247', trend: 'up' },
];

export default function aiTrackingSpecialistPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks', value: '2,197', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.1%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.7s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.3%', icon: Target, color: '#26A69A' }
  ];

  const capabilities = [
    'Shipment Tracking', 'ETA Prediction', 'Exception Alerting', 'Real-time Monitoring', 'GPS Tracking', 'Geofencing', 'Status Updates', 'Delay Prediction', 'Milestone Tracking', 'Proof of Delivery', 'Customer Visibility', 'Carrier Integration', 'Multi-modal Tracking', 'Temperature Monitoring', 'Chain of Custody', 'Analytics & Reporting'
  ];

  const responsibilities = [
    'Real-time shipment monitoring and status tracking',
    'ETA prediction and delivery time estimation',
    'Exception detection and proactive alert management',
    'GPS tracking and geofencing management',
    'Milestone tracking and proof of delivery verification',
    'Customer visibility and status update management',
    'Carrier integration and multi-modal tracking',
    'Temperature monitoring for cold chain shipments',
    'Chain of custody and analytics reporting'
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Tracking Specialist</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Shipment Tracking & ETA Prediction</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Tracking Specialist manages shipment tracking & eta prediction across the transportation and logistics network. This agent ensures optimal performance through its specialized sub-agents: Shipment Monitor, ETA Predictor, Exception Alerter.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary, marginBottom: 12 }]}>
          Direct reports executing specialized functions under AI Tracking Specialist direction.
        </Text>
      ...
      <AgentFeatures agentId="ai-tracking-specialist" agentName="AI Tracking Specialist" />
