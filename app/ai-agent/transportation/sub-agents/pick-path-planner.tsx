...
export default function PickPathPlannerPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Warehouse Manager', icon: CircleCheckBig, color: '#007AFF' }
  ];

  const capabilities = [
    'Pick Path Planner', 'Automation', 'AI-Powered', 'Real-time', 'Analytics',
  ...
  const responsibilities = [
    'Execute Pick Path Planner operations with enterprise-grade precision',
    'Monitor and optimize performance metrics for continuous improvement',
    'Coordinate with parent agent AI Warehouse Manager on strategic initiatives',
  ...
  const activities = [
    { time: '2 min ago', text: 'Processed Pick Path Planner workflow batch', icon: Zap },
    { time: '15 min ago', text: 'Updated performance metrics dashboard', icon: ChartBarBig },
    { time: '30 min ago', text: 'Coordinated with AI Warehouse Manager on priority task', icon: Users },
    { time: '1 hr ago', text: 'Generated automated compliance report', icon: FileText },
    { time: '3 hr ago', text: 'Optimized Pick Path Planner parameters for 5% improvement', icon: TrendingUp },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Pick Path Planner</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Warehouse Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          AI Pick Path Planner is a specialized sub-agent supporting AI Warehouse Manager. It provides enterprise-grade Pick Path Planner capabilities with real-time monitoring, optimization, and analytics integration across the transportation and logistics network.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/transportation/warehouse-manager')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
          <ArrowRight size={24} color="#0EA5E9" />
          <View style={styles.parentInfo}>
            <Text style={[styles.parentName, { color: theme.colors.text }]}>AI Warehouse Manager</Text>
      ...
      <AgentFeatures agentId="pick-path-planner" agentName="AI Pick Path Planner" />
