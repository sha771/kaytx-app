...
import { Activity, Star, CircleCheckBig, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, ArrowDown } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function InventoryPutAwayAgentPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Warehouse Operator', icon: CircleCheckBig, color: '#007AFF' }
  ];

  const capabilities = [
    'Inventory Put-away Agent', 'Automation', 'AI-Powered', 'Real-time', 'Analytics',
  ...
  const responsibilities = [
    'Execute Inventory Put-away Agent operations with enterprise-grade precision',
    'Monitor and optimize performance metrics for continuous improvement',
    'Coordinate with parent agent AI Warehouse Operator on strategic initiatives',
  ...
  const activities = [
    { time: '2 min ago', text: 'Processed Inventory Put-away Agent workflow batch', icon: Zap },
    { time: '15 min ago', text: 'Updated performance metrics dashboard', icon: ChartBarBig },
    { time: '30 min ago', text: 'Coordinated with AI Warehouse Operator on priority task', icon: Users },
    { time: '1 hr ago', text: 'Generated automated compliance report', icon: FileText },
    { time: '3 hr ago', text: 'Optimized Inventory Put-away Agent parameters for 5% improvement', icon: TrendingUp },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0EA5E920' }]}>
          <ArrowDown size={56} color="#0EA5E9" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Inventory Put-away Agent</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Warehouse Operator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          AI Inventory Put-away Agent is a specialized sub-agent supporting AI Warehouse Operator. It provides enterprise-grade Inventory Put-away Agent capabilities with real-time monitoring, optimization, and analytics integration across the transportation and logistics network.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/transportation/ai-warehouse-operator')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
          <ArrowRight size={24} color="#0EA5E9" />
          <View style={styles.parentInfo}>
            <Text style={[styles.parentName, { color: theme.colors.text }]}>AI Warehouse Operator</Text>
      ...
      <AgentFeatures agentId="inventory-put-away-agent" agentName="AI Inventory Put-away Agent" />
