...
import {
  Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Package, ArrowDown, RotateCcw, Truck, CheckCircle
...
const METRICS = [
  { label: 'Put-away Speed', value: '2.1 min', change: '-18%', trend: 'up' },
  { label: 'Pick Rate', value: '142/hr', change: '+12%', trend: 'up' },
  { label: 'Pack Accuracy', value: '99.8%', change: '+0.1%', trend: 'up' },
  { label: 'Returns Processed', value: '847', change: '+94', trend: 'up' },
];

export default function aiWarehouseOperatorPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks', value: '3,400', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.0%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.9s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.2%', icon: Target, color: '#26A69A' }
  ];

  const capabilities = [
    'Warehouse Execution', 'Put-away Optimization', 'Pick & Pack', 'Return Processing', 'Material Handling', 'Conveyor Operations', 'RF Scanning', 'Batch Picking', 'Wave Planning', 'Cross-docking', 'Quality Inspection', 'Labeling', 'Shrink Wrap', 'Pallet Building', 'Staging Management', 'Dock Operations'
  ];

  const responsibilities = [
    'Intelligent put-away and slot assignment for inbound inventory',
    'Order picking and packing coordination and optimization',
    'Return processing and disposition management',
    'Material handling and conveyor operations management',
    'Wave planning and batch picking coordination',
    'Cross-docking and flow-through operations',
    'Quality inspection and labeling management',
    'Pallet building and staging management',
    'Dock operations and trailer coordination'
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Warehouse Operator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Warehouse Execution & Material Handling</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Warehouse Operator manages warehouse execution & material handling across the transportation and logistics network. This agent ensures optimal performance through its specialized sub-agents: Inventory Put-away Agent, Pick & Pack Coordinator, Return Processor.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary, marginBottom: 12 }]}>
          Direct reports executing specialized functions under AI Warehouse Operator direction.
        </Text>
      ...
      <AgentFeatures agentId="ai-warehouse-operator" agentName="AI Warehouse Operator" />
