...
import { Crown, ChevronLeft, Settings, Zap, Users, BarChart3, Target, Route, Brain, Workflow, Link, ChevronRight, Play, TrendingUp, Award, Sparkles, Clock, CheckCircle2, Activity, Database, MoreVertical, Search, Gauge, PieChart, Shield, Cpu, Lightbulb } from 'lucide-react-native';
import { aiChiefCustomerOfficer, getSubAgentsByParentId } from '@/constants/customerExperienceAgents';

export default function AIChiefCustomerOfficerPage() {
  const router = useRouter();
  const { theme } = useTheme();
  const agent = aiChiefCustomerOfficer;
  const subAgents = getSubAgentsByParentId(agent.id);
  ...
  const MetricCard = ({ title, value, trend, trendUp, icon: Icon, color }: any) => (
    <View style={[styles.metricCard, { backgroundColor: theme.colors.cardBackground }]}>
      <View style={styles.metricHeader}>
        ...
        {trend && (
          <View style={[styles.trendBadge, { backgroundColor: trendUp ? '#4CAF5020' : '#F4433620' }]}>
            <Text style={[styles.trendText, { color: trendUp ? '#4CAF50' : '#F44336' }]}>{trendUp ? '↑' : '↓'} {trend}</Text>
          </View>
        )}
      </View>
  ...
  const renderOverview = () => (
    <View style={styles.tabContent}>
      <View style={[styles.statusBanner, { backgroundColor: isActive ? '#4CAF5020' : '#F4433620' }]}>
        ...
        <Text style={[styles.versionText, { color: theme.colors.secondaryText }]}>v{agent.version} • Updated: {new Date(agent.updatedAt).toLocaleDateString()}</Text>
      </View>

      <View style={styles.metricsGrid}>
        <MetricCard title="Tasks Completed" value={agent.metrics.tasksCompleted.toLocaleString()} trend="12%" trendUp={true} icon={CheckCircle2} color="#4CAF50" />
        <MetricCard title="Avg Response" value={agent.metrics.avgResponseTime} trend="8%" trendUp={true} icon={Clock} color="#2196F3" />
        <MetricCard title="Success Rate" value={`${agent.metrics.successRate}%`} trend="5%" trendUp={true} icon={TrendingUp} color="#9C27B0" />
        <MetricCard title="CSAT Score" value={agent.metrics.customerSatisfaction.toString()} trend="0.3" trendUp={true} icon={Award} color="#FF9800" />
      </View>

      <View style={[styles.card, { backgroundColor: theme.colors.cardBackground }]}>
        <View style={styles.cardHeader}>
          <Brain size={24} color={agent.color} />
          <Text style={[styles.cardTitle, { color: theme.colors.text }]}>About This Agent</Text>
        </View>
        ...
        {agent.responsibilities.slice(0, 4).map((resp, idx) => (
          <View key={idx} style={styles.responsibilityItem}>
            ...
            <Text style={[styles.responsibilityText, { color: theme.colors.secondaryText }]}>{resp}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={[styles.card, { backgroundColor: theme.colors.cardBackground }]}>
        <View style={styles.cardHeader}>
          ...
          <Text style={[styles.cardTitle, { color: theme.colors.text }]}>Business Impact</Text>
        </View>
        <View style={styles.roiGrid}>
          <View style={styles.roiItem}>
            <Text style={[styles.roiValue, { color: '#4CAF50' }]}>{agent.metrics.costSavings}</Text>
            <Text style={[styles.roiLabel, { color: theme.colors.secondaryText }]}>Annual Savings</Text>
          </View>
          <View style={styles.roiDivider} />
          <View style={styles.roiItem}>
            <Text style={[styles.roiValue, { color: '#2196F3' }]}>{agent.metrics.roi}</Text>
      ...
      {agent.capabilities.map((cap) => (
        <View key={cap.id} style={[styles.capabilityCard, { backgroundColor: theme.colors.cardBackground }]}>
          <View style={styles.capabilityHeader}>
            <View style={[styles.capabilityIconContainer, { backgroundColor: agent.color + '20' }]}>
            ...
            <View style={styles.capabilityInfo}>
              ...
              <Text style={[styles.capabilityMeta, { color: theme.colors.secondaryText }]}>{cap.automationLevel.toUpperCase()} • {cap.estimatedTimeSaved} saved</Text>
            </View>
          </View>
        ...
        <View key={workflow.id} style={[styles.workflowCard, { backgroundColor: theme.colors.cardBackground }]}>
          <View style={styles.workflowHeader}>
            <View style={styles.workflowTitleSection}>
              <Workflow size={20} color={agent.color} />
              <Text style={[styles.workflowName, { color: theme.colors.text }]}>{workflow.name}</Text>
            </View>
            <Pressable style={[styles.executeButton, { backgroundColor: agent.color }]}>
          ...
          <View style={styles.workflowSection}>
            ...
            {workflow.steps.slice(0, 3).map((step, idx) => (
              <View key={idx} style={styles.workflowStep}>
                <View style={[styles.stepNumber, { backgroundColor: agent.color + '30' }]}>
                  <Text style={[styles.stepNumberText, { color: agent.color }]}>{idx + 1}</Text>
                </View>
    ...
    <View style={styles.tabContent}>
      <View style={styles.integrationsSummary}>
        <View style={[styles.summaryCard, { backgroundColor: '#4CAF5020' }]}>
          <Text style={[styles.summaryValue, { color: '#4CAF50' }]}>{agent.integrations.filter(i => i.status === 'active').length}</Text>
          <Text style={[styles.summaryLabel, { color: theme.colors.secondaryText }]}>Active</Text>
        </View>
        <View style={[styles.summaryCard, { backgroundColor: '#FF980020' }]}>
          <Text style={[styles.summaryValue, { color: '#FF9800' }]}>{agent.integrations.filter(i => i.status === 'pending').length}</Text>
          <Text style={[styles.summaryLabel, { color: theme.colors.secondaryText }]}>Pending</Text>
        </View>
        <View style={[styles.summaryCard, { backgroundColor: '#2196F320' }]}>
          <Text style={[styles.summaryValue, { color: '#2196F3' }]}>{agent.integrations.length}</Text>
          <Text style={[styles.summaryLabel, { color: theme.colors.secondaryText }]}>Total</Text>
        </View>
      </View>
        ...
        <View key={idx} style={[styles.integrationCard, { backgroundColor: theme.colors.cardBackground }]}>
          <View style={styles.integrationHeader}>
            <View style={styles.integrationMain}>
              <Database size={20} color={agent.color} />
              <View>
                <Text style={[styles.integrationName, { color: theme.colors.text }]}>{integration.system}</Text>
                <Text style={[styles.integrationType, { color: theme.colors.secondaryText }]}>{integration.type}</Text>
              </View>
            </View>
            <View style={[styles.statusBadge, { backgroundColor: integration.status === 'active' ? '#4CAF5020' : '#FF980020' }]}>
              <Text style={[styles.statusBadgeText, { color: integration.status === 'active' ? '#4CAF50' : '#FF9800' }]}>{integration.status}</Text>
            </View>
          </View>
        </View>
      ))}
  ...
  const renderSubAgents = () => (
    <View style={styles.tabContent}>
      <View style={[styles.subAgentsHeader, { backgroundColor: theme.colors.cardBackground }]}>
        <Text style={[styles.subAgentsTitle, { color: theme.colors.text }]}>Direct Reports ({subAgents.length})</Text>
        <Text style={[styles.subAgentsSubtitle, { color: theme.colors.secondaryText }]}>Sub-agents reporting to {agent.name}</Text>
      </View>
      {subAgents.map((subAgent) => (
        <Pressable key={subAgent.id} style={[styles.subAgentCard, { backgroundColor: theme.colors.cardBackground }]} onPress={() => router.push(`/ai-agent/customer-experience/${subAgent.id}`)}>
          <View style={styles.subAgentHeader}>
            <View style={[styles.subAgentIconContainer, { backgroundColor: subAgent.color + '20' }]}>
              <subAgent.icon size={24} color={subAgent.color} />
            </View>
            <View style={styles.subAgentInfo}>
              <Text style={[styles.subAgentName, { color: theme.colors.text }]}>{subAgent.name}</Text>
              <Text style={[styles.subAgentRole, { color: theme.colors.secondaryText }]}>{subAgent.role}</Text>
            </View>
            <View style={[styles.subAgentStatus, { backgroundColor: subAgent.status === 'active' ? '#4CAF5020' : '#F4433620' }]}>
              <View style={[styles.subAgentStatusDot, { backgroundColor: subAgent.status === 'active' ? '#4CAF50' : '#F44336' }]} />
              <Text style={[styles.subAgentStatusText, { color: subAgent.status === 'active' ? '#4CAF50' : '#F44336' }]}>{subAgent.status}</Text>
            </View>
          </View>
          <Text style={[styles.subAgentDescription, { color: theme.colors.secondaryText }]} numberOfLines={2}>{subAgent.description}</Text>
          <View style={styles.subAgentMetrics}>
            <View style={styles.subAgentMetric}>
              <Activity size={14} color={theme.colors.secondaryText} />
              <Text style={[styles.subAgentMetricText, { color: theme.colors.secondaryText }]}>{subAgent.metrics.successRate}% success</Text>
            </View>
            <View style={styles.subAgentMetric}>
              <Zap size={14} color={theme.colors.secondaryText} />
              <Text style={[styles.subAgentMetricText, { color: theme.colors.secondaryText }]}>{subAgent.capabilities.length} capabilities</Text>
            </View>
            <View style={styles.subAgentMetric}>
              <TrendingUp size={14} color={theme.colors.secondaryText} />
              <Text style={[styles.subAgentMetricText, { color: theme.colors.secondaryText }]}>{subAgent.metrics.roi} ROI</Text>
            </View>
          </View>
        </Pressable>
      ))}
    ...
    <View style={styles.tabContent}>
      <View style={styles.analyticsGrid}>
        <View style={[styles.analyticsCard, { backgroundColor: theme.colors.cardBackground }]}>
          ...
          <Text style={[styles.analyticsValue, { color: theme.colors.text }]}>{agent.metrics.successRate}%</Text>
          <Text style={[styles.analyticsLabel, { color: theme.colors.secondaryText }]}>Success Rate</Text>
        </View>
        <View style={[styles.analyticsCard, { backgroundColor: theme.colors.cardBackground }]}>
          <Gauge size={20} color="#2196F3" />
          <Text style={[styles.analyticsValue, { color: theme.colors.text }]}>{agent.slaTargets.availability}</Text>
          <Text style={[styles.analyticsLabel, { color: theme.colors.secondaryText }]}>Uptime</Text>
        </View>
        <View style={[styles.analyticsCard, { backgroundColor: theme.colors.cardBackground }]}>
          <Users size={20} color="#4CAF50" />
          <Text style={[styles.analyticsValue, { color: theme.colors.text }]}>{agent.metrics.tasksCompleted.toLocaleString()}</Text>
          <Text style={[styles.analyticsLabel, { color: theme.colors.secondaryText }]}>Tasks Done</Text>
        </View>
        <View style={[styles.analyticsCard, { backgroundColor: theme.colors.cardBackground }]}>
          <Activity size={20} color="#FF9800" />
          <Text style={[styles.analyticsValue, { color: theme.colors.text }]}>{agent.metrics.customerSatisfaction}/5</Text>
          <Text style={[styles.analyticsLabel, { color: theme.colors.secondaryText }]}>CSAT</Text>
        </View>
      </View>
        ...
        {agent.kpis.map((kpi, idx) => (
          <View key={idx} style={styles.kpiRow}>
            <View style={styles.kpiLeft}>
              <Target size={16} color={agent.color} />
              <Text style={[styles.kpiText, { color: theme.colors.text }]}>{kpi}</Text>
            </View>
            <View style={[styles.kpiBadge, { backgroundColor: agent.color + '20' }]}>
              <Text style={[styles.kpiBadgeText, { color: agent.color }]}>On Track</Text>
            </View>
          </View>
        ))}
      </View>
      ...
      <View style={[styles.settingsCard, { backgroundColor: theme.colors.cardBackground }]}>
        ...
        <View style={styles.settingRow}>
          <View style={styles.settingLeft}>
            <View style={[styles.powerIcon, { backgroundColor: isActive ? '#4CAF50' : '#F44336' }]} />
            <View>
              <Text style={[styles.settingName, { color: theme.colors.text }]}>Active Status</Text>
              <Text style={[styles.settingDescription, { color: theme.colors.secondaryText }]}>Enable or disable this agent</Text>
            </View>
          </View>
      ...
      <View style={[styles.settingsCard, { backgroundColor: theme.colors.cardBackground }]}>
        ...
        <View style={styles.scaleConfig}>
          <View style={styles.scaleItem}>
            <Text style={[styles.scaleLabel, { color: theme.colors.secondaryText }]}>Min Instances</Text>
            <Text style={[styles.scaleValue, { color: theme.colors.text }]}>{agent.autoScale.minInstances}</Text>
          </View>
          <View style={styles.scaleItem}>
            <Text style={[styles.scaleLabel, { color: theme.colors.secondaryText }]}>Max Instances</Text>
            <Text style={[styles.scaleValue, { color: theme.colors.text }]}>{agent.autoScale.maxInstances}</Text>
          </View>
        </View>
        <View style={styles.scaleTriggerBox}>
          <Text style={[styles.scaleLabel, { color: theme.colors.secondaryText }]}>Scale Trigger</Text>
          <Text style={[styles.scaleTriggerText, { color: theme.colors.text }]}>{agent.autoScale.scaleTrigger}</Text>
        </View>
      </View>
      <View style={[styles.settingsCard, { backgroundColor: theme.colors.cardBackground }]}>
      ...
      <View style={[styles.settingsCard, { backgroundColor: theme.colors.cardBackground }]}>
        ...
        {agent.tools.slice(0, 5).map((tool, idx) => (
          <View key={idx} style={styles.toolItem}>
            <Lightbulb size={16} color={agent.color} />
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.cardBackground }]}>
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerText}>
            ...
            <Text style={[styles.headerSubtitle, { color: theme.colors.secondaryText }]}>{agent.role} • {agent.department}</Text>
          </View>
        </View>
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Pressable key={tab.id} style={[styles.tabButton, isTabActive && { backgroundColor: agent.color + '20', borderColor: agent.color }]} onPress={() => setActiveTab(tab.id)}>
              <TabIcon size={16} color={isTabActive ? agent.color : theme.colors.secondaryText} />
              <Text style={[styles.tabText, { color: isTabActive ? agent.color : theme.colors.secondaryText }]}>{tab.label}</Text>
            </Pressable>
          );
        })}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  subAgentsHeader: { padding: 16, borderRadius: 12, marginBottom: 8 },
  subAgentsTitle: { fontSize: 16, fontWeight: '700', marginBottom: 4 },
  subAgentsSubtitle: { fontSize: 13 },
  subAgentCard: { padding: 16, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(0,0,0,0.05)' },
  subAgentHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  subAgentIconContainer: { width: 44, height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center', marginRight: 12 },
  subAgentInfo: { flex: 1 },
  subAgentName: { fontSize: 15, fontWeight: '600', marginBottom: 2 },
  subAgentRole: { fontSize: 12 },
  subAgentStatus: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, gap: 6 },
  subAgentStatusDot: { width: 6, height: 6, borderRadius: 3 },
  subAgentStatusText: { fontSize: 11, fontWeight: '600', textTransform: 'capitalize' },
  subAgentDescription: { fontSize: 13, lineHeight: 20, marginBottom: 10 },
  subAgentMetrics: { flexDirection: 'row', gap: 16 },
  subAgentMetric: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  subAgentMetricText: { fontSize: 12 },
