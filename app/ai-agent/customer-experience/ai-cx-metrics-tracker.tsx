...
import { BarChart3, ChevronLeft, Settings, Zap, Workflow, Link, Play, TrendingUp, Award, Clock, CheckCircle2, Activity, Database, MoreVertical, Gauge, PieChart, Shield, Lightbulb } from 'lucide-react-native';
import { aiCXMetricsTracker, getCustomerExperienceAgentById } from '@/constants/customerExperienceAgents';

export default function AICXMetricsTrackerPage() {
  const router = useRouter();
  const { theme } = useTheme();
  const agent = aiCXMetricsTracker;
  ...
  const MetricCard = ({ title, value, trend, trendUp, icon: Icon, color }: any) => (
    <View style={[styles.metricCard, { backgroundColor: theme.colors.cardBackground }]}>
      <View style={styles.metricHeader}>
        <View style={[styles.metricIconContainer, { backgroundColor: color + '20' }]}><Icon size={20} color={color} /></View>
  ...
  const renderOverview = () => (
    <View style={styles.tabContent}>
      ...
      <View style={styles.metricsGrid}>
        <MetricCard title="Dashboards" value="24" trend="20%" trendUp={true} icon={PieChart} color="#9C27B0" />
        <MetricCard title="Avg Response" value="1.5m" trend="15%" trendUp={true} icon={Clock} color="#2196F3" />
        <MetricCard title="Success Rate" value={`${agent.metrics.successRate}%`} trend="7%" trendUp={true} icon={TrendingUp} color="#4CAF50" />
      ...
      <View style={[styles.card, { backgroundColor: theme.colors.cardBackground }]}>
        <View style={styles.cardHeader}><BarChart3 size={24} color={agent.color} /><Text style={[styles.cardTitle, { color: theme.colors.text }]}>About This Agent</Text></View>
  ...
  const renderAnalytics = () => (
    <View style={styles.tabContent}>
      <View style={styles.analyticsGrid}>
        ...
        <View style={[styles.analyticsCard, { backgroundColor: theme.colors.cardBackground }]}><BarChart3 size={20} color="#4CAF50" /><Text style={[styles.analyticsValue, { color: theme.colors.text }]}>24</Text><Text style={[styles.analyticsLabel, { color: theme.colors.secondaryText }]}>Dashboards</Text></View>
      ...
      <View style={[styles.metricsListCard, { backgroundColor: theme.colors.cardBackground }]}>
        ...
        {agent.kpis.map((kpi, idx) => (
          <View key={idx} style={styles.kpiRow}>
            <View style={styles.kpiLeft}><BarChart3 size={16} color={agent.color} /><Text style={[styles.kpiText, { color: theme.colors.text }]}>{kpi}</Text></View>
