...
import { 
  Building2, Shield, AlertTriangle, Wrench, CheckCircle, Activity, TrendingUp,
  Clock, ArrowUpRight, Zap, Gauge, Radio, Network
} from 'lucide-react-native';
import { ENERGY_COLORS, INFRASTRUCTURE_DATA } from '@/constants/energyUtilities';
...
export default function Infrastructure() {
  const INFRASTRUCTURE_METRICS = [
    ...
    { label: 'Infrastructure Health', value: '95.2%', icon: Shield, color: ENERGY_COLORS.emeraldGreen, trend: '+2.4%', trendUp: true },
    { label: 'Critical Assets', value: '8,640', icon: AlertTriangle, color: ENERGY_COLORS.amber, trend: '-8.6%', trendUp: true },
    { label: 'Maintenance Completion', value: '94.2%', icon: Wrench, color: ENERGY_COLORS.neonCyan, trend: '+6.8%', trendUp: true },
    { label: 'Asset Health Score', value: '93.8%', icon: Gauge, color: ENERGY_COLORS.electricBlue, trend: '+4.2%', trendUp: true },
    { label: 'Predictive Alerts', value: '2,840', icon: Activity, color: ENERGY_COLORS.magenta, trend: '+34.6%', trendUp: true },
  ];

  const INFRASTRUCTURE_ALERTS = [
    ...
    { type: 'success', message: 'Infrastructure health score improved to 95.2% with new monitoring system', impact: 'Positive', time: '6h ago' },
    { type: 'trend', message: 'Asset failure rate decreased by 42% over past quarter', impact: 'Medium', time: '8h ago' },
  ];
  ...
  const renderInfrastructureAssets = () => (
    <View style={styles.section}>
      ...
      <View style={styles.assetsList}>
        {INFRASTRUCTURE_DATA.map((asset) => (
          <View key={asset.asset} style={[styles.assetCard, { backgroundColor: '#0A0F1A', borderLeftWidth: 3, borderLeftColor: asset.health === '99.8%' ? ENERGY_COLORS.emeraldGreen : asset.health === '96.8%' ? ENERGY_COLORS.neonCyan : ENERGY_COLORS.amber }]}>
            <View style={styles.assetHeader}>
              <Building2 size={20} color={asset.health === '99.8%' ? ENERGY_COLORS.emeraldGreen : asset.health === '96.8%' ? ENERGY_COLORS.neonCyan : ENERGY_COLORS.amber} />
            ...
            <View style={styles.assetMetrics}>
              <View style={styles.assetMetric}>
                ...
                <Text style={styles.assetMetricValue}>{asset.health}</Text>
              </View>
              <View style={styles.assetMetric}>
                <AlertTriangle size={12} color="#6B7280" />
                <Text style={styles.assetMetricLabel}>Critical</Text>
                <Text style={styles.assetMetricValue}>{asset.critical}</Text>
              </View>
              <View style={styles.assetMetric}>
                <Wrench size={12} color="#6B7280" />
                <Text style={styles.assetMetricLabel}>Maintenance</Text>
                <Text style={styles.assetMetricValue}>{asset.maintenance}</Text>
              </View>
            </View>
          </View>
        ))}
  ...
  const renderAlerts = () => (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>Infrastructure Alerts</Text>
      {INFRASTRUCTURE_ALERTS.map((alert, index) => (
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Network size={32} color={ENERGY_COLORS.purple} />
        <View>
          <Text style={styles.headerTitle}>Infrastructure Command Center</Text>
          <Text style={styles.headerSubtitle}>Substations, transmission lines, transformers, and distribution assets</Text>
        </View>
      </View>
