...
import { 
  BarChart3, DollarSign, TrendingUp, Activity, AlertTriangle, CheckCircle,
  Clock, ArrowUpRight, PieChart, Target, Shield, Percent
} from 'lucide-react-native';
import { ENERGY_COLORS, ENERGY_TRADING_DATA } from '@/constants/energyUtilities';
...
export default function EnergyTrading() {
  ...
  const renderMetrics = () => (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>Energy Trading Metrics</Text>
      <View style={styles.metricsGrid}>
        {TRADING_METRICS.map((metric, index) => (
  ...
  const renderTradingMarkets = () => (
    <View style={styles.section}>
      ...
      <View style={styles.marketsList}>
        {ENERGY_TRADING_DATA.map((market) => (
          <View key={market.market} style={[styles.marketCard, { backgroundColor: '#0A0F1A', borderLeftWidth: 3, borderLeftColor: market.volatility === '6.2%' ? ENERGY_COLORS.emeraldGreen : market.volatility === '8.4%' ? ENERGY_COLORS.neonCyan : ENERGY_COLORS.amber }]}>
            <View style={styles.marketHeader}>
              <BarChart3 size={20} color={market.volatility === '6.2%' ? ENERGY_COLORS.emeraldGreen : market.volatility === '8.4%' ? ENERGY_COLORS.neonCyan : ENERGY_COLORS.amber} />
            ...
            <View style={styles.marketMetrics}>
              ...
              <View style={styles.marketMetric}>
                ...
                <Text style={styles.marketMetricLabel}>Volatility</Text>
                <Text style={styles.marketMetricValue}>{market.volatility}</Text>
              </View>
            </View>
          </View>
        ))}
  ...
  const renderAlerts = () => (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>Energy Trading Alerts</Text>
      {TRADING_ALERTS.map((alert, index) => (
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <BarChart3 size={32} color={ENERGY_COLORS.amber} />
        <View>
          <Text style={styles.headerTitle}>Energy Trading Center</Text>
          <Text style={styles.headerSubtitle}>Day-ahead, real-time, futures, and capacity market trading</Text>
        </View>
      </View>
