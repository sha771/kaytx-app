...
export default function FinancialServicesScreen() {
  ...
  const industryMetrics = useMemo(() => [
    { label: 'Risk Audits/Day', value: statsData?.tasksToday ? `${(statsData.tasksToday / 12).toFixed(1)}K+` : '12K+', icon: Shield, color: '#007AFF' },
    { label: 'Avg Audit Time', value: '450ms', icon: Activity, color: '#34C759' },
    { label: 'Compliance Score', value: statsData?.avgHealthScore ? `${statsData.avgHealthScore}%` : '100%', icon: Shield, color: '#FF9500' },
    { label: 'Resource Efficiency', value: statsData?.avgSuccessRate ? `${(statsData.avgSuccessRate / 4).toFixed(1)}x` : '18x', icon: TrendingUp, color: '#5856D6' },
  ], [statsData]);

  const industryAgents = useMemo(() => aiEmployees.filter(emp =>
    emp.capabilities.some(cap =>
      cap.toLowerCase().includes('finance') ||
      cap.toLowerCase().includes('banking') ||
      cap.toLowerCase().includes('account') ||
      cap.toLowerCase().includes('payment') ||
      (emp as any).category === 'data-intelligence' ||
      emp.category === 'support' ||
      emp.id === 'ai-negotiator'
    )
  ), []);
  ...
  const categories = [
    ...
    { id: 'operations', label: 'Compliance', icon: Shield },
  ];
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <LinearGradient colors={['#5856D6', '#007AFF']} style={[styles.header, { paddingTop: 50 }]}>
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={styles.industryTitle}>Trust & Integrity</Text>
            <Text style={styles.industrySubtitle}>Secure AI Financial Operations</Text>
          </View>
        </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            ...
            <Text style={styles.statLabel}>Fin-Agents</Text>
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryScroll}>
            {categories.map((cat) => (
              ...
              >
                ...
                <Text style={[styles.categoryLabel, { color: selectedCategory === cat.id ? '#fff' : theme.colors.text }]}>{cat.label}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>

        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Enterprise Solutions</Text>
        <FlatList
          data={filteredAgents}
