...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'92',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'2.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.3%',icon:Target,color:'#6A1B9A'}];
  const capabilities = ['A/B Test Design','Experiment Management','Result Analysis'];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI A/B Test Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Designs, runs, and analyzes A/B tests for product optimization.</Text></View>
      ...
      <AgentFeatures agentId="ab-test-agent" agentName="AI A/B Test Agent" />
    </ScrollView>
  );
}
