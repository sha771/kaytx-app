...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'1783',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.4s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.7%',icon:Target,color:'#6A1B9A'}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#6A1B9A20' }]}><Settings size={48} color="#6A1B9A" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Product Operations</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Product Operations AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Product division.</Text></View>
      ...
      <AgentFeatures agentId="vp-product-operations" agentName="VP Product Operations" />
    </ScrollView>
  );
}
