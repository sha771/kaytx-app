...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3411',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.2%',icon:Target,color:'#4A148C'}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#4A148C20' }]}><Zap size={48} color="#4A148C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AOD Lead</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Executive</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#4A148C22' }]}><Star size={12} color="#4A148C" /><Text style={[styles.badgeText, { color: '#4A148C' }]}>Manager</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AOD Lead AI coordinates team operations, streamlines workflows, and ensures quality delivery within the Executive department.</Text></View>
      ...
      <AgentFeatures agentId="aod-lead" agentName="AOD Lead" />
    </ScrollView>
  );
}
