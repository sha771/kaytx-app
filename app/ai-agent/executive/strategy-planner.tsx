...
import { Map, Activity, Star, Users, CircleCheckBig, Clock, Target, Zap, ArrowRight, ChartBar, MessageSquare, Calendar, Shield, ChartBar } from 'lucide-react-native';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: theme.colors.primary + '15' }]}><Map size={48} color={theme.colors.primary} /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Strategy Planner</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Strategic Planning AI</Text>
      ...
      <View style={styles.statsContainer}>
        {[{label:'Plans',value:'2,341',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.97%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.5%',icon:Target,color:'#AF52DE'}].map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}
      </View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text>
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Strategy Planner develops long-term strategic plans, identifies opportunities, and aligns organizational goals. This agent leverages market intelligence and organizational data to craft actionable roadmaps.</Text>
      </View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Capabilities</Text>
        <View style={styles.tagsContainer}>
          {['Strategic Planning','Opportunity ID','Goal Alignment','Market Intelligence','Roadmap Design','Competitive Analysis'].map((cap,i)=>(<View key={i} style={[styles.tag, { backgroundColor: theme.colors.primary + '18' }]}><Text style={[styles.tagText, { color: theme.colors.primary }]}>{cap}</Text></View>))}
        </View>
      </View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Key Responsibilities</Text>
        {['Long-term Strategic Plan Development','Opportunity Identification & Assessment','Organizational Goal Alignment','Market Intelligence & Trend Analysis','Strategic Roadmap Design','Competitive Landscape Analysis','Resource Allocation Planning','Strategic Initiative Prioritization'].map((item,i)=>(<View key={i} style={styles.responsibilityRow}><ArrowRight size={14} color={theme.colors.primary} /><Text style={[styles.responsibilityText, { color: theme.colors.textSecondary }]}>{item}</Text></View>))}
      </View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Recent Activity</Text>
        {[{time:'2 min ago',text:'Completed 5-year strategic roadmap draft'},{time:'15 min ago',text:'Identified 3 emerging market opportunities'},{time:'40 min ago',text:'Updated competitive positioning analysis'},{time:'2 hours ago',text:'Published strategic initiative priority matrix'},{time:'4 hours ago',text:'Aligned Q4 goals across 8 departments'}].map((act,i)=>(<View key={i} style={styles.activityRow}><View style={[styles.activityIcon, { backgroundColor: theme.colors.primary + '15' }]}><Zap size={14} color={theme.colors.primary} /></View><View style={styles.activityContent}><Text style={[styles.activityText, { color: theme.colors.text }]}>{act.text}</Text><Text style={[styles.activityTime, { color: theme.colors.textSecondary }]}>{act.time}</Text></View></View>))}
      </View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
      ...
      <AgentFeatures agentId="strategy-planner" agentName="Strategy Planner" />
    </ScrollView>
  );
}
