...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3,700',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.8%',icon:Target,color:'#0277BD'}];
  const capabilities = ['Regulatory Compliance','Trade Surveillance','AML/KYC','Reporting','Policy Enforcement','Audit Support'];
  const responsibilities = ['Trading regulatory compliance monitoring','Trade surveillance & anomaly detection','AML/KYC verification processes','Regulatory reporting & documentation','Policy enforcement & violation tracking','Audit preparation & support'];
  const activities = [{time:'3 min ago',text:'Cleared 500 trade surveillance alerts',icon: CircleCheckBig},{time:'6 min ago',text:'Updated AML screening rules',icon:Clock},{time:'9 min ago',text:'Filed monthly regulatory report',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0277BD20' }]}><ShieldCheck size={48} color="#0277BD" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Trading Compliance</Text>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#0277BD22' }]}><Star size={12} color="#0277BD" /><Text style={[styles.badgeText, { color: '#0277BD' }]}>Officer</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Trading Compliance AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Trading & Investment department.</Text></View>
      ...
      <AgentFeatures agentId="compliance-trading" agentName="Trading Compliance" />
    </ScrollView>
  );
}
