...
import { Briefcase, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'3,857',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.0s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.6%',icon:Target,color:'#0277BD'}];
  const capabilities = ['Investment Strategy','Portfolio Oversight','Risk Management','Asset Allocation','Performance Analysis','Regulatory Compliance'];
  const responsibilities = ['Investment strategy development & oversight','Portfolio performance monitoring','Risk management framework enforcement','Strategic asset allocation decisions','Regulatory compliance for investments','Team leadership & talent management'];
  const activities = [{time:'3 min ago',text:'Approved Q3 investment strategy',icon: CircleCheckBig},{time:'6 min ago',text:'Reviewed portfolio risk exposure',icon:Clock},{time:'9 min ago',text:'Met with regulatory compliance team',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0277BD20' }]}><Briefcase size={48} color="#0277BD" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>CIO</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Investment Management</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#0277BD22' }]}><Star size={12} color="#0277BD" /><Text style={[styles.badgeText, { color: '#0277BD' }]}>C-Suite</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The CIO AI provides executive-level strategic oversight, drives organizational alignment, and ensures operational excellence across the Trading & Investment division.</Text></View>
      ...
      <AgentFeatures agentId="cio" agentName="CIO" />
    </ScrollView>
  );
}
