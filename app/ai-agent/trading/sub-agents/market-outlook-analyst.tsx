...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, BarChart3, Layers, Briefcase, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Forecast Acc","value":"94.2%","change":"+1.4%","trend":"up"},{"label":"Signal Lead","value":"3.2 days","change":"+0.4d","trend":"up"},{"label":"Regime Calls","value":"89%","change":"+3%","trend":"up"},{"label":"Surprise Index","value":"0.14","change":"+0.02","trend":"up"}];

export default function MarketOutlookAnalystPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Indicators',value:'847',icon:BarChart3,color:'#34C759'},{label:'Forecasts',value:'94.2%',icon:Target,color:'#007AFF'},{label:'Scenarios',value:'24',icon:Layers,color:'#FF9500'},{label:'Reports',value:'312',icon:FileText,color:'#AF52DE'}];
  const capabilities = ["Macro Forecasting","Economic Indicators","Yield Curve Analysis","Sentiment Analysis","Regime Detection","Scenario Modeling","Cross-Asset Signals","Geopolitical Assessment"];
  const responsibilities = ["Produce macro market outlook and forecasts","Track economic indicators and surprise indices","Analyze yield curve signals and recession risk","Monitor sentiment and positioning data","Detect market regime changes","Assess geopolitical impacts on markets"];
  const activities = [{time:'2 min ago',text:'Updated US GDP nowcast to 2.1%',icon:BarChart3},{time:'6 min ago',text:'Detected yield curve flattening signal',icon:TrendingUp},{time:'14 min ago',text:'Raised recession probability to 18%',icon:AlertTriangle},{time:'28 min ago',text:'Updated sentiment composite score',icon:Activity},{time:'1 hr ago',text:'Published weekly market outlook',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#0277BD20' }]}>
          <BarChart3 size={48} color="#0277BD" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Market Outlook Analyst</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Macro Forecasting & Economic Indicator Analysis</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Market Outlook Analyst operates as a specialized sub-agent under AI Chief Investment Officer, delivering enterprise-grade capabilities in macro forecasting & economic indicator analysis within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="market-outlook-analyst" agentName="AI Market Outlook Analyst"/>
