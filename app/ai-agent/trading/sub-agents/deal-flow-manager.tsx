...
const METRICS = [{"label":"Pipeline Size","value":"47","change":"+6","trend":"up"},{"label":"Conversion","value":"32%","change":"+4%","trend":"up"},{"label":"Avg Score","value":"8.4","change":"+0.3","trend":"up"},{"label":"Source Mix","value":"Diversified","change":"Stable","trend":"up"}];

export default function DealFlowManagerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Pipeline',value:'47',icon:TrendingUp,color:'#34C759'},{label:'Active',value:'18',icon:Activity,color:'#007AFF'},{label:'Conversion',value:'32%',icon:Target,color:'#FF9500'},{label:'Avg Score',value:'8.4',icon:Star,color:'#AF52DE'}];
  const capabilities = ["Pipeline Management","Deal Scoring","Prioritization Engine","Source Tracking","CRM Integration","Funnel Analytics","Conversion Metrics","Relationship Mapping"];
  const responsibilities = ["Manage the investment deal pipeline end-to-end","Score and rank deals using multi-factor evaluation","Prioritize deals based on strategic fit and return potential","Track deal sources and relationship mapping","Analyze pipeline funnel and conversion metrics","Produce pipeline status reports for leadership"];
  const activities = [{time:'2 min ago',text:'Scored 3 new deals with multi-factor model',icon:Target},{time:'6 min ago',text:'Updated pipeline conversion funnel',icon:Activity},{time:'14 min ago',text:'Flagged 2 deals for priority review',icon:AlertTriangle},{time:'28 min ago',text:'Mapped relationship for Series C lead',icon:Users},{time:'1 hr ago',text:'Published weekly pipeline report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Deal Flow Manager</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Pipeline Management & Deal Scoring</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Deal Flow Manager operates as a specialized sub-agent under AI VP Investments, delivering enterprise-grade capabilities in pipeline management & deal scoring within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="deal-flow-manager" agentName="AI Deal Flow Manager"/>
