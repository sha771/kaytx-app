...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Search, Briefcase, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Signal Sharpe","value":"1.92","change":"+0.14","trend":"up"},{"label":"Strategy Win","value":"68%","change":"+3%","trend":"up"},{"label":"Alpha Gen","value":"+2.8%","change":"+0.4%","trend":"up"},{"label":"Regime Det","value":"94%","change":"+2%","trend":"up"}];

export default function InvestmentStrategyAdvisorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Strategies',value:'24',icon:Target,color:'#34C759'},{label:'Alpha Signals',value:'847',icon:Zap,color:'#007AFF'},{label:'Accuracy',value:'96.4%',icon:CircleCheckBig,color:'#FF9500'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#AF52DE'}];
  const capabilities = ["Strategy Formulation","Alpha Research","Factor Analysis","Market Regime Detection","Asset Class Research","Tactical Overlay","Investment Thesis","Portfolio Construction Input"];
  const responsibilities = ["Develop and maintain investment strategy frameworks","Conduct alpha research and signal validation","Analyze factor exposures and regime shifts","Produce tactical overlay recommendations","Build investment thesis documentation","Support portfolio construction with strategy inputs"];
  const activities = [{time:'3 min ago',text:'Published Q3 tactical overlay update',icon:FileText},{time:'8 min ago',text:'Validated new alpha signal with 1.9 Sharpe',icon:Zap},{time:'15 min ago',text:'Detected regime shift in credit spreads',icon:Activity},{time:'30 min ago',text:'Updated factor exposure model',icon:Target},{time:'1 hr ago',text:'Completed asset class research note',icon:Search}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#0277BD20' }]}>
          <TrendingUp size={48} color="#0277BD" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Investment Strategy Advisor</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Strategy Framework & Alpha Signal Research</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Investment Strategy Advisor operates as a specialized sub-agent under AI Chief Investment Officer, delivering enterprise-grade capabilities in strategy framework & alpha signal research within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="investment-strategy-advisor" agentName="AI Investment Strategy Advisor"/>
