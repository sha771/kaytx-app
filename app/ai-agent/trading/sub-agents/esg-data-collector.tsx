...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Database, Layers, Leaf, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Data Quality","value":"98.4%","change":"+0.6%","trend":"up"},{"label":"Source Count","value":"42","change":"+3","trend":"up"},{"label":"Coverage","value":"92%","change":"+2%","trend":"up"},{"label":"Ingestion","value":"Real-time","change":"Stable","trend":"up"}];

export default function EsgDataCollectorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Sources',value:'42',icon:Database,color:'#34C759'},{label:'Data Points',value:'4.2M',icon:Activity,color:'#007AFF'},{label:'Quality',value:'98.4%',icon:Target,color:'#FF9500'},{label:'Coverage',value:'92%',icon:CircleCheckBig,color:'#AF52DE'}];
  const capabilities = ["Data Aggregation","Source Validation","Normalization Engine","Quality Scoring","Real-time Ingestion","Historical Database","API Integration","Missing Data Handler"];
  const responsibilities = ["Aggregate ESG data from 42+ sources and providers","Validate data quality and flag inconsistencies","Normalize data across different reporting frameworks","Score data quality and reliability metrics","Ingest real-time ESG event feeds and news","Maintain historical ESG database for backtesting"];
  const activities = [{time:'2 min ago',text:'Ingested ESG data from 12 providers',icon:Database},{time:'6 min ago',text:'Validated 42K data points for quality',icon:Activity},{time:'14 min ago',text:'Normalized SASB to GRI mapping',icon:Layers},{time:'28 min ago',text:'Flagged 3 data inconsistencies',icon:AlertTriangle},{time:'1 hr ago',text:'Published data quality dashboard',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#2E7D3220' }]}>
          <Database size={48} color="#2E7D32" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI ESG Data Collector</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Multi-source ESG Data Aggregation</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI ESG Data Collector operates as a specialized sub-agent under AI ESG Analyst, delivering enterprise-grade capabilities in multi-source esg data aggregation within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="esg-data-collector" agentName="AI ESG Data Collector"/>
