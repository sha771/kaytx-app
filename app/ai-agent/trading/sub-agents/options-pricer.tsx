...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Calculator, DollarSign, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Pricing Acc","value":"99.8%","change":"+0.1%","trend":"up"},{"label":"Batch Speed","value":"0.8ms","change":"-0.2ms","trend":"up"},{"label":"Model Count","value":"8","change":"+1","trend":"up"},{"label":"Exotic Support","value":"6 types","change":"+1","trend":"up"}];

export default function OptionsPricerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Models',value:'8',icon:Calculator,color:'#34C759'},{label:'Priced',value:'24,000',icon:Activity,color:'#007AFF'},{label:'Speed',value:'0.1ms',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.8%',icon:Target,color:'#AF52DE'}];
  const capabilities = ["Black-Scholes","Binomial Model","Monte Carlo","Local Vol","Stochastic Vol","American Options","Exotic Pricing","Dividend Adjustment"];
  const responsibilities = ["Price European and American options using multiple models","Run Monte Carlo simulations for exotic and path-dependent options","Apply local and stochastic volatility models for surface fitting","Price exotic options including barriers, Asians, and lookbacks","Adjust pricing for dividend and corporate action impacts","Validate model outputs against market observed prices"];
  const activities = [{time:'1 min ago',text:'Priced 847 SPX options in 0.8ms batch',icon:Calculator},{time:'4 min ago',text:'Ran Monte Carlo for barrier option',icon:Activity},{time:'12 min ago',text:'Calibrated local vol model',icon:Target},{time:'28 min ago',text:'Adjusted pricing for AAPL dividend',icon:DollarSign},{time:'1 hr ago',text:'Published pricing model validation report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Options Pricer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Real-time Options Pricing Engine</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Options Pricer operates as a specialized sub-agent under AI Derivatives Specialist, delivering enterprise-grade capabilities in real-time options pricing engine within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="options-pricer" agentName="AI Options Pricer"/>
