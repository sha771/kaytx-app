...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, ShieldAlert, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Breach Rate","value":"0%","change":"0%","trend":"up"},{"label":"Detection","value":"100%","change":"0%","trend":"up"},{"label":"Avg Response","value":"0.3s","change":"-0.1s","trend":"up"},{"label":"Kill Switch","value":"Ready","change":"Ready","trend":"up"}];

export default function LimitBreachAlerterPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Monitored',value:'847',icon:Shield,color:'#34C759'},{label:'Breaches',value:'0',icon:CircleCheckBig,color:'#007AFF'},{label:'Escalations',value:'12',icon:AlertTriangle,color:'#FF9500'},{label:'Response',value:'0.3s',icon:Clock,color:'#AF52DE'}];
  const capabilities = ["Real-time Monitoring","Breach Detection","Auto-escalation","Kill Switch","Notification Engine","Workflow Trigger","Historical Log","Root Cause Analysis"];
  const responsibilities = ["Monitor risk limits in real-time across all dimensions","Detect limit breaches and threshold violations","Auto-escalate breaches to appropriate management","Activate kill switches for critical breaches","Generate breach notifications and workflow triggers","Maintain historical breach log and root cause analysis"];
  const activities = [{time:'1 min ago',text:'Cleared all 847 limit checks',icon:Shield},{time:'4 min ago',text:'Escalated near-breach on FX desk',icon:AlertTriangle},{time:'12 min ago',text:'Triggered notification for VaR threshold',icon:Zap},{time:'28 min ago',text:'Resolved escalation from morning session',icon:CircleCheckBig},{time:'1 hr ago',text:'Published breach analytics report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#C6282820' }]}>
          <Zap size={48} color="#C62828" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Limit Breach Alerter</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Real-time Limit Monitoring & Escalation</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Limit Breach Alerter operates as a specialized sub-agent under AI Trading Risk Manager, delivering enterprise-grade capabilities in real-time limit monitoring & escalation within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="limit-breach-alerter" agentName="AI Limit Breach Alerter"/>
