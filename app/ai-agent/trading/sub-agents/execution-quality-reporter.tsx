...
const METRICS = [{"label":"TCA Coverage","value":"100%","change":"0%","trend":"up"},{"label":"Avg Slippage","value":"0.3bps","change":"-0.1bps","trend":"up"},{"label":"Venue Score","value":"94.2","change":"+1.4","trend":"up"},{"label":"Reports","value":"847","change":"+42","trend":"up"}];

export default function ExecutionQualityReporterPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'TCA Reports',value:'847',icon:FileText,color:'#34C759'},{label:'Venues',value:'18',icon:Globe,color:'#007AFF'},{label:'Slippage',value:'0.3bps',icon:Activity,color:'#FF9500'},{label:'Score',value:'94.2',icon:Star,color:'#AF52DE'}];
  const capabilities = ["TCA Reporting","Slippage Analysis","Venue Scorecard","Fill Rate Metrics","Benchmark Comparison","Cost Attribution","Real-time QC","Historical Trending"];
  const responsibilities = ["Produce transaction cost analysis (TCA) reports","Analyze slippage by venue, strategy, and time period","Score venues on execution quality metrics","Track fill rates and partial fill statistics","Compare execution costs against VWAP/arrival benchmarks","Maintain historical execution quality trending"];
  const activities = [{time:'2 min ago',text:'Generated TCA report for 8,421 trades',icon:FileText},{time:'6 min ago',text:'Scored 18 venues on execution quality',icon:Star},{time:'14 min ago',text:'Analyzed slippage by strategy type',icon:Activity},{time:'28 min ago',text:'Compared costs vs arrival price',icon:Target},{time:'1 hr ago',text:'Published monthly venue scorecard',icon:ChartBarBig}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#7B1FA220' }]}>
          <FileText size={48} color="#7B1FA2" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Execution Quality Reporter</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>TCA Reporting & Venue Performance Scoring</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Execution Quality Reporter operates as a specialized sub-agent under AI Equity Trader, delivering enterprise-grade capabilities in tca reporting & venue performance scoring within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="execution-quality-reporter" agentName="AI Execution Quality Reporter"/>
