...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Briefcase, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Meetings Held","value":"84","change":"+4","trend":"up"},{"label":"Approval Rate","value":"78%","change":"+2%","trend":"up"},{"label":"Action Close","value":"94%","change":"+3%","trend":"up"},{"label":"On-time","value":"100%","change":"0%","trend":"up"}];

export default function InvestmentCommitteeCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Meetings',value:'84',icon:Calendar,color:'#34C759'},{label:'Approvals',value:'312',icon:CircleCheckBig,color:'#007AFF'},{label:'Action Items',value:'128',icon:Target,color:'#FF9500'},{label:'On-time',value:'100%',icon:Clock,color:'#AF52DE'}];
  const capabilities = ["Meeting Scheduling","Agenda Management","Approval Tracking","Memo Distribution","Vote Recording","Action Item Tracking","Quorum Verification","Document Repository"];
  const responsibilities = ["Schedule and coordinate Investment Committee meetings","Prepare and distribute meeting agendas and materials","Track approval status for investment decisions","Record votes and maintain decision audit trail","Manage action items and follow-up tracking","Ensure quorum and governance compliance"];
  const activities = [{time:'3 min ago',text:'Scheduled weekly IC meeting for Thursday',icon:Calendar},{time:'8 min ago',text:'Distributed diligence memo to committee',icon:FileText},{time:'15 min ago',text:'Recorded unanimous approval for Series B',icon:CircleCheckBig},{time:'30 min ago',text:'Updated 12 action items from last session',icon:Target},{time:'1 hr ago',text:'Verified quorum for special session',icon:Users}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <TouchableOpacity onPress={() => router.push('/ai-agent/trading/vp-investments')} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <Briefcase size={16} color="#1565C0" />
          <Text style={{ fontSize: 13, color: '#1565C0', marginLeft: 6, fontWeight: '600' }}>← AI VP Investments</Text>
        </TouchableOpacity>
        <View style={[styles.heroIconWrap, { backgroundColor: '#1565C020' }]}>
          <Users size={48} color="#1565C0" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Investment Committee Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Committee Scheduling & Approval Tracking</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Investment Committee Coordinator operates as a specialized sub-agent under AI VP Investments, delivering enterprise-grade capabilities in committee scheduling & approval tracking within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <TouchableOpacity onPress={()=>router.push('/ai-agent/trading/vp-investments')} style={[styles.agentCard,{backgroundColor:theme.colors.background||'#F2F2F7'}]}>
          <View style={[styles.agentIcon,{backgroundColor:'#1565C020'}]}><Briefcase size={28} color="#1565C0"/></View>
      ...
      <AgentFeatures agentId="investment-committee-coordinator" agentName="AI Investment Committee Coordinator"/>
