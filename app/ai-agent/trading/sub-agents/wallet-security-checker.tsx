...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, ShieldCheck, Eye, Key, Bitcoin } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Security Score","value":"98/100","change":"+2","trend":"up"},{"label":"Vuln Found","value":"0","change":"0","trend":"up"},{"label":"Approval Rev","value":"42","change":"+8","trend":"up"},{"label":"Audit Coverage","value":"100%","change":"0%","trend":"up"}];

export default function WalletSecurityCheckerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Audits',value:'847',icon:ShieldCheck,color:'#34C759'},{label:'Vulnerabilities',value:'0',icon:Shield,color:'#007AFF'},{label:'Approvals',value:'42K',icon:Eye,color:'#FF9500'},{label:'Score',value:'98/100',icon:Star,color:'#AF52DE'}];
  const capabilities = ["Multi-sig Verification","Cold Storage Audit","Vulnerability Scan","Approval Checker","Phishing Detection","Contract Interaction","Key Management","Incident Response"];
  const responsibilities = ["Verify multi-sig wallet configurations and quorum settings","Audit cold storage procedures and key management protocols","Scan for known vulnerabilities in connected contracts","Check token approval exposure and revoke risky approvals","Detect phishing and social engineering attack vectors","Maintain incident response procedures for security events"];
  const activities = [{time:'1 min ago',text:'Verified multi-sig quorum for treasury wallet',icon:ShieldCheck},{time:'4 min ago',text:'Scanned 42 contracts for vulnerabilities',icon:Shield},{time:'12 min ago',text:'Revoked 3 risky token approvals',icon:Eye},{time:'28 min ago',text:'Audited cold storage key rotation',icon:Key},{time:'1 hr ago',text:'Published security audit report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Wallet Security Checker</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Multi-sig Verification & Vulnerability Scan</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Wallet Security Checker operates as a specialized sub-agent under AI Crypto Trader, delivering enterprise-grade capabilities in multi-sig verification & vulnerability scan within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="wallet-security-checker" agentName="AI Wallet Security Checker"/>
