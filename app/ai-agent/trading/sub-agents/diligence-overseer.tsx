...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Search, ClipboardCheck, Briefcase, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Completion","value":"94%","change":"+2%","trend":"up"},{"label":"Avg Duration","value":"18 days","change":"-2 days","trend":"up"},{"label":"Risk Flags","value":"12","change":"+3","trend":"up"},{"label":"Quality Score","value":"96.4%","change":"+1.2%","trend":"up"}];

export default function DiligenceOverseerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Diligence',value:'24',icon:Search,color:'#34C759'},{label:'Checklists',value:'847',icon:ClipboardCheck,color:'#007AFF'},{label:'Completion',value:'94%',icon:CircleCheckBig,color:'#FF9500'},{label:'Flags',value:'12',icon:AlertTriangle,color:'#AF52DE'}];
  const capabilities = ["Due Diligence Workflow","Checklist Management","Vendor Coordination","Document Review","Risk Flagging","Compliance Check","Timeline Tracking","Quality Assurance"];
  const responsibilities = ["Orchestrate due diligence workflows across workstreams","Manage diligence checklists and milestone tracking","Coordinate with external vendors and consultants","Review and flag issues in diligence documentation","Ensure compliance requirements are met in diligence","Track timeline and escalate delays and blockers"];
  const activities = [{time:'2 min ago',text:'Completed financial diligence on 2 deals',icon:Search},{time:'6 min ago',text:'Flagged legal risk in term sheet',icon:AlertTriangle},{time:'14 min ago',text:'Updated 847 checklist items',icon:ClipboardCheck},{time:'28 min ago',text:'Coordinated vendor call for tech audit',icon:Users},{time:'1 hr ago',text:'Published diligence status dashboard',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#1565C020' }]}>
          <Search size={48} color="#1565C0" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Diligence Overseer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Due Diligence Process Coordination</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Diligence Overseer operates as a specialized sub-agent under AI VP Investments, delivering enterprise-grade capabilities in due diligence process coordination within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="diligence-overseer" agentName="AI Diligence Overseer"/>
