...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Layers, History, Globe, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Scenario Count","value":"847","change":"+42","trend":"up"},{"label":"Impact Acc","value":"96.4%","change":"+0.8%","trend":"up"},{"label":"Variables","value":"42","change":"+4","trend":"up"},{"label":"Coverage","value":"100%","change":"0%","trend":"up"}];

export default function ScenarioModelerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Scenarios',value:'847',icon:Layers,color:'#34C759'},{label:'Variables',value:'42',icon:Target,color:'#007AFF'},{label:'Impact',value:'Real-time',icon:Activity,color:'#FF9500'},{label:'Accuracy',value:'96.4%',icon:CircleCheckBig,color:'#AF52DE'}];
  const capabilities = ["Macro Scenarios","Factor Shocks","Historical Replay","Custom Events","Probability Weighting","Impact Estimation","Multi-asset","Conditional Analysis"];
  const responsibilities = ["Design macro scenario frameworks for stress testing","Model factor shock scenarios with probability weighting","Run historical replay scenarios on current portfolio","Create custom event scenarios for emerging risks","Estimate portfolio impact under each scenario","Produce conditional analysis for tail risk scenarios"];
  const activities = [{time:'2 min ago',text:'Designed rate shock scenario with 3 levels',icon:Layers},{time:'6 min ago',text:'Ran 2008 replay on current portfolio',icon:History},{time:'14 min ago',text:'Created custom geopolitical scenario',icon:Globe},{time:'28 min ago',text:'Estimated portfolio impact per scenario',icon:Target},{time:'1 hr ago',text:'Published scenario analysis report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#D32F2F20' }]}>
          <Layers size={48} color="#D32F2F" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Scenario Modeler</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Custom Scenario Creation & Impact Analysis</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Scenario Modeler operates as a specialized sub-agent under AI Trading Risk Analyst, delivering enterprise-grade capabilities in custom scenario creation & impact analysis within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="scenario-modeler" agentName="AI Scenario Modeler"/>
