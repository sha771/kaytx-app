...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, ClipboardCheck, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Match Rate","value":"99.8%","change":"+0.1%","trend":"up"},{"label":"Auto-resolve","value":"94%","change":"+2%","trend":"up"},{"label":"Avg Resolution","value":"4.2 min","change":"-0.8 min","trend":"up"},{"label":"Coverage","value":"100%","change":"0%","trend":"up"}];

export default function TradeReconcilerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Matched',value:'18,400',icon:ClipboardCheck,color:'#34C759'},{label:'Breaks',value:'12',icon:AlertTriangle,color:'#007AFF'},{label:'Auto-resolved',value:'94%',icon:Zap,color:'#FF9500'},{label:'Accuracy',value:'99.8%',icon:Target,color:'#AF52DE'}];
  const capabilities = ["Trade Matching","Break Detection","Multi-system Sync","Auto-resolution","Exception Queue","Custodian Reconcile","Real-time Monitor","Historical Audit"];
  const responsibilities = ["Match trade records across front-office, back-office, and custodian","Detect breaks and discrepancies between systems","Synchronize trade data across multiple platforms","Auto-resolve common break types with rule engine","Manage exception queue for complex breaks","Reconcile with custodian statements daily"];
  const activities = [{time:'1 min ago',text:'Reconciled 4,200 equity trades for T+1',icon:ClipboardCheck},{time:'4 min ago',text:'Detected 3 breaks in FX settlement',icon:AlertTriangle},{time:'12 min ago',text:'Auto-resolved 8 common break types',icon:Zap},{time:'28 min ago',text:'Synced trade data with custodian',icon:Activity},{time:'1 hr ago',text:'Published reconciliation status report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Trade Reconciler</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Multi-system Trade Matching & Break Resolution</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Trade Reconciler operates as a specialized sub-agent under AI Settlement Specialist, delivering enterprise-grade capabilities in multi-system trade matching & break resolution within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="trade-reconciler" agentName="AI Trade Reconciler"/>
