...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Globe, Code, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Avg Latency","value":"0.3ms","change":"-0.05ms","trend":"up"},{"label":"P99 Latency","value":"0.8ms","change":"-0.1ms","trend":"up"},{"label":"Bottlenecks","value":"0","change":"0","trend":"up"},{"label":"Optimizations","value":"847","change":"+42","trend":"up"}];

export default function LatencyOptimizerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Avg Latency',value:'0.3ms',icon:Zap,color:'#34C759'},{label:'P99',value:'0.8ms',icon:Clock,color:'#007AFF'},{label:'Bottlenecks',value:'0',icon:Target,color:'#FF9500'},{label:'Optimizations',value:'847',icon:Activity,color:'#AF52DE'}];
  const capabilities = ["Latency Profiling","Bottleneck Detection","Memory Optimization","Network Tuning","CPU Optimization","Queue Management","Kernel Bypass","FPGA Interface"];
  const responsibilities = ["Profile end-to-end latency from signal to order","Detect and eliminate bottlenecks in execution pipeline","Optimize memory allocation and garbage collection","Tune network stack for minimal round-trip time","Optimize CPU utilization and thread scheduling","Manage message queue priority and processing order"];
  const activities = [{time:'1 min ago',text:'Reduced signal-to-order latency by 0.1ms',icon:Zap},{time:'4 min ago',text:'Eliminated GC pause bottleneck',icon:Activity},{time:'12 min ago',text:'Tuned TCP buffer for exchange connection',icon:Globe},{time:'28 min ago',text:'Optimized thread scheduling for strategy',icon:Code},{time:'1 hr ago',text:'Published latency profiling report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#4527A020' }]}>
          <Zap size={48} color="#4527A0" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Latency Optimizer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>End-to-end Latency Profiling & Optimization</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Latency Optimizer operates as a specialized sub-agent under AI Algo Trading Developer, delivering enterprise-grade capabilities in end-to-end latency profiling & optimization within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="latency-optimizer" agentName="AI Latency Optimizer"/>
