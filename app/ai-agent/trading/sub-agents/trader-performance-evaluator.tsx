...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, BarChart3, Monitor, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Avg Score","value":"87.4","change":"+2.1","trend":"up"},{"label":"Top Score","value":"96.4","change":"+1.2","trend":"up"},{"label":"Skill Ratio","value":"72%","change":"+4%","trend":"up"},{"label":"Compliance","value":"100%","change":"0%","trend":"up"}];

export default function TraderPerformanceEvaluatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Traders',value:'24',icon:Users,color:'#34C759'},{label:'KPIs',value:'847',icon:Target,color:'#007AFF'},{label:'Top Score',value:'96.4',icon:Star,color:'#FF9500'},{label:'Reviews',value:'312',icon:FileText,color:'#AF52DE'}];
  const capabilities = ["KPI Tracking","Benchmark Scoring","P&L Attribution","Behavioral Analysis","Skill Assessment","Compliance Scoring","Peer Ranking","Development Plans"];
  const responsibilities = ["Track trader KPIs and performance metrics","Score traders against execution benchmarks","Attribute P&L to skill vs market factors","Analyze behavioral patterns and risk-taking","Assess trader skill and development areas","Produce peer ranking and development plans"];
  const activities = [{time:'2 min ago',text:'Scored 24 traders on daily KPIs',icon:BarChart3},{time:'6 min ago',text:'Attributed P&L to skill vs market',icon:Target},{time:'14 min ago',text:'Flagged risk-taking behavior pattern',icon:AlertTriangle},{time:'28 min ago',text:'Updated peer ranking leaderboard',icon:Users},{time:'1 hr ago',text:'Published weekly performance review',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#0288D120' }]}>
          <BarChart3 size={48} color="#0288D1" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Trader Performance Evaluator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>KPI Tracking & Performance Scoring</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Trader Performance Evaluator operates as a specialized sub-agent under AI Trading Desk Manager, delivering enterprise-grade capabilities in kpi tracking & performance scoring within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="trader-performance-evaluator" agentName="AI Trader Performance Evaluator"/>
