...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Globe, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Fill Rate","value":"98.6%","change":"+0.4%","trend":"up"},{"label":"Avg Slippage","value":"0.3bps","change":"-0.1bps","trend":"up"},{"label":"VWAP Dev","value":"-0.2bps","change":"-0.1bps","trend":"up"},{"label":"Speed","value":"0.2ms","change":"-0.05ms","trend":"up"}];

export default function OrderExecutorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Orders',value:'8,421',icon:Zap,color:'#34C759'},{label:'Fill Rate',value:'98.6%',icon:Target,color:'#007AFF'},{label:'Slippage',value:'0.3bps',icon:Activity,color:'#FF9500'},{label:'Speed',value:'0.2ms',icon:Clock,color:'#AF52DE'}];
  const capabilities = ["Order Slicing","Smart Routing","VWAP Execution","TWAP Execution","Implementation Shortfall","Participation Rate","Dark Pool Access","Block Execution"];
  const responsibilities = ["Slice large orders for optimal execution","Route orders to best venues including dark pools","Execute VWAP and TWAP strategies","Manage implementation shortfall algorithms","Control participation rate and market impact","Execute block trades with minimal information leakage"];
  const activities = [{time:'1 min ago',text:'Sliced 50K share order into 847 child orders',icon:Zap},{time:'4 min ago',text:'Routed 200K to dark pool for block fill',icon:Globe},{time:'12 min ago',text:'Completed VWAP execution for large order',icon:Target},{time:'28 min ago',text:'Adjusted participation rate from 15% to 12%',icon:Activity},{time:'1 hr ago',text:'Published execution quality summary',icon:FileText}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <TouchableOpacity onPress={() => router.push('/ai-agent/trading/equity-trader')} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <TrendingUp size={16} color="#7B1FA2" />
          <Text style={{ fontSize: 13, color: '#7B1FA2', marginLeft: 6, fontWeight: '600' }}>← AI Equity Trader</Text>
        </TouchableOpacity>
        <View style={[styles.heroIconWrap, { backgroundColor: '#7B1FA220' }]}>
          <Zap size={48} color="#7B1FA2" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Order Executor</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Automated Order Slicing & Routing</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#7B1FA222' }]}><Star size={12} color="#7B1FA2" /><Text style={[styles.badgeText, { color: '#7B1FA2' }]}>Sub-Agent</Text></View>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Order Executor operates as a specialized sub-agent under AI Equity Trader, delivering enterprise-grade capabilities in automated order slicing & routing within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        <Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Enterprise Capabilities</Text>
        <View style={styles.tagsContainer}>{capabilities.map((c,i)=>(<View key={i} style={[styles.tag,{backgroundColor:'#7B1FA218'}]}><Text style={[styles.tagText,{color:'#7B1FA2'}]}>{c}</Text></View>))}</View>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        <Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Key Responsibilities</Text>
        {responsibilities.map((r,i)=>(<View key={i} style={styles.responsibilityRow}><ArrowRight size={14} color="#7B1FA2"/><Text style={[styles.responsibilityText,{color:theme.colors.textSecondary}]}>{r}</Text></View>))}
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        {activities.map((a,i)=>(<View key={i} style={styles.activityRow}><View style={[styles.activityIcon,{backgroundColor:'#7B1FA215'}]}><a.icon size={14} color="#7B1FA2"/></View><View style={styles.activityContent}><Text style={[styles.activityText,{color:theme.colors.text}]}>{a.text}</Text><Text style={[styles.activityTime,{color:theme.colors.textSecondary}]}>{a.time}</Text></View></View>))}
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        <Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Parent Agent</Text>
        <TouchableOpacity onPress={()=>router.push('/ai-agent/trading/equity-trader')} style={[styles.agentCard,{backgroundColor:theme.colors.background||'#F2F2F7'}]}>
          <View style={[styles.agentIcon,{backgroundColor:'#7B1FA220'}]}><TrendingUp size={28} color="#7B1FA2"/></View>
      ...
      <AgentFeatures agentId="order-executor" agentName="AI Order Executor"/>
