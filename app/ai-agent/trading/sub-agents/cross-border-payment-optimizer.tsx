...
const METRICS = [{"label":"Cost Saved","value":"$840K","change":"+$120K","trend":"up"},{"label":"Avg Speed","value":"T+0","change":"-T+0.2","trend":"up"},{"label":"Route Opt","value":"98.4%","change":"+1.2%","trend":"up"},{"label":"Compliance","value":"100%","change":"0%","trend":"up"}];

export default function CrossBorderPaymentOptimizerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Payments',value:'2,400',icon:DollarSign,color:'#34C759'},{label:'Currencies',value:'42',icon:Globe,color:'#007AFF'},{label:'Avg Cost',value:'0.12%',icon:Target,color:'#FF9500'},{label:'Speed',value:'T+0',icon:Clock,color:'#AF52DE'}];
  const capabilities = ["Route Optimization","FX Conversion","Settlement Network","Compliance Check","Cost Minimization","Speed Optimization","Multi-currency","Beneficiary Mgmt"];
  const responsibilities = ["Optimize cross-border payment routing and settlement","Minimize FX conversion costs through netting and batching","Select optimal settlement networks per currency pair","Ensure compliance with cross-border payment regulations","Balance cost vs speed for different payment priorities","Manage multi-currency beneficiary information"];
  const activities = [{time:'2 min ago',text:'Optimized CNY payment route via HK network',icon:DollarSign},{time:'6 min ago',text:'Netted 42 EUR payments for cost savings',icon:Activity},{time:'14 min ago',text:'Selected SWIFT gpi for urgent USD transfer',icon:Globe},{time:'28 min ago',text:'Cleared compliance for 18 cross-border payments',icon:Shield},{time:'1 hr ago',text:'Published payment cost analysis report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#00897B20' }]}>
          <DollarSign size={48} color="#00897B" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Cross-border Payment Optimizer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Settlement Route Optimization & FX Conversion</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Cross-border Payment Optimizer operates as a specialized sub-agent under AI Forex Trader, delivering enterprise-grade capabilities in settlement route optimization & fx conversion within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="cross-border-payment-optimizer" agentName="AI Cross-border Payment Optimizer"/>
