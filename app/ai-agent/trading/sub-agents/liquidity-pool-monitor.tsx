...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, DollarSign, BarChart3, Bitcoin, Key, Monitor } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"TVL Tracked","value":"$89M","change":"+$12M","trend":"up"},{"label":"Avg APY","value":"14.2%","change":"+1.8%","trend":"up"},{"label":"IL Minimized","value":"94%","change":"+2%","trend":"up"},{"label":"Pool Count","value":"847","change":"+42","trend":"up"}];

export default function LiquidityPoolMonitorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Pools',value:'847',icon:Activity,color:'#34C759'},{label:'TVL',value:'$89M',icon:DollarSign,color:'#007AFF'},{label:'APY',value:'14.2%',icon:TrendingUp,color:'#FF9500'},{label:'IL Track',value:'Real-time',icon:AlertTriangle,color:'#AF52DE'}];
  const capabilities = ["TVL Tracking","Pool Analytics","Impermanent Loss","APY Monitoring","Concentration Analysis","Fee Revenue","Volume Tracking","Protocol Comparison"];
  const responsibilities = ["Track total value locked across DeFi protocols","Analyze pool composition and concentration risk","Calculate impermanent loss for LP positions","Monitor APY changes and yield opportunity signals","Track fee revenue and volume for pool profitability","Compare protocol metrics for allocation decisions"];
  const activities = [{time:'2 min ago',text:'Updated TVL across 847 pools',icon:Activity},{time:'6 min ago',text:'Calculated IL for 42 LP positions',icon:AlertTriangle},{time:'14 min ago',text:'Detected APY spike in ETH/USDC pool',icon:TrendingUp},{time:'28 min ago',text:'Compared Uniswap vs Curve metrics',icon:BarChart3},{time:'1 hr ago',text:'Published DeFi pool performance report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Liquidity Pool Monitor</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>DEX Pool Tracking & Impermanent Loss</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Liquidity Pool Monitor operates as a specialized sub-agent under AI Crypto Trader, delivering enterprise-grade capabilities in dex pool tracking & impermanent loss within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="liquidity-pool-monitor" agentName="AI Liquidity Pool Monitor"/>
