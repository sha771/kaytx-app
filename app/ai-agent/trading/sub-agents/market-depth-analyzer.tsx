...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Search, Layers, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Signal Acc","value":"91.4%","change":"+2.1%","trend":"up"},{"label":"Imbalance Det","value":"87%","change":"+3%","trend":"up"},{"label":"Hidden Liq","value":"$4.2B","change":"+$600M","trend":"up"},{"label":"Spread Track","value":"Real-time","change":"Stable","trend":"up"}];

export default function MarketDepthAnalyzerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Books',value:'847',icon:Search,color:'#34C759'},{label:'Liquidity',value:'Real-time',icon:Activity,color:'#007AFF'},{label:'Signals',value:'42',icon:Zap,color:'#FF9500'},{label:'Depth',value:'L3',icon:Layers,color:'#AF52DE'}];
  const capabilities = ["Order Book Analysis","Liquidity Mapping","Bid-Ask Spread","Depth Charts","Hidden Liquidity","Venue Comparison","Imbalance Detection","Auction Analysis"];
  const responsibilities = ["Analyze real-time order book depth and liquidity","Map liquidity across venues and dark pools","Monitor bid-ask spread dynamics and trends","Detect hidden liquidity and iceberg orders","Compare venue depth and fill probability","Identify order imbalance and auction signals"];
  const activities = [{time:'1 min ago',text:'Detected iceberg order in AAPL book',icon:Search},{time:'4 min ago',text:'Mapped liquidity across 18 venues',icon:Activity},{time:'12 min ago',text:'Flagged imbalance signal for NVDA',icon:Zap},{time:'28 min ago',text:'Updated depth chart visualization',icon:Layers},{time:'1 hr ago',text:'Published liquidity assessment report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#7B1FA220' }]}>
          <Search size={48} color="#7B1FA2" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Market Depth Analyzer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Order Book Analysis & Liquidity Mapping</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Market Depth Analyzer operates as a specialized sub-agent under AI Equity Trader, delivering enterprise-grade capabilities in order book analysis & liquidity mapping within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="market-depth-analyzer" agentName="AI Market Depth Analyzer"/>
