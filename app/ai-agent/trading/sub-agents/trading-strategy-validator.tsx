...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, History, BarChart3, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Pass Rate","value":"94.2%","change":"+1.8%","trend":"up"},{"label":"Avg Sharpe","value":"1.84","change":"+0.12","trend":"up"},{"label":"Overfit Det","value":"98.4%","change":"+0.6%","trend":"up"},{"label":"Backtest Speed","value":"8ms","change":"-2ms","trend":"up"}];

export default function TradingStrategyValidatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Validated',value:'128',icon:CircleCheckBig,color:'#34C759'},{label:'Backtests',value:'24,000',icon:History,color:'#007AFF'},{label:'Pass Rate',value:'94.2%',icon:Target,color:'#FF9500'},{label:'Avg Sharpe',value:'1.84',icon:TrendingUp,color:'#AF52DE'}];
  const capabilities = ["Strategy Backtesting","Performance Benchmarking","Walk-forward Analysis","Parameter Sensitivity","Overfitting Detection","Sharpe Validation","Drawdown Testing","Correlation Check"];
  const responsibilities = ["Validate new trading strategies before deployment","Run comprehensive backtests with realistic assumptions","Conduct walk-forward and out-of-sample testing","Detect overfitting and parameter sensitivity issues","Benchmark strategy performance against peers","Validate drawdown and risk-adjusted return metrics"];
  const activities = [{time:'2 min ago',text:'Validated momentum strategy with 2.1 Sharpe',icon:Target},{time:'6 min ago',text:'Completed walk-forward analysis on 3 strategies',icon:History},{time:'14 min ago',text:'Detected overfitting in parameter grid',icon:AlertTriangle},{time:'28 min ago',text:'Benchmarked strategy vs peer universe',icon:BarChart3},{time:'1 hr ago',text:'Published validation report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#1565C020' }]}>
          <Target size={48} color="#1565C0" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Trading Strategy Validator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Strategy Backtesting & Performance Benchmarking</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Trading Strategy Validator operates as a specialized sub-agent under AI VP Trading, delivering enterprise-grade capabilities in strategy backtesting & performance benchmarking within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="trading-strategy-validator" agentName="AI Trading Strategy Validator"/>
