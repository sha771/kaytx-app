...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Eye, ShieldCheck, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Detection Rate","value":"99.2%","change":"+0.4%","trend":"up"},{"label":"False Pos","value":"2.1%","change":"-0.4%","trend":"up"},{"label":"Scan Speed","value":"0.4s","change":"-0.1s","trend":"up"},{"label":"Coverage","value":"100%","change":"0%","trend":"up"}];

export default function TradeSurveillanceAgentPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Monitored',value:'24,000',icon:Eye,color:'#34C759'},{label:'Alerts',value:'42',icon:AlertTriangle,color:'#007AFF'},{label:'Abuse Det',value:'99.2%',icon:Shield,color:'#FF9500'},{label:'False Pos',value:'2.1%',icon:Target,color:'#AF52DE'}];
  const capabilities = ["Pattern Detection","Abuse Monitoring","Insider Detection","Spoofing Detection","Wash Trade Alert","Front-running Check","Unusual Volume","Communication Scan"];
  const responsibilities = ["Detect suspicious trading patterns and market abuse","Monitor for insider trading signals and information leakage","Identify spoofing, layering, and wash trade patterns","Check for front-running and conflict of interest","Alert on unusual volume and price movements","Scan communications for compliance violations"];
  const activities = [{time:'1 min ago',text:'Scanned 24,000 trades for abuse patterns',icon:Eye},{time:'4 min ago',text:'Detected potential spoofing in options book',icon:AlertTriangle},{time:'12 min ago',text:'Cleared insider trading screen for 847 trades',icon:Shield},{time:'28 min ago',text:'Flagged unusual volume in small-cap',icon:Activity},{time:'1 hr ago',text:'Published surveillance summary report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#455A6420' }]}>
          <Eye size={48} color="#455A64" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Trade Surveillance Agent</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Real-time Pattern Monitoring & Abuse Detection</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Trade Surveillance Agent operates as a specialized sub-agent under AI Trading Compliance, delivering enterprise-grade capabilities in real-time pattern monitoring & abuse detection within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="trade-surveillance-agent" agentName="AI Trade Surveillance Agent"/>
