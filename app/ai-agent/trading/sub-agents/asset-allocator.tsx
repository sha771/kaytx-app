...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, PieChart, DollarSign, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Opt Sharpe","value":"1.84","change":"+0.12","trend":"up"},{"label":"Constraint Met","value":"100%","change":"0%","trend":"up"},{"label":"Risk Parity","value":"Balanced","change":"Stable","trend":"up"},{"label":"Liquidity","value":"98%","change":"+1%","trend":"up"}];

export default function AssetAllocatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Allocations',value:'42',icon:PieChart,color:'#34C759'},{label:'AUM',value:'$1.2B',icon:DollarSign,color:'#007AFF'},{label:'Optimizations',value:'847',icon:Activity,color:'#FF9500'},{label:'Sharpe',value:'1.84',icon:TrendingUp,color:'#AF52DE'}];
  const capabilities = ["Strategic Allocation","Tactical Tilts","Mean-Variance Opt","Black-Litterman","Risk Parity","Factor Allocation","Liquidity Budget","Constraint Engine"];
  const responsibilities = ["Execute strategic asset allocation per investment policy","Implement tactical tilts based on market views","Run mean-variance and Black-Litterman optimization","Manage risk parity and factor allocation","Ensure liquidity budget compliance","Apply custom constraints and regulatory limits"];
  const activities = [{time:'2 min ago',text:'Ran Black-Litterman optimization for Q3',icon:PieChart},{time:'6 min ago',text:'Implemented tactical tilt to emerging markets',icon:Zap},{time:'14 min ago',text:'Updated risk parity weights',icon:Activity},{time:'28 min ago',text:'Applied regulatory constraint set',icon:Shield},{time:'1 hr ago',text:'Published allocation efficiency report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Asset Allocator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Strategic & Tactical Allocation Optimization</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Asset Allocator operates as a specialized sub-agent under AI Portfolio Manager, delivering enterprise-grade capabilities in strategic & tactical allocation optimization within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="asset-allocator" agentName="AI Asset Allocator"/>
