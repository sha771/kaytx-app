...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, DollarSign, Layers, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"CVaR Acc","value":"96.4%","change":"+0.8%","trend":"up"},{"label":"EVT Fit","value":"0.94","change":"+0.02","trend":"up"},{"label":"Tail Det","value":"92%","change":"+2%","trend":"up"},{"label":"Coverage","value":"100%","change":"0%","trend":"up"}];

export default function TailRiskAssessorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tail Events',value:'12',icon:AlertTriangle,color:'#34C759'},{label:'CVaR',value:'$68M',icon:DollarSign,color:'#007AFF'},{label:'EVT Models',value:'3',icon:Layers,color:'#FF9500'},{label:'Accuracy',value:'96.4%',icon:Target,color:'#AF52DE'}];
  const capabilities = ["EVT Analysis","Conditional VaR","Expected Shortfall","Tail Dependence","Stress Loss Est","Drawdown Analysis","Recovery Modeling","Extreme Scenario"];
  const responsibilities = ["Apply extreme value theory to tail risk estimation","Calculate Conditional VaR and Expected Shortfall","Estimate tail dependence across asset classes","Project stress loss under extreme market conditions","Analyze drawdown depth and recovery time distributions","Model extreme scenario outcomes and probability bounds"];
  const activities = [{time:'2 min ago',text:'Calculated CVaR at 99.5% confidence',icon:AlertTriangle},{time:'6 min ago',text:'Applied EVT to equity tail distribution',icon:Layers},{time:'14 min ago',text:'Estimated tail dependence for credit-equity',icon:Activity},{time:'28 min ago',text:'Projected stress loss under crash scenario',icon:DollarSign},{time:'1 hr ago',text:'Published tail risk assessment report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Tail Risk Assessor</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Extreme Value Analysis & Conditional Tail Expectation</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Tail Risk Assessor operates as a specialized sub-agent under AI Trading Risk Analyst, delivering enterprise-grade capabilities in extreme value analysis & conditional tail expectation within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="tail-risk-assessor" agentName="AI Tail Risk Assessor"/>
