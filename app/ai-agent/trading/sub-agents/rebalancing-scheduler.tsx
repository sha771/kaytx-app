...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, DollarSign, PieChart, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Drift Catch","value":"99.2%","change":"+0.4%","trend":"up"},{"label":"Tax Saved","value":"$4.2M","change":"+$640K","trend":"up"},{"label":"Avg Cost","value":"0.8bps","change":"-0.2bps","trend":"up"},{"label":"On-time","value":"100%","change":"0%","trend":"up"}];

export default function RebalancingSchedulerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Rebalances',value:'847',icon:Calendar,color:'#34C759'},{label:'Drift Alerts',value:'42',icon:AlertTriangle,color:'#007AFF'},{label:'Tax Saved',value:'$4.2M',icon:DollarSign,color:'#FF9500'},{label:'On-schedule',value:'100%',icon:CircleCheckBig,color:'#AF52DE'}];
  const capabilities = ["Drift Monitoring","Threshold Alerts","Calendar Rebalancing","Tax-aware Execution","Trade Sizing","Multi-portfolio","Cash Flow Adj","Transaction Cost"];
  const responsibilities = ["Monitor portfolio drift against target weights","Generate threshold-based rebalancing alerts","Schedule calendar-based rebalancing events","Execute tax-aware rebalancing with loss harvesting","Size trades to minimize transaction costs","Manage rebalancing across multiple portfolios"];
  const activities = [{time:'1 min ago',text:'Triggered drift rebalance for 3 portfolios',icon:Calendar},{time:'4 min ago',text:'Harvested $840K in tax losses',icon:DollarSign},{time:'12 min ago',text:'Sized rebalance trades for min cost',icon:Target},{time:'28 min ago',text:'Adjusted cash flow for upcoming rebalance',icon:Activity},{time:'1 hr ago',text:'Published rebalancing execution report',icon:FileText}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Rebalancing Scheduler</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Automated Rebalancing Trigger & Execution</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Rebalancing Scheduler operates as a specialized sub-agent under AI Portfolio Manager, delivering enterprise-grade capabilities in automated rebalancing trigger & execution within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="rebalancing-scheduler" agentName="AI Rebalancing Scheduler"/>
