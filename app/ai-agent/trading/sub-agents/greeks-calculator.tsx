...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, BarChart3, Calculator, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Calc Speed","value":"0.2ms","change":"-0.05ms","trend":"up"},{"label":"Greeks Acc","value":"99.6%","change":"+0.2%","trend":"up"},{"label":"Hedge Ratio","value":"0.98","change":"+0.01","trend":"up"},{"label":"Coverage","value":"100%","change":"0%","trend":"up"}];

export default function GreeksCalculatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Positions',value:'2,847',icon:BarChart3,color:'#34C759'},{label:'Greeks',value:'Real-time',icon:Activity,color:'#007AFF'},{label:'Delta',value:'$12M',icon:TrendingUp,color:'#FF9500'},{label:'Vega',value:'$8.4M',icon:Zap,color:'#AF52DE'}];
  const capabilities = ["Delta Calculation","Gamma Analysis","Vega Exposure","Theta Decay","Rho Sensitivity","Higher-order Greeks","Portfolio Aggregation","Hedge Ratio Calc"];
  const responsibilities = ["Calculate first-order Greeks (Delta, Vega, Theta, Rho)","Compute second-order Greeks (Gamma, Vanna, Volga, Charm)","Aggregate Greeks across portfolio positions","Calculate hedge ratios for risk management","Monitor Greek exposure limits and concentration","Produce Greeks sensitivity reports and dashboards"];
  const activities = [{time:'1 min ago',text:'Aggregated portfolio delta across 2,847 positions',icon:BarChart3},{time:'4 min ago',text:'Computed Vanna and Volga for vol book',icon:Activity},{time:'12 min ago',text:'Calculated hedge ratios for delta-neutral',icon:Target},{time:'28 min ago',text:'Monitored gamma exposure limits',icon:AlertTriangle},{time:'1 hr ago',text:'Published Greeks sensitivity dashboard',icon:FileText}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <TouchableOpacity onPress={() => router.push('/ai-agent/trading/derivatives-specialist')} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <Calculator size={16} color="#E65100" />
          <Text style={{ fontSize: 13, color: '#E65100', marginLeft: 6, fontWeight: '600' }}>← AI Derivatives Specialist</Text>
        </TouchableOpacity>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E6510020' }]}>
          <BarChart3 size={48} color="#E65100" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Greeks Calculator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Portfolio Greeks Aggregation & Sensitivity</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#E6510022' }]}><Star size={12} color="#E65100" /><Text style={[styles.badgeText, { color: '#E65100' }]}>Sub-Agent</Text></View>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Greeks Calculator operates as a specialized sub-agent under AI Derivatives Specialist, delivering enterprise-grade capabilities in portfolio greeks aggregation & sensitivity within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        <Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Enterprise Capabilities</Text>
        <View style={styles.tagsContainer}>{capabilities.map((c,i)=>(<View key={i} style={[styles.tag,{backgroundColor:'#E6510018'}]}><Text style={[styles.tagText,{color:'#E65100'}]}>{c}</Text></View>))}</View>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        <Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Key Responsibilities</Text>
        {responsibilities.map((r,i)=>(<View key={i} style={styles.responsibilityRow}><ArrowRight size={14} color="#E65100"/><Text style={[styles.responsibilityText,{color:theme.colors.textSecondary}]}>{r}</Text></View>))}
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <TouchableOpacity onPress={()=>router.push('/ai-agent/trading/derivatives-specialist')} style={[styles.agentCard,{backgroundColor:theme.colors.background||'#F2F2F7'}]}>
          <View style={[styles.agentIcon,{backgroundColor:'#E6510020'}]}><Calculator size={28} color="#E65100"/></View>
      ...
      <AgentFeatures agentId="greeks-calculator" agentName="AI Greeks Calculator"/>
