...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Users, MessageSquare, Calendar, ChartBarBig, TrendingUp, AlertTriangle, FileText, ChevronRight, Shield, Layers, BarChart3, Leaf, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

const METRICS = [{"label":"Avg Score","value":"78.4","change":"+2.1","trend":"up"},{"label":"Score Acc","value":"96.2%","change":"+0.8%","trend":"up"},{"label":"Coverage","value":"4,200","change":"+180","trend":"up"},{"label":"Controversies","value":"3","change":"-2","trend":"up"}];

export default function SustainabilityScorerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Scores',value:'4,200',icon:Star,color:'#34C759'},{label:'Pillars',value:'3',icon:Layers,color:'#007AFF'},{label:'Avg Score',value:'78.4',icon:Target,color:'#FF9500'},{label:'Accuracy',value:'96.2%',icon:CircleCheckBig,color:'#AF52DE'}];
  const capabilities = ["Composite Scoring","Custom Weighting","Pillar Analysis","Controversy Adjustment","Peer Benchmarking","Sector-relative","Trend Analysis","Score Validation"];
  const responsibilities = ["Calculate composite ESG scores with custom weighting","Analyze E, S, and G pillar scores independently","Adjust scores for controversy events and severity","Benchmark scores against sector and peer groups","Produce sector-relative ESG rankings","Validate score stability and trend consistency"];
  const activities = [{time:'2 min ago',text:'Updated ESG scores for 42 holdings',icon:Star},{time:'6 min ago',text:'Adjusted scores for mining controversy',icon:AlertTriangle},{time:'14 min ago',text:'Benchmarked scores vs sector peers',icon:BarChart3},{time:'28 min ago',text:'Validated score trend consistency',icon:Activity},{time:'1 hr ago',text:'Published composite scoring report',icon:FileText}];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={[styles.heroIconWrap, { backgroundColor: '#2E7D3220' }]}>
          <Star size={48} color="#2E7D32" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Sustainability Scorer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Composite ESG Scoring Engine</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
        ...
        <Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Sustainability Scorer operates as a specialized sub-agent under AI ESG Analyst, delivering enterprise-grade capabilities in composite esg scoring engine within the Trading & Investments department.</Text>
      </View>
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}>
      ...
      <AgentFeatures agentId="sustainability-scorer" agentName="AI Sustainability Scorer"/>
