...
import { Calculator, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'4,396',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.4%',icon:Target,color:'#0277BD'}];
  const capabilities = ['Quant Modeling','Statistical Analysis','Machine Learning','Factor Analysis','Time Series','Risk Modeling'];
  const responsibilities = ['Quantitative model development & testing','Statistical analysis & hypothesis testing','Machine learning model deployment','Factor analysis & alpha generation','Time series forecasting','Quantitative risk modeling'];
  const activities = [{time:'3 min ago',text:'Deployed new ML alpha model',icon: CircleCheckBig},{time:'6 min ago',text:'Validated factor return predictions',icon:Clock},{time:'9 min ago',text:'Updated risk model parameters',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0277BD20' }]}><Calculator size={48} color="#0277BD" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Quantitative Analyst</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Quantitative Research</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#0277BD22' }]}><Star size={12} color="#0277BD" /><Text style={[styles.badgeText, { color: '#0277BD' }]}>Analyst</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Quantitative Analyst AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Trading & Investment department.</Text></View>
      ...
      <AgentFeatures agentId="quant-analyst-1" agentName="Quantitative Analyst" />
    </ScrollView>
  );
}
