...
import { Shield, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Key } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'1,542',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.0%',icon:Target,color:'#0277BD'}];
  const capabilities = ['Risk Mgmt','Policy Design','Limit Setting','Counterparty Risk','Market Risk','Operational Risk'];
  const responsibilities = ['Trading risk management framework','Risk policy design & implementation','Trading limit setting & monitoring','Counterparty risk assessment','Market risk monitoring & control','Operational risk identification'];
  const activities = [{time:'3 min ago',text:'Updated risk policy for Q3',icon: CircleCheckBig},{time:'6 min ago',text:'Reviewed counterparty exposures',icon:Clock},{time:'9 min ago',text:'Approved new limit framework',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0277BD20' }]}><Shield size={48} color="#0277BD" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Trading Risk Manager</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Risk Management</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#0277BD22' }]}><Star size={12} color="#0277BD" /><Text style={[styles.badgeText, { color: '#0277BD' }]}>Manager</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Trading Risk Manager AI coordinates team operations, streamlines workflows, and ensures quality delivery within the Trading & Investment department.</Text></View>
      ...
      <AgentFeatures agentId="risk-manager-trading" agentName="Trading Risk Manager" />
    </ScrollView>
  );
}
