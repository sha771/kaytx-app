...
import { Activity, Package, Clock, Target, Zap, ArrowRight, Briefcase, Layers, PieChart, BarChart3 } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

const agent = {
  name: 'AI ABC Analyzer',
  parent: 'AI Inventory Specialist',
  parentRoute: '/ai-agent/supply-chain/ai-inventory-specialist',
  color: '#EC4899',
};

export default function AbcAnalyzerPage() {
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#EC489922' }]}><Briefcase size={12} color="#EC4899" /><Text style={[styles.badgeText, { color: '#EC4899' }]}>Analyzer</Text></View>
      ...
      <View style={styles.statsContainer}>
        {[
          ...
          {label:'Level',value:'Analyzer',icon: Briefcase, color: '#EC4899'},
          {label:'Classified',value:'50K+',icon: PieChart, color: '#FF9500'},
          {label:'Value',value:'80%',icon: BarChart3, color: '#007AFF'}
        ].map((stat,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI ABC Analyzer performs ABC analysis to classify inventory items based on value and consumption. This enterprise-level sub-agent helps prioritize inventory management efforts and allocate resources effectively.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.tagsContainer}>
          {['Value Classification','Consumption Analysis','Category Management','Pareto Analysis','Inventory Segmentation','Reporting'].map((cap,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {['/consult/abc-analyzer','/abc-analyzer/execute','/abc-analyzer/analyze'].map((endpoint,index)=>(
      ...
      <AgentFeatures agentId="abc-analyzer" agentName="AI ABC Analyzer" />
