...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3,487',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.5%',icon:Target,color:'#E65100'}];
  const capabilities = ['BD Strategy','Partnership Development','Market Expansion','Strategic Alliances','Revenue Growth','Team Leadership'];
  const responsibilities = ['Business development strategy','Partnership development & management','New market expansion planning','Strategic alliance cultivation','Revenue growth initiatives','BD team leadership & development'];
  const activities = [{time:'3 min ago',text:'Closed 3 strategic partnerships',icon: CircleCheckBig},{time:'6 min ago',text:'Entered 2 new market segments',icon:Clock},{time:'9 min ago',text:'Managed BD team of 12',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E6510020' }]}><Globe size={48} color="#E65100" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Business Development</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Business Development</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#E6510022' }]}><Star size={12} color="#E65100" /><Text style={[styles.badgeText, { color: '#E65100' }]}>VP Level</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Business Development AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Sales division.</Text></View>
      ...
      <AgentFeatures agentId="vp-business-development" agentName="VP Business Development" />
    </ScrollView>
  );
}
