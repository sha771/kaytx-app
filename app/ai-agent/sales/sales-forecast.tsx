...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'5,423',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.0%',icon:Target,color:'#E65100'}];
  const capabilities = ['Revenue Forecasting','Pipeline Analysis','Trend Analysis','Data Modeling','Reporting','Accuracy Tracking'];
  const responsibilities = ['Revenue forecasting & prediction','Sales pipeline analysis & scoring','Market & trend analysis','Forecast data modeling','Forecast reporting & presentation','Forecast accuracy tracking & improvement'];
  const activities = [{time:'3 min ago',text:'Published Q3 revenue forecast',icon: CircleCheckBig},{time:'6 min ago',text:'Analyzed pipeline conversion rates',icon:Clock},{time:'9 min ago',text:'Improved forecast accuracy to 92%',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#E6510020' }]}><TrendingUp size={48} color="#E65100" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Sales Forecast Analyst</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Revenue Forecasting</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Sales Forecast Analyst AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Sales department.</Text></View>
      ...
      <AgentFeatures agentId="sales-forecast" agentName="Sales Forecast Analyst" />
    </ScrollView>
  );
}
