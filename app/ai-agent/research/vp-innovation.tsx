...
import { Rocket, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'5,351',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.9%',icon:Target,color:'#006064'}];
  const capabilities = ['Innovation Strategy','Portfolio Mgmt','External Partnerships','Budget Oversight','Talent Strategy','Board Reporting'];
  const responsibilities = ['Innovation strategy & vision','Innovation portfolio management','External partnership development','Innovation budget oversight','Innovation talent strategy','Board & executive reporting'];
  const activities = [{time:'3 min ago',text:'Presented innovation strategy to board',icon: CircleCheckBig},{time:'6 min ago',text:'Approved $5M innovation budget',icon:Clock},{time:'9 min ago',text:'Established 3 research partnerships',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#00606420' }]}><Rocket size={48} color="#006064" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Innovation</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Innovation Division</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#00606422' }]}><Star size={12} color="#006064" /><Text style={[styles.badgeText, { color: '#006064' }]}>VP Level</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Innovation AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Research & Development division.</Text></View>
      ...
      <AgentFeatures agentId="vp-innovation" agentName="VP Innovation" />
    </ScrollView>
  );
}
