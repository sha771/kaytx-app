...
import { Activity, Microscope, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, Lightbulb, CheckCircle, Layers, Settings, Beaker, FlaskConical, Play, Pause } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function ExperimentExecutorPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Experiments', value: '1,247', icon: Beaker, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.4s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.2%', icon: Briefcase, color: '#14B8A6' },
  ];

  const capabilities = ['Experiment Execution', 'Data Collection', 'Control Management', 'Variable Manipulation', 'Real-time Monitoring', 'Error Detection', 'Result Logging', 'Safety Protocols'];

  const responsibilities = [
    'Execute designed experiments according to protocols',
    'Collect and record experimental data accurately',
    'Monitor experiments in real-time for anomalies',
    'Manage experimental controls and variables',
    'Detect and report errors or deviations',
    'Log results with proper documentation'
  ];

  const activities = [
    { time: '2 min ago', text: 'Started ML training experiment #1247', icon: Play },
    { time: '15 min ago', text: 'Completed A/B test analysis', icon: CheckCircle },
    { time: '1 hour ago', text: 'Detected anomaly in experiment #1245', icon: Zap },
    { time: '3 hours ago', text: 'Logged 847 data points', icon: FileText },
  ];

  const quickActions = [
    { label: 'Run', icon: Play, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/experiment/executor/status', description: 'Get execution status' },
    { method: 'POST', endpoint: '/api/v1/experiment/executor/start', description: 'Start experiment' },
    { method: 'POST', endpoint: '/api/v1/experiment/executor/stop', description: 'Stop experiment' },
    { method: 'GET', endpoint: '/api/v1/experiment/executor/results', description: 'Get results' },
  ];

  const performanceMetrics = [
    { label: 'Experiments Run', value: '1,247', trend: '+28%' },
    { label: 'Success Rate', value: '96%', trend: '+10%' },
    { label: 'Avg Duration', value: '4.2h', trend: '-15%' },
    { label: 'Data Points', value: '89K', trend: '+45%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#14B8A618', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#14B8A625' }]}>
          <Beaker size={56} color="#14B8A6" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Experiment Executor</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Experiment Executor runs designed experiments, collects data, monitors for anomalies, and logs results with precision and safety compliance.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/hypothesis-designer' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#6366F115' }]}>
            <FlaskConical size={18} color="#6366F1" />
            <Text style={[styles.relatedAgentText, { color: '#6366F1' }]}>Hypothesis Designer</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/experiment-tracker' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#F59E0B15' }]}>
            <Target size={18} color="#F59E0B" />
            <Text style={[styles.relatedAgentText, { color: '#F59E0B' }]}>Experiment Tracker</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="experiment-executor" agentName="AI Experiment Executor" />
