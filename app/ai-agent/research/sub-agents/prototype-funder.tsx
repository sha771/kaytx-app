...
import { Activity, Lightbulb, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, CheckCircle, Layers, Settings, DollarSign, Wallet, TrendingUp as TrendUp, Rocket } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function PrototypeFunderPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Funded', value: '$2.4M', icon: DollarSign, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.5s', icon: Clock, color: '#FF9500' },
    { label: 'Success', value: '89%', icon: Briefcase, color: '#22C55E' },
  ];

  const capabilities = ['Funding Evaluation', 'Budget Allocation', 'Investment Analysis', 'Risk Assessment', 'ROI Projection', 'Milestone Tracking', 'Portfolio Management', 'Due Diligence'];

  const responsibilities = [
    'Evaluate prototype funding requests',
    'Allocate budgets to promising projects',
    'Analyze investment opportunities',
    'Assess funding risks and returns',
    'Project ROI for prototypes',
    'Track funding milestones'
  ];

  const activities = [
    { time: '5 min ago', text: 'Approved $50K for AI prototype', icon: DollarSign },
    { time: '20 min ago', text: 'Completed ROI analysis for 3 projects', icon: TrendUp },
    { time: '1 hour ago', text: 'Allocated $200K to innovation fund', icon: Wallet },
    { time: '3 hours ago', text: 'Generated quarterly funding report', icon: FileText },
  ];

  const quickActions = [
    { label: 'Fund', icon: DollarSign, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/funding/requests/list', description: 'List requests' },
    { method: 'POST', endpoint: '/api/v1/funding/approve', description: 'Approve funding' },
    { method: 'GET', endpoint: '/api/v1/funding/portfolio', description: 'Get portfolio' },
    { method: 'POST', endpoint: '/api/v1/funding/allocate', description: 'Allocate funds' },
  ];

  const performanceMetrics = [
    { label: 'Total Funded', value: '$2.4M', trend: '+85%' },
    { label: 'Projects', value: '156', trend: '+42%' },
    { label: 'Success Rate', value: '89%', trend: '+22%' },
    { label: 'Avg. ROI', value: '3.2x', trend: '+45%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#22C55E18', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#22C55E25' }]}>
          <Wallet size={56} color="#22C55E" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Prototype Funder</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Prototype Funder evaluates and allocates funding to promising prototype projects, managing investment portfolios and tracking ROI.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-vp-innovation' as any)} style={[styles.parentCard, { backgroundColor: '#22C55E15' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          ...
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/demo-builder' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#3B82F615' }]}>
            <Layers size={18} color="#3B82F6" />
            <Text style={[styles.relatedAgentText, { color: '#3B82F6' }]}>Demo Builder</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="prototype-funder" agentName="AI Prototype Funder" />
