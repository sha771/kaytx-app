...
import { Activity, Lightbulb, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, CheckCircle, Layers, Settings, Compass, Globe, Radar, Search } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function TechnologyScoutPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tech Found', value: '567', icon: Radar, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.3s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '95.4%', icon: Briefcase, color: '#F59E0B' },
  ];

  const capabilities = ['Technology Discovery', 'Trend Monitoring', 'Startup Tracking', 'Patent Analysis', 'Market Intelligence', 'Competitive Scanning', 'Innovation Mapping', 'Opportunity Alerting'];

  const responsibilities = [
    'Scout emerging technologies across industries',
    'Monitor technology trends and breakthroughs',
    'Track innovative startups and companies',
    'Analyze patent filings for new tech',
    'Gather market intelligence on innovations',
    'Alert stakeholders on high-potential opportunities'
  ];

  const activities = [
    { time: '5 min ago', text: 'Discovered 3 quantum computing startups', icon: Radar },
    { time: '20 min ago', text: 'Analyzed 12 new AI patents', icon: Search },
    { time: '1 hour ago', text: 'Mapped emerging blockchain landscape', icon: Globe },
    { time: '3 hours ago', text: 'Alerted team on breakthrough battery tech', icon: Zap },
  ];

  const quickActions = [
    { label: 'Scout', icon: Radar, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/tech/scout/discoveries', description: 'Get discoveries' },
    { method: 'POST', endpoint: '/api/v1/tech/scout/search', description: 'Search technologies' },
    { method: 'GET', endpoint: '/api/v1/tech/scout/trends', description: 'Get trends' },
    { method: 'POST', endpoint: '/api/v1/tech/scout/alert', description: 'Create alert' },
  ];

  const performanceMetrics = [
    { label: 'Tech Discovered', value: '567', trend: '+42%' },
    { label: 'Alerts Sent', value: '234', trend: '+28%' },
    { label: 'Accuracy', value: '95%', trend: '+8%' },
    { label: 'Time Saved', value: '120h', trend: '+35%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#F59E0B18', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#F59E0B25' }]}>
          <Radar size={56} color="#F59E0B" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Technology Scout</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Technology Scout discovers emerging technologies, monitors trends, tracks innovative companies, and alerts stakeholders on high-potential opportunities.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-innovation-analyst' as any)} style={[styles.parentCard, { backgroundColor: '#F59E0B15' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/trend-spotter' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#8B5CF615' }]}>
            <TrendingUp size={18} color="#8B5CF6" />
            <Text style={[styles.relatedAgentText, { color: '#8B5CF6' }]}>Trend Spotter</Text>
          </TouchableOpacity>
      ...
      <AgentFeatures agentId="technology-scout" agentName="AI Technology Scout" />
