...
import { Activity, FlaskConical, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, Lightbulb, CheckCircle, Layers, Settings, DollarSign, FileCheck, PenTool, Wallet } from 'lucide-react-native';
...
export default function GrantProposalWriterPage() {
  ...
  const stats = [
    { label: 'Proposals', value: '234', icon: FileText, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.8s', icon: Clock, color: '#FF9500' },
    { label: 'Success', value: '78%', icon: Briefcase, color: '#22C55E' },
  ];

  const capabilities = ['Grant Writing', 'Budget Planning', 'Compliance Checking', 'Impact Assessment', 'Research Narrative', 'Timeline Creation', 'Partner Coordination', 'Reporting Templates'];

  const responsibilities = [
    'Write compelling grant proposals',
    'Create detailed budget plans',
    'Ensure compliance with grant requirements',
    'Assess project impact and outcomes',
    'Develop research narratives',
    'Create realistic project timelines'
  ];

  const activities = [
    { time: '5 min ago', text: 'Completed NSF grant proposal', icon: FileText },
    { time: '20 min ago', text: 'Generated budget breakdown', icon: DollarSign },
    { time: '1 hour ago', text: 'Checked compliance requirements', icon: FileCheck },
    { time: '3 hours ago', text: 'Created project timeline', icon: Calendar },
  ];

  const quickActions = [
    { label: 'Write', icon: PenTool, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/grant/writer/list', description: 'List proposals' },
    { method: 'POST', endpoint: '/api/v1/grant/writer/create', description: 'Create proposal' },
    { method: 'POST', endpoint: '/api/v1/grant/writer/budget', description: 'Generate budget' },
    { method: 'GET', endpoint: '/api/v1/grant/writer/compliance', description: 'Check compliance' },
  ];

  const performanceMetrics = [
    { label: 'Proposals Written', value: '234', trend: '+28%' },
    { label: 'Success Rate', value: '78%', trend: '+15%' },
    { label: 'Avg Time', value: '2.5h', trend: '-40%' },
    { label: 'Funding Secured', value: '$12.5M', trend: '+35%' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#22C55E18', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Grant Proposal Writer</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Grant Proposal Writer creates compelling grant proposals, detailed budgets, and ensures compliance with funding requirements.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/vp-research' as any)} style={[styles.parentCard, { backgroundColor: '#22C55E15' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/research-agenda-setter' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#3B82F615' }]}>
            <Target size={18} color="#3B82F6" />
            <Text style={[styles.relatedAgentText, { color: '#3B82F6' }]}>Agenda Setter</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/publication-planner' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#8B5CF615' }]}>
            <FileText size={18} color="#8B5CF6" />
            <Text style={[styles.relatedAgentText, { color: '#8B5CF6' }]}>Publication Planner</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="grant-proposal-writer" agentName="AI Grant Proposal Writer" />
