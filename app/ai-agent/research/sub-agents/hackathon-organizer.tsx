...
import { Activity, Lightbulb, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, CheckCircle, Layers, Settings, Trophy, Rocket, Users2, Medal } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function HackathonOrganizerPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Hackathons', value: '156', icon: Trophy, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.6s', icon: Clock, color: '#FF9500' },
    { label: 'Success', value: '94%', icon: Briefcase, color: '#F59E0B' },
  ];

  const capabilities = ['Event Planning', 'Team Formation', 'Schedule Management', 'Prize Coordination', 'Judge Management', 'Submission Tracking', 'Live Updates', 'Winner Selection'];

  const responsibilities = [
    'Plan and organize hackathon events',
    'Facilitate team formation and matching',
    'Manage event schedules and milestones',
    'Coordinate prizes and sponsorships',
    'Organize judging panels and criteria',
    'Track submissions and announce winners'
  ];

  const activities = [
    { time: '5 min ago', text: 'Launched AI Innovation Hackathon 2024', icon: Rocket },
    { time: '20 min ago', text: 'Registered 45 new participants', icon: Users2 },
    { time: '1 hour ago', text: 'Updated judging criteria', icon: Medal },
    { time: '3 hours ago', text: 'Sent sponsor partnership invites', icon: FileText },
  ];

  const quickActions = [
    { label: 'Organize', icon: Trophy, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/hackathon/events/list', description: 'List events' },
    { method: 'POST', endpoint: '/api/v1/hackathon/events/create', description: 'Create event' },
    { method: 'GET', endpoint: '/api/v1/hackathon/participants', description: 'Get participants' },
    { method: 'POST', endpoint: '/api/v1/hackathon/winners', description: 'Announce winners' },
  ];

  const performanceMetrics = [
    { label: 'Events Run', value: '156', trend: '+42%' },
    { label: 'Participants', value: '12.4K', trend: '+65%' },
    { label: 'Projects', value: '3,892', trend: '+58%' },
    { label: 'Satisfaction', value: '96%', trend: '+12%' },
  ];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#F59E0B18', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#F59E0B25' }]}>
          <Trophy size={56} color="#F59E0B" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Hackathon Organizer</Text>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#F59E0B22' }]}><Briefcase size={12} color="#F59E0B" /><Text style={[styles.badgeText, { color: '#F59E0B' }]}>Specialist</Text></View>
        </View>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Hackathon Organizer plans and executes innovation hackathons, manages participants, coordinates judging, and announces winners.
        </Text>
      </View>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#F59E0B12' }]} onPress={() => router.push(action.route as any)}>
              <action.icon size={20} color="#F59E0B" />
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-innovation-manager' as any)} style={[styles.parentCard, { backgroundColor: '#F59E0B15' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/innovation-workshop-facilitator' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#E91E6315' }]}>
            <Users size={18} color="#E91E63" />
            <Text style={[styles.relatedAgentText, { color: '#E91E63' }]}>Workshop Facilitator</Text>
          </TouchableOpacity>
      ...
      <AgentFeatures agentId="hackathon-organizer" agentName="AI Hackathon Organizer" />
