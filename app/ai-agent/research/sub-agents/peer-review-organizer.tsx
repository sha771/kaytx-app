...
import { Activity, BookOpen, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, CheckCircle, Layers, Settings, ClipboardCheck, UserCheck, Star, Send } from 'lucide-react-native';
...
export default function PeerReviewOrganizerPage() {
  ...
  const stats = [
    { label: 'Reviews', value: '1,234', icon: ClipboardCheck, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.4s', icon: Clock, color: '#FF9500' },
    { label: 'Quality', value: '97%', icon: Briefcase, color: '#F97316' },
  ];

  const capabilities = ['Review Coordination', 'Reviewer Matching', 'Quality Assessment', 'Feedback Management', 'Deadline Tracking', 'Conflict Detection', 'Scoring Systems', 'Report Generation'];

  const responsibilities = [
    'Coordinate peer review processes',
    'Match reviewers to submissions',
    'Assess review quality and completeness',
  ...
  const activities = [
    { time: '5 min ago', text: 'Assigned 3 reviewers to paper', icon: UserCheck },
    { time: '20 min ago', text: 'Completed quality check for review', icon: ClipboardCheck },
    { time: '1 hour ago', text: 'Generated review summary report', icon: FileText },
    { time: '3 hours ago', text: 'Sent deadline reminder to reviewers', icon: Send },
  ];

  const quickActions = [
    { label: 'Organize', icon: ClipboardCheck, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/review/list', description: 'List reviews' },
    { method: 'POST', endpoint: '/api/v1/review/assign', description: 'Assign reviewer' },
    { method: 'GET', endpoint: '/api/v1/review/status', description: 'Get status' },
    { method: 'POST', endpoint: '/api/v1/review/complete', description: 'Complete review' },
  ];

  const performanceMetrics = [
    { label: 'Reviews Managed', value: '1,234', trend: '+42%' },
    { label: 'Reviewers', value: '456', trend: '+28%' },
    { label: 'Avg. Turnaround', value: '12d', trend: '-35%' },
    { label: 'Satisfaction', value: '94%', trend: '+18%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#F9731618', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#F9731625' }]}>
          <ClipboardCheck size={56} color="#F97316" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Peer Review Organizer</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Peer Review Organizer manages the entire peer review process, from reviewer matching to feedback collection and quality assessment.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-research-lead' as any)} style={[styles.parentCard, { backgroundColor: '#F9731615' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          ...
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/paper-drafter' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#EC489915' }]}>
            <FileText size={18} color="#EC4899" />
            <Text style={[styles.relatedAgentText, { color: '#EC4899' }]}>Paper Drafter</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="peer-review-organizer" agentName="AI Peer Review Organizer" />
