...
import { Activity, Wrench, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, Lightbulb, CheckCircle, Layers, Settings, Play, Monitor, Video, Presentation } from 'lucide-react-native';
...
export default function DemoBuilderPage() {
  ...
  const stats = [
    { label: 'Demos Built', value: '567', icon: Play, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.6s', icon: Clock, color: '#FF9500' },
    { label: 'Success', value: '98.2%', icon: Briefcase, color: '#6366F1' },
  ];

  const capabilities = ['Demo Creation', 'Interactive Walkthroughs', 'Screen Recording', 'Feature Highlighting', 'Narration Generation', 'Scenario Building', 'Export Options', 'Analytics Integration'];

  const responsibilities = [
    'Build interactive product demonstrations',
    'Create guided feature walkthroughs',
    'Generate automated screen recordings',
    'Highlight key product features',
    'Produce AI-powered narration',
    'Build realistic usage scenarios'
  ];

  const activities = [
    { time: '5 min ago', text: 'Built demo for new analytics feature', icon: Play },
    { time: '20 min ago', text: 'Generated interactive walkthrough', icon: Monitor },
    { time: '1 hour ago', text: 'Created feature highlight video', icon: Video },
    { time: '3 hours ago', text: 'Exported demo to multiple formats', icon: Layers },
  ];

  const quickActions = [
    { label: 'Build Demo', icon: Play, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/demo/builder/list', description: 'List demos' },
    { method: 'POST', endpoint: '/api/v1/demo/builder/create', description: 'Create demo' },
    { method: 'POST', endpoint: '/api/v1/demo/builder/export', description: 'Export demo' },
    { method: 'GET', endpoint: '/api/v1/demo/builder/analytics', description: 'Get analytics' },
  ];

  const performanceMetrics = [
    { label: 'Demos Created', value: '567', trend: '+42%' },
    { label: 'Avg Build Time', value: '3.2m', trend: '-35%' },
    { label: 'User Engagement', value: '87%', trend: '+18%' },
    { label: 'Conversions', value: '34%', trend: '+22%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#6366F118', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#6366F125' }]}>
          <Play size={56} color="#6366F1" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Demo Builder</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Demo Builder creates interactive product demonstrations, guided walkthroughs, and automated screen recordings with AI-powered narration.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-prototype-engineer' as any)} style={[styles.parentCard, { backgroundColor: '#6366F115' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/rapid-prototyper' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#EC489915' }]}>
            <Layers size={18} color="#EC4899" />
            <Text style={[styles.relatedAgentText, { color: '#EC4899' }]}>Rapid Prototyper</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/user-feedback-collector' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#10B98115' }]}>
            <Users size={18} color="#10B981" />
            <Text style={[styles.relatedAgentText, { color: '#10B981' }]}>Feedback Collector</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="demo-builder" agentName="AI Demo Builder" />
