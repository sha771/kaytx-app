...
import { Activity, Scale, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, Lightbulb, CheckCircle, Layers, Settings, FileCheck, PenTool, FileSignature, BookOpen } from 'lucide-react-native';
...
export default function PatentDrafterPage() {
  ...
  const stats = [
    { label: 'Patents', value: '312', icon: FileSignature, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.7s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.5%', icon: Briefcase, color: '#8B5CF6' },
  ];

  const capabilities = ['Patent Drafting', 'Claims Writing', 'Technical Descriptions', 'Prior Art Integration', 'Legal Compliance', 'Figure Generation', 'Examiner Response', 'Continuation Drafts'];

  const responsibilities = [
    'Draft comprehensive patent applications',
    'Write clear and precise patent claims',
    'Create detailed technical descriptions',
    'Integrate prior art references properly',
    'Ensure legal compliance and standards',
    'Generate supporting figures and diagrams'
  ];

  const activities = [
    { time: '5 min ago', text: 'Completed draft for AI algorithm patent', icon: FileSignature },
    { time: '20 min ago', text: 'Generated 15 patent claims', icon: PenTool },
    { time: '1 hour ago', text: 'Integrated prior art references', icon: BookOpen },
    { time: '3 hours ago', text: 'Created technical diagrams', icon: Layers },
  ];

  const quickActions = [
    { label: 'Draft', icon: PenTool, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/patent/drafter/list', description: 'List drafts' },
    { method: 'POST', endpoint: '/api/v1/patent/drafter/create', description: 'Create draft' },
    { method: 'POST', endpoint: '/api/v1/patent/drafter/claims', description: 'Generate claims' },
    { method: 'GET', endpoint: '/api/v1/patent/drafter/review', description: 'Review draft' },
  ];

  const performanceMetrics = [
    { label: 'Patents Drafted', value: '312', trend: '+25%' },
    { label: 'Approval Rate', value: '92%', trend: '+8%' },
    { label: 'Avg Time', value: '4.5h', trend: '-20%' },
    { label: 'Claims/Doc', value: '18', trend: '+12%' },
  ];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#8B5CF618', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#8B5CF625' }]}>
          <FileSignature size={56} color="#8B5CF6" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Patent Drafter</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Patent Researcher</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Patent Drafter creates comprehensive patent applications, writes precise claims, and ensures legal compliance with patent office standards.
        </Text>
      </View>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#8B5CF612' }]} onPress={() => router.push(action.route as any)}>
              <action.icon size={20} color="#8B5CF6" />
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-patent-researcher' as any)} style={[styles.parentCard, { backgroundColor: '#8B5CF615' }]}>
          <Scale size={24} color="#8B5CF6" />
          <View style={styles.parentInfo}>
            <Text style={[styles.parentName, { color: theme.colors.text }]}>AI Patent Researcher</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/prior-art-searcher' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#14B8A615' }]}>
            <FileCheck size={18} color="#14B8A6" />
            <Text style={[styles.relatedAgentText, { color: '#14B8A6' }]}>Prior Art Searcher</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/ip-filing-coordinator' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#F59E0B15' }]}>
            <FileText size={18} color="#F59E0B" />
            <Text style={[styles.relatedAgentText, { color: '#F59E0B' }]}>IP Filing Coordinator</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="patent-drafter" agentName="AI Patent Drafter" />
