...
import { Activity, FileText, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, Users, CheckCircle, Layers, Settings, PenTool, FileEdit, BookOpen, Send } from 'lucide-react-native';
...
export default function PaperDrafterPage() {
  ...
  const stats = [
    { label: 'Papers', value: '456', icon: FileText, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.5s', icon: Clock, color: '#FF9500' },
    { label: 'Quality', value: '94%', icon: Briefcase, color: '#EC4899' },
  ];

  const capabilities = ['Draft Generation', 'Structure Planning', 'Citation Integration', 'Abstract Writing', 'Methodology Description', 'Results Analysis', 'Discussion Drafting', 'Reference Formatting'];

  const responsibilities = [
    'Generate comprehensive paper drafts',
    'Plan document structure and flow',
    'Integrate citations and references',
    'Write compelling abstracts',
    'Describe methodologies clearly',
    'Analyze and present results'
  ];

  const activities = [
    { time: '5 min ago', text: 'Completed draft for ML paper', icon: FileEdit },
    { time: '20 min ago', text: 'Generated abstract for submission', icon: PenTool },
    { time: '1 hour ago', text: 'Formatted 45 references', icon: BookOpen },
    { time: '3 hours ago', text: 'Submitted paper to arXiv', icon: Send },
  ];
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/paper/drafts/list', description: 'List drafts' },
    { method: 'POST', endpoint: '/api/v1/paper/drafts/create', description: 'Create draft' },
    { method: 'GET', endpoint: '/api/v1/paper/sections', description: 'Get sections' },
    { method: 'POST', endpoint: '/api/v1/paper/format', description: 'Format paper' },
  ];

  const performanceMetrics = [
    { label: 'Papers Drafted', value: '456', trend: '+38%' },
    { label: 'Words Generated', value: '2.3M', trend: '+52%' },
    { label: 'Acceptance Rate', value: '78%', trend: '+15%' },
    { label: 'Time Saved', value: '180h', trend: '+65%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#EC489918', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#EC489925' }]}>
          <PenTool size={56} color="#EC4899" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Paper Drafter</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Paper Drafter generates comprehensive academic paper drafts, from abstracts to conclusions, with proper structure and citations.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-research-scientist' as any)} style={[styles.parentCard, { backgroundColor: '#EC489915' }]}>
      ...
      <AgentFeatures agentId="paper-drafter" agentName="AI Paper Drafter" />
