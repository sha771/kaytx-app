...
import { Activity, Scale, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, Lightbulb, CheckCircle, Layers, Settings, Search, Database, Globe, BookOpen } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function PriorArtSearcherPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Searches', value: '2,847', icon: Search, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.4s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '96.2%', icon: Briefcase, color: '#14B8A6' },
  ];

  const capabilities = ['Prior Art Search', 'Patent Database Mining', 'Literature Analysis', 'Citation Tracking', 'Similarity Detection', 'Global Patent Search', 'Non-Patent Literature', 'Relevance Ranking'];

  const responsibilities = [
    'Search global patent databases for prior art',
    'Analyze scientific literature and publications',
    'Track citations and related patents',
    'Detect similar inventions and technologies',
    'Rank results by relevance to invention',
    'Document comprehensive prior art reports'
  ];

  const activities = [
    { time: '5 min ago', text: 'Completed prior art search for AI patent', icon: Search },
    { time: '20 min ago', text: 'Analyzed 45 related patents', icon: Database },
    { time: '1 hour ago', text: 'Found 12 relevant non-patent literature', icon: BookOpen },
    { time: '3 hours ago', text: 'Generated prior art report', icon: FileText },
  ];
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/priorart/searcher/results', description: 'Get search results' },
    { method: 'POST', endpoint: '/api/v1/priorart/searcher/query', description: 'Execute search' },
    { method: 'GET', endpoint: '/api/v1/priorart/searcher/report', description: 'Get report' },
    { method: 'POST', endpoint: '/api/v1/priorart/searcher/analyze', description: 'Analyze results' },
  ];

  const performanceMetrics = [
    { label: 'Searches Done', value: '2,847', trend: '+32%' },
    { label: 'Avg Time', value: '1.2m', trend: '-40%' },
    { label: 'Relevance', value: '94%', trend: '+10%' },
    { label: 'Coverage', value: '99%', trend: '+5%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#14B8A618', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#14B8A625' }]}>
          <Search size={56} color="#14B8A6" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Prior Art Searcher</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Prior Art Searcher conducts comprehensive searches across global patent databases and scientific literature to identify relevant prior art.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-patent-researcher' as any)} style={[styles.parentCard, { backgroundColor: '#14B8A615' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/patent-drafter' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#8B5CF615' }]}>
            <FileText size={18} color="#8B5CF6" />
            <Text style={[styles.relatedAgentText, { color: '#8B5CF6' }]}>Patent Drafter</Text>
          </TouchableOpacity>
      ...
      <AgentFeatures agentId="prior-art-searcher" agentName="AI Prior Art Searcher" />
