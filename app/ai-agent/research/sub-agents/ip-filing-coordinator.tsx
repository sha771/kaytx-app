...
import { Activity, Lightbulb, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, CheckCircle, Layers, Settings, FileCheck, ShieldCheck, Globe, Send } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function IpFilingCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Filings', value: '567', icon: FileCheck, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.4s', icon: Clock, color: '#FF9500' },
    { label: 'Success', value: '98%', icon: Briefcase, color: '#14B8A6' },
  ];

  const capabilities = ['IP Filing Coordination', 'Document Preparation', 'Jurisdiction Management', 'Timeline Tracking', 'Compliance Verification', 'Status Monitoring', 'Renewal Management', 'Portfolio Organization'];

  const responsibilities = [
    'Coordinate IP filing processes',
    'Prepare and review filing documents',
    'Manage multi-jurisdiction filings',
    'Track filing timelines and deadlines',
    'Verify compliance requirements',
    'Monitor filing statuses and updates'
  ];

  const activities = [
    { time: '5 min ago', text: 'Coordinated patent filing in 3 countries', icon: Globe },
    { time: '20 min ago', text: 'Verified compliance for trademark filing', icon: ShieldCheck },
    { time: '1 hour ago', text: 'Submitted IP application to USPTO', icon: Send },
    { time: '3 hours ago', text: 'Updated portfolio status report', icon: FileText },
  ];

  const quickActions = [
    { label: 'Coordinate', icon: FileCheck, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/ip/filing/list', description: 'List filings' },
    { method: 'POST', endpoint: '/api/v1/ip/filing/submit', description: 'Submit filing' },
    { method: 'GET', endpoint: '/api/v1/ip/filing/status', description: 'Get status' },
    { method: 'POST', endpoint: '/api/v1/ip/filing/verify', description: 'Verify compliance' },
  ];

  const performanceMetrics = [
    { label: 'Filings Coordinated', value: '567', trend: '+35%' },
    { label: 'Success Rate', value: '98%', trend: '+8%' },
    { label: 'Countries', value: '45', trend: '+22%' },
    { label: 'Time Saved', value: '120h', trend: '+55%' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#14B8A618', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI IP Filing Coordinator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI IP Filing Coordinator manages intellectual property filing processes across jurisdictions, ensuring compliance and timely submissions.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-innovation-manager' as any)} style={[styles.parentCard, { backgroundColor: '#14B8A615' }]}>
          <Lightbulb size={24} color="#14B8A6" />
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/patent-drafter' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#3B82F615' }]}>
            <FileText size={18} color="#3B82F6" />
            <Text style={[styles.relatedAgentText, { color: '#3B82F6' }]}>Patent Drafter</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/filing-tracker' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#8B5CF615' }]}>
            <FileCheck size={18} color="#8B5CF6" />
            <Text style={[styles.relatedAgentText, { color: '#8B5CF6' }]}>Filing Tracker</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="ip-filing-coordinator" agentName="AI IP Filing Coordinator" />
