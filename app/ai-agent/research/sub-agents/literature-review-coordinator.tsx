...
import { Activity, BookOpen, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, CheckCircle, Layers, Settings, Search, Library, FileSearch, BookmarkPlus } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function LiteratureReviewCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Reviews', value: '892', icon: BookOpen, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.3s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '96%', icon: Briefcase, color: '#6366F1' },
  ];

  const capabilities = ['Literature Search', 'Citation Management', 'Meta-Analysis', 'Gap Identification', 'Trend Analysis', 'Reference Organization', 'Abstract Summarization', 'Quality Assessment'];

  const responsibilities = [
    'Coordinate comprehensive literature reviews',
    'Manage citation databases and references',
    'Identify research gaps and opportunities',
    'Perform meta-analysis of findings',
    'Organize and categorize publications',
    'Summarize and synthesize research'
  ];

  const activities = [
    { time: '5 min ago', text: 'Completed review of 45 AI papers', icon: BookOpen },
    { time: '20 min ago', text: 'Added 12 new citations to database', icon: BookmarkPlus },
    { time: '1 hour ago', text: 'Identified 3 research gaps', icon: Search },
    { time: '3 hours ago', text: 'Generated meta-analysis report', icon: FileText },
  ];

  const quickActions = [
    { label: 'Review', icon: BookOpen, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/literature/reviews/list', description: 'List reviews' },
    { method: 'POST', endpoint: '/api/v1/literature/reviews/create', description: 'Create review' },
    { method: 'GET', endpoint: '/api/v1/literature/citations', description: 'Get citations' },
    { method: 'POST', endpoint: '/api/v1/literature/analyze', description: 'Analyze literature' },
  ];

  const performanceMetrics = [
    { label: 'Reviews Completed', value: '892', trend: '+45%' },
    { label: 'Papers Analyzed', value: '23.5K', trend: '+62%' },
    { label: 'Citations Managed', value: '156K', trend: '+38%' },
    { label: 'Time Saved', value: '240h', trend: '+72%' },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#6366F118', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#6366F125' }]}>
          <Library size={56} color="#6366F1" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Literature Review Coordinator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Literature Review Coordinator manages comprehensive literature reviews, citation databases, and research synthesis across multiple domains.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-research-lead' as any)} style={[styles.parentCard, { backgroundColor: '#6366F115' }]}>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/paper-drafter' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#10B98115' }]}>
            <FileText size={18} color="#10B981" />
            <Text style={[styles.relatedAgentText, { color: '#10B981' }]}>Paper Drafter</Text>
          </TouchableOpacity>
      ...
      <AgentFeatures agentId="literature-review-coordinator" agentName="AI Literature Review Coordinator" />
