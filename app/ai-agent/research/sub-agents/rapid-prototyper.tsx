...
import { Activity, Wrench, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, BarChart3, MessageSquare, Calendar, Shield, FileText, Users, Lightbulb, CheckCircle, Layers, Settings, Rocket, Cpu, Hammer, Cog } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function RapidPrototyperPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Prototypes', value: '423', icon: Rocket, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.5s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '97.8%', icon: Briefcase, color: '#EC4899' },
  ];

  const capabilities = ['Rapid Prototyping', 'MVP Development', 'Code Generation', 'UI Mockups', 'Feature Testing', 'Iteration Cycles', 'User Feedback Integration', 'Technical Validation'];

  const responsibilities = [
    'Create rapid prototypes from concepts',
    'Develop minimum viable products (MVPs)',
    'Generate code and UI mockups quickly',
  ...
  const activities = [
    { time: '5 min ago', text: 'Deployed MVP for payment feature', icon: Rocket },
    { time: '20 min ago', text: 'Generated UI mockup for dashboard', icon: Layers },
    { time: '1 hour ago', text: 'Completed iteration cycle #12', icon: Cog },
    { time: '3 hours ago', text: 'Validated technical feasibility', icon: CheckCircle },
  ];

  const quickActions = [
    { label: 'Prototype', icon: Rocket, route: '/ai-agent/research' },
  ...
  const a2aEndpoints = [
    { method: 'GET', endpoint: '/api/v1/prototype/rapid/list', description: 'List prototypes' },
    { method: 'POST', endpoint: '/api/v1/prototype/rapid/create', description: 'Create prototype' },
    { method: 'POST', endpoint: '/api/v1/prototype/rapid/iterate', description: 'Iterate design' },
    { method: 'GET', endpoint: '/api/v1/prototype/rapid/validate', description: 'Validate tech' },
  ];

  const performanceMetrics = [
    { label: 'Prototypes Built', value: '423', trend: '+38%' },
    { label: 'Avg Build Time', value: '2.1h', trend: '-25%' },
    { label: 'User Approved', value: '89%', trend: '+12%' },
    { label: 'Iterations', value: '1.8K', trend: '+45%' },
  ];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#EC489918', borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#EC489925' }]}>
          <Rocket size={56} color="#EC4899" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Rapid Prototyper</Text>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#EC489922' }]}><Briefcase size={12} color="#EC4899" /><Text style={[styles.badgeText, { color: '#EC4899' }]}>Specialist</Text></View>
        </View>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Rapid Prototyper creates quick prototypes, develops MVPs, generates code and UI mockups, and iterates rapidly based on user feedback.
        </Text>
      </View>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#EC489912' }]} onPress={() => router.push(action.route as any)}>
              <action.icon size={20} color="#EC4899" />
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <TouchableOpacity onPress={() => router.push('/ai-agent/research/ai-prototype-engineer' as any)} style={[styles.parentCard, { backgroundColor: '#EC489915' }]}>
          <Wrench size={24} color="#EC4899" />
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.relatedAgentsRow}>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/demo-builder' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#8B5CF615' }]}>
            <Layers size={18} color="#8B5CF6" />
            <Text style={[styles.relatedAgentText, { color: '#8B5CF6' }]}>Demo Builder</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => router.push('/ai-agent/research/sub-agents/feasibility-assessor' as any)} style={[styles.relatedAgentCard, { backgroundColor: '#10B98115' }]}>
            <Target size={18} color="#10B981" />
            <Text style={[styles.relatedAgentText, { color: '#10B981' }]}>Feasibility Assessor</Text>
          </TouchableOpacity>
        </View>
      </View>

      <AgentFeatures agentId="rapid-prototyper" agentName="AI Rapid Prototyper" />
