...
import { Microscope, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'3,765',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.5%',icon:Target,color:'#006064'}];
  const capabilities = ['Research Design','Algorithm Development','Paper Writing','Peer Review','Experimentation','Collaboration'];
  const responsibilities = ['Research design & hypothesis formulation','Novel algorithm development','Research paper writing & publication','Peer review & academic collaboration','Experiment design & execution','Cross-team research collaboration'];
  const activities = [{time:'3 min ago',text:'Submitted paper to NeurIPS',icon: CircleCheckBig},{time:'6 min ago',text:'Developed novel attention mechanism',icon:Clock},{time:'9 min ago',text:'Reviewed 4 research papers',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>AI Research</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#00606422' }]}><Star size={12} color="#006064" /><Text style={[styles.badgeText, { color: '#006064' }]}>Scientist</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Research Scientist AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Research & Development department.</Text></View>
