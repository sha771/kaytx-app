...
export default function ValueEstimatorPage() {
  ...
  const stats = [{label:'Tasks',value:'1,789',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.9s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.5%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Property Valuation','Income Approach','Sales Comparison','Cost Approach','DCF Modeling','Value Forecasting'];
  const responsibilities = ['Property valuation & estimation','Income approach valuation & analysis','Sales comparison approach valuation','Cost approach valuation & depreciation','Discounted cash flow modeling','Property value forecasting & projection'];
  const activities = [{time:'3 min ago',text:'Valued 30 properties totaling $500M',icon:CircleCheckBig},{time:'15 min ago',text:'Built DCF models for 8 investment properties',icon:Clock},{time:'30 min ago',text:'Forecasted 5-year value appreciation trends',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Calculator size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Value Estimator</Text>
      ...
      <AgentFeatures agentId="value-estimator" agentName="AI Value Estimator" />
