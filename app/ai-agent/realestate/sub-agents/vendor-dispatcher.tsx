...
import { Truck, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function VendorDispatcherPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'2,345',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.6%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Vendor Dispatch','Contractor Matching','Availability Tracking','Performance Rating','Cost Management','Scheduling'];
  const responsibilities = ['Vendor & contractor dispatch management','Contractor skill matching & selection','Vendor availability tracking & scheduling','Vendor performance rating & tracking','Cost management & quote comparison','Service scheduling & coordination'];
  const activities = [{time:'3 min ago',text:'Dispatched 80 vendor calls this week',icon:CircleCheckBig},{time:'15 min ago',text:'Maintained 95% vendor response rate',icon:Clock},{time:'30 min ago',text:'Compared quotes from 15 vendors for major project',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Truck size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Vendor Dispatcher</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Vendor Dispatcher provides specialized capabilities supporting the AI Maintenance Coordinator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
