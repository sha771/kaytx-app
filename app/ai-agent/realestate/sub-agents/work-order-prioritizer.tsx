...
import { ListOrdered, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function WorkOrderPrioritizerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'4,567',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.1%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Priority Assessment','SLA Compliance','Urgency Classification','Resource Matching','Queue Management','Workload Balancing'];
  const responsibilities = ['Work order priority assessment & ranking','SLA compliance tracking & management','Urgency classification & routing','Resource matching & assignment optimization','Work order queue management','Technician workload balancing'];
  const activities = [{time:'3 min ago',text:'Prioritized 300 work orders by urgency',icon:CircleCheckBig},{time:'15 min ago',text:'Achieved 97% SLA compliance rate',icon:Clock},{time:'30 min ago',text:'Balanced workload across 12 technicians',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><ListOrdered size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Work Order Prioritizer</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Maintenance Coordinator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Work Order Prioritizer provides specialized capabilities supporting the AI Maintenance Coordinator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/realestate/maintenance-coordinator')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><ListOrdered size={24} color="#33691E" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Maintenance Coordinator</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent</Text></View><ArrowRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="work-order-prioritizer" agentName="AI Work Order Prioritizer" />
