...
export default function LeaseNegotiatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'1,567',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.0s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.6%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Lease Negotiation','Term Optimization','Concession Management','Renewal Negotiation','Counter-Offer Analysis','Deal Structuring'];
  const responsibilities = ['Lease negotiation & deal structuring','Term optimization & rent escalation','Concession management & analysis','Lease renewal negotiation & strategy','Counter-offer analysis & response','Deal structuring & documentation'];
  const activities = [{time:'3 min ago',text:'Negotiated 15 new leases this month',icon:CircleCheckBig},{time:'15 min ago',text:'Achieved 3% above asking on 8 deals',icon:Clock},{time:'30 min ago',text:'Structured 5 renewal deals with favorable terms',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Handshake size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Lease Negotiator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Lease Negotiator provides specialized capabilities supporting the AI Leasing Manager within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
