...
import { ClipboardCheck, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function SatisfactionSurveyorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'1,678',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.8s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.4%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Survey Design','Satisfaction Measurement','NPS Tracking','Sentiment Analysis','Action Planning','Benchmarking'];
  const responsibilities = ['Tenant satisfaction survey design & execution','Satisfaction measurement & scoring','Net Promoter Score tracking & analysis','Sentiment analysis & trend identification','Action planning based on survey results','Satisfaction benchmarking across portfolio'];
  const activities = [{time:'3 min ago',text:'Achieved 85% survey response rate this quarter',icon:CircleCheckBig},{time:'15 min ago',text:'Measured NPS improvement from 32 to 48',icon:Clock},{time:'30 min ago',text:'Designed targeted survey for 5 properties',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><ClipboardCheck size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Satisfaction Surveyor</Text>
