...
import { Calendar, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function CriticalDateTrackerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'3,567',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.6%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Date Tracking','Expiration Management','Renewal Deadlines','Option Tracking','Automated Alerts','Timeline Management'];
  const responsibilities = ['Critical date tracking & management','Lease expiration management & notification','Renewal deadline tracking & alerts','Option exercise date tracking','Automated alert system & notifications','Timeline management & milestone tracking'];
  const activities = [{time:'3 min ago',text:'Tracked 500 critical dates across portfolio',icon:CircleCheckBig},{time:'15 min ago',text:'Sent 45 renewal notifications this month',icon:Clock},{time:'30 min ago',text:'Managed 30 option exercise deadlines',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Calendar size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Critical Date Tracker</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Lease Administrator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Critical Date Tracker provides specialized capabilities supporting the AI Lease Administrator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/realestate/lease-administrator')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><Calendar size={24} color="#33691E" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Lease Administrator</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent</Text></View><ArrowRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="critical-date-tracker" agentName="AI Critical Date Tracker" />
