...
import { MessageSquare, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function CommunicationCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'3,456',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.2%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Communication Management','Notification Systems','Channel Coordination','Response Tracking','Broadcast Messaging','Feedback Collection'];
  const responsibilities = ['Tenant communication coordination & management','Notification system management','Multi-channel communication coordination','Response tracking & follow-up','Broadcast messaging & announcements','Tenant feedback collection & analysis'];
  const activities = [{time:'3 min ago',text:'Coordinated 500 tenant communications this month',icon:CircleCheckBig},{time:'15 min ago',text:'Managed 3 property-wide announcements',icon:Clock},{time:'30 min ago',text:'Collected feedback from 200 tenants on services',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><MessageSquare size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Communication Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Tenant Relations Specialist</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Communication Coordinator provides specialized capabilities supporting the AI Tenant Relations Specialist within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/realestate/tenant-relations')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><MessageSquare size={24} color="#33691E" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Tenant Relations Specialist</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent</Text></View><ArrowRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="communication-coordinator" agentName="AI Communication Coordinator" />
