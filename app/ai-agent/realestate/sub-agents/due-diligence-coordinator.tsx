...
export default function DueDiligenceCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'1,234',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.4%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Due Diligence','Checklist Management','Team Coordination','Document Review','Risk Identification','Timeline Management'];
  const responsibilities = ['Due diligence process coordination & management','Checklist management & tracking','Due diligence team coordination','Document review & organization','Risk identification & assessment','Due diligence timeline management'];
  const activities = [{time:'3 min ago',text:'Coordinated due diligence for 5 acquisitions',icon:CircleCheckBig},{time:'15 min ago',text:'Managed 200-item checklist for $50M deal',icon:Clock},{time:'30 min ago',text:'Identified 3 material risks during DD review',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Search size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Due Diligence Coordinator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Due Diligence Coordinator provides specialized capabilities supporting the AI Acquisition Analyst within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
