...
import { Filter, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function DealScreenerPage() {
  ...
  const stats = [{label:'Tasks',value:'3,456',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.2%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Deal Screening','Criteria Matching','Quick Assessment','Pipeline Filtering','Market Scanning','Opportunity Identification'];
  const responsibilities = ['Acquisition deal screening & filtering','Investment criteria matching & scoring','Quick deal assessment & triage','Pipeline filtering & prioritization','Market scanning & opportunity identification','Deal flow management & tracking'];
  const activities = [{time:'3 min ago',text:'Screened 50 acquisition opportunities this month',icon:CircleCheckBig},{time:'15 min ago',text:'Advanced 8 deals to due diligence phase',icon:Clock},{time:'30 min ago',text:'Filtered out 42 deals not meeting criteria',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Filter size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Deal Screener</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Deal Screener provides specialized capabilities supporting the AI Acquisition Analyst within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
