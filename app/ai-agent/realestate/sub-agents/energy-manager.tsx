...
export default function EnergyManagerPage() {
  ...
  const stats = [{label:'Tasks',value:'2,345',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.5%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Energy Optimization','Consumption Tracking','Sustainability','Cost Reduction','Carbon Reporting','Green Certifications'];
  const responsibilities = ['Energy consumption tracking & optimization','Utility cost reduction & management','Sustainability program management','Carbon footprint reporting & reduction','Green building certification support','Energy efficiency improvement programs'];
  const activities = [{time:'3 min ago',text:'Reduced energy costs by 15% across portfolio',icon:CircleCheckBig},{time:'15 min ago',text:'Tracked consumption for 12 buildings',icon:Clock},{time:'30 min ago',text:'Prepared LEED certification documentation',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Zap size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Energy Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Energy Manager provides specialized capabilities supporting the AI Facilities Manager within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
