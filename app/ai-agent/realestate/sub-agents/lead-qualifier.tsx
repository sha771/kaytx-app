...
import { UserSearch, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function LeadQualifierPage() {
  ...
  const stats = [{label:'Tasks',value:'3,890',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.0%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Lead Qualification','Intent Scoring','Contact Management','Pipeline Tracking','Response Routing','CRM Integration'];
  const responsibilities = ['Lead qualification & scoring','Intent scoring & behavioral analysis','Contact management & organization','Lead pipeline tracking & management','Response routing & assignment','CRM integration & data synchronization'];
  const activities = [{time:'3 min ago',text:'Qualified 500 leads this month with 35% conversion',icon:CircleCheckBig},{time:'15 min ago',text:'Scored leads using behavioral intent model',icon:Clock},{time:'30 min ago',text:'Routed 150 hot leads to leasing agents',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><UserSearch size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Lead Qualifier</Text>
