...
import { Percent, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function ReturnCalculatorPage() {
  ...
  const stats = [{label:'Tasks',value:'2,123',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.1%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Return Calculation','IRR Modeling','Equity Multiple','Cash-on-Cash','ROI Analysis','Performance Attribution'];
  const responsibilities = ['Investment return calculation & modeling','Internal rate of return (IRR) modeling','Equity multiple calculation & projection','Cash-on-cash return analysis','ROI analysis & comparison','Performance attribution & decomposition'];
  const activities = [{time:'3 min ago',text:'Calculated IRR for 15 assets averaging 18%',icon:CircleCheckBig},{time:'15 min ago',text:'Modeled equity multiples for 5-year hold periods',icon:Clock},{time:'30 min ago',text:'Analyzed performance attribution across portfolio',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Percent size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Return Calculator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Return Calculator provides specialized capabilities supporting the AI Asset Manager within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
