...
import { Wallet, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function BudgetTrackerPage() {
  ...
  const stats = [{label:'Tasks',value:'2,345',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.9%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Budget Tracking','Cost Control','Variance Analysis','Forecasting','Draw Management','Financial Reporting'];
  const responsibilities = ['Development budget tracking & management','Cost control & change order management','Budget variance analysis & reporting','Cost forecasting & projection','Draw management & lender reporting','Financial reporting & documentation'];
  const activities = [{time:'3 min ago',text:'Tracked $200M in development budgets',icon:CircleCheckBig},{time:'15 min ago',text:'Identified $5M in cost overruns early',icon:Clock},{time:'30 min ago',text:'Managed 30 draw requests across 5 projects',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Wallet size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Budget Tracker</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Budget Tracker provides specialized capabilities supporting the AI Development Coordinator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
