...
import { FileSpreadsheet, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function UnderwritingAssistantPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'2,567',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.9s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.8%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Underwriting Support','Financial Analysis','Deal Structuring','Lender Coordination','Compliance Review','Documentation'];
  const responsibilities = ['Underwriting support & analysis','Financial analysis & modeling','Deal structuring & terms negotiation','Lender coordination & package preparation','Compliance review & verification','Underwriting documentation management'];
  const activities = [{time:'3 min ago',text:'Supported underwriting for 8 deals totaling $200M',icon:CircleCheckBig},{time:'15 min ago',text:'Prepared lender packages for 3 acquisitions',icon:Clock},{time:'30 min ago',text:'Verified compliance for 15 loan applications',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><FileSpreadsheet size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Underwriting Assistant</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Acquisition Analyst</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Underwriting Assistant provides specialized capabilities supporting the AI Acquisition Analyst within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/realestate/acquisition-analyst')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><FileSpreadsheet size={24} color="#33691E" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Acquisition Analyst</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent</Text></View><ArrowRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="underwriting-assistant" agentName="AI Underwriting Assistant" />
