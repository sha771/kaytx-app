...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function AssetPerformanceTrackerPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'3,789',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.4s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.0%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Performance Tracking','KPI Monitoring','Benchmarking','Variance Reporting','Trend Analysis','Alert Management'];
  const responsibilities = ['Asset performance tracking & monitoring','KPI monitoring & threshold management','Performance benchmarking & comparison','Variance reporting & analysis','Performance trend analysis & projection','Automated alert management & notification'];
  const activities = [{time:'3 min ago',text:'Tracked performance for 25 assets in portfolio',icon:CircleCheckBig},{time:'15 min ago',text:'Generated monthly variance reports for board',icon:Clock},{time:'30 min ago',text:'Alerted on 3 assets below target returns',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Activity size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Asset Performance Tracker</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Asset Performance Tracker provides specialized capabilities supporting the AI Asset Manager within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <AgentFeatures agentId="asset-performance-tracker" agentName="AI Asset Performance Tracker" />
