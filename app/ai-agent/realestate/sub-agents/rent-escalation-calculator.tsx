...
export default function RentEscalationCalculatorPage() {
  ...
  const stats = [{label:'Tasks',value:'2,890',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.4s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.3%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Rent Escalation','CPI Adjustments','Fixed Increase','Percentage Rent','Step-Up Calculations','Escalation Modeling'];
  const responsibilities = ['Rent escalation calculation & management','CPI-based adjustment calculations','Fixed increase schedule management','Percentage rent calculation & reconciliation','Step-up rent calculation & tracking','Escalation modeling & projection'];
  const activities = [{time:'3 min ago',text:'Calculated CPI adjustments for 200 leases',icon:CircleCheckBig},{time:'15 min ago',text:'Projected 5-year rent escalation schedules',icon:Clock},{time:'30 min ago',text:'Reconciled percentage rent for 25 retail leases',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Calculator size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Rent Escalation Calculator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Rent Escalation Calculator provides specialized capabilities supporting the AI Lease Administrator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <AgentFeatures agentId="rent-escalation-calculator" agentName="AI Rent Escalation Calculator" />
