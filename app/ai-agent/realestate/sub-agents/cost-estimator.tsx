...
import { Coins, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function CostEstimatorPage() {
  ...
  const stats = [{label:'Tasks',value:'1,890',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.7s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.9%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Cost Estimation','Budget Forecasting','Quote Analysis','Material Pricing','Labor Costing','Variance Tracking'];
  const responsibilities = ['Maintenance cost estimation & quoting','Budget forecasting & planning','Quote analysis & comparison','Material pricing & tracking','Labor costing & rate management','Cost variance tracking & reporting'];
  const activities = [{time:'3 min ago',text:'Estimated costs for 200 maintenance projects',icon:CircleCheckBig},{time:'15 min ago',text:'Forecasted Q3 maintenance budget at $1.2M',icon:Clock},{time:'30 min ago',text:'Tracked 5% cost variance from estimates',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Coins size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Cost Estimator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Cost Estimator provides specialized capabilities supporting the AI Maintenance Coordinator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
