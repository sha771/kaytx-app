...
import { LineChart, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function MarketTrendReporterPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'1,234',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'96.8%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Trend Analysis','Market Reporting','Data Visualization','Forecasting','Demographic Analysis','Supply/Demand'];
  const responsibilities = ['Market trend analysis & identification','Market reporting & publication','Data visualization & dashboard creation','Market forecasting & projection','Demographic analysis & impact assessment','Supply & demand analysis & balance reporting'];
  const activities = [{time:'3 min ago',text:'Published monthly market trend report',icon:CircleCheckBig},{time:'15 min ago',text:'Analyzed demographic shifts in 3 submarkets',icon:Clock},{time:'30 min ago',text:'Created interactive market dashboard',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><LineChart size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Market Trend Reporter</Text>
