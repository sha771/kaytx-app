...
import { Clock, Activity, Star, CircleCheckBig, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function TimelineManagerPage() {
  ...
  const stats = [{label:'Tasks',value:'2,890',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.7%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Timeline Management','Milestone Tracking','Gantt Charting','Critical Path','Delay Analysis','Schedule Optimization'];
  const responsibilities = ['Development timeline management & tracking','Milestone tracking & reporting','Gantt chart management & visualization','Critical path analysis & management','Delay analysis & impact assessment','Schedule optimization & acceleration'];
  const activities = [{time:'3 min ago',text:'Managed timelines for 8 development projects',icon:CircleCheckBig},{time:'15 min ago',text:'Identified critical path delay saving 3 weeks',icon:Clock},{time:'30 min ago',text:'Updated Gantt charts across $400M pipeline',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Clock size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Timeline Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Timeline Manager provides specialized capabilities supporting the AI Development Coordinator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
