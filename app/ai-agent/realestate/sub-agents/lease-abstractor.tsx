...
export default function LeaseAbstractorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'4,123',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.5%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Lease Abstraction','Data Extraction','Key Term Summary','Clause Analysis','Database Management','Document Processing'];
  const responsibilities = ['Lease abstraction & data extraction','Key term summary & documentation','Clause analysis & categorization','Lease database management & maintenance','Document processing & OCR integration','Abstract quality assurance & verification'];
  const activities = [{time:'3 min ago',text:'Abstracted 75 new leases this month',icon:CircleCheckBig},{time:'15 min ago',text:'Extracted key terms from 200-page master lease',icon:Clock},{time:'30 min ago',text:'Maintained database of 1,500 lease abstracts',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><FileText size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Lease Abstractor</Text>
