...
export default function TenantQualifierPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'3,234',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.4s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.0%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Tenant Screening','Credit Analysis','Background Verification','Income Validation','Risk Scoring','Qualification Criteria'];
  const responsibilities = ['Tenant screening & qualification','Credit analysis & scoring','Background verification & reference checks','Income validation & employment verification','Risk scoring & assessment','Qualification criteria management'];
  const activities = [{time:'3 min ago',text:'Screened 150 tenant applications this month',icon:CircleCheckBig},{time:'15 min ago',text:'Approved 85% of applicants meeting criteria',icon:Clock},{time:'30 min ago',text:'Flagged 12 high-risk applications for review',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><UserCheck size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Tenant Qualifier</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Leasing Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Tenant Qualifier provides specialized capabilities supporting the AI Leasing Manager within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/realestate/leasing-manager')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><UserCheck size={24} color="#33691E" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Leasing Manager</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent</Text></View><ArrowRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="tenant-qualifier" agentName="AI Tenant Qualifier" />
