...
import { Video, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
...
export default function VirtualTourBuilderPage() {
  ...
  const stats = [{label:'Tasks',value:'1,234',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'96.2%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Virtual Tours','3D Modeling','Interactive Floor Plans','Video Production','VR Integration','Tour Analytics'];
  const responsibilities = ['Virtual tour creation & management','3D model generation & integration','Interactive floor plan development','Video tour production & editing','VR experience integration','Tour analytics & engagement tracking'];
  const activities = [{time:'3 min ago',text:'Built virtual tours for 20 properties',icon:CircleCheckBig},{time:'15 min ago',text:'Generated 3D models from floor plans',icon:Clock},{time:'30 min ago',text:'Tracked 5,000 tour views with 12% conversion',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Video size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Virtual Tour Builder</Text>
