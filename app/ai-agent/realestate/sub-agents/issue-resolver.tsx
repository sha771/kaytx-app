...
export default function IssueResolverPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'5,234',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.3s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.7%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Issue Resolution','Ticket Management','Escalation Handling','Root Cause Analysis','Resolution Tracking','Prevention Planning'];
  const responsibilities = ['Tenant issue resolution & management','Ticket management & prioritization','Escalation handling & routing','Root cause analysis & prevention','Resolution tracking & follow-up','Issue prevention planning & programs'];
  const activities = [{time:'3 min ago',text:'Resolved 150 tenant issues this week',icon:CircleCheckBig},{time:'15 min ago',text:'Reduced average resolution time from 48h to 12h',icon:Clock},{time:'30 min ago',text:'Identified root cause for recurring HVAC complaints',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Wrench size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Issue Resolver</Text>
