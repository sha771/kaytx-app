...
export default function BuildingSystemsMonitorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'5,678',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.7%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Systems Monitoring','HVAC Control','Electrical Monitoring','Plumbing Tracking','Fire Safety','IoT Integration'];
  const responsibilities = ['Building systems monitoring & control','HVAC system monitoring & optimization','Electrical system monitoring & load management','Plumbing system tracking & leak detection','Fire safety system monitoring & compliance','IoT sensor integration & data collection'];
  const activities = [{time:'3 min ago',text:'Monitored 500+ building sensors in real-time',icon:CircleCheckBig},{time:'15 min ago',text:'Detected HVAC anomaly 2 hours before failure',icon:Clock},{time:'30 min ago',text:'Maintained 99.9% building systems uptime',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><Cpu size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Building Systems Monitor</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Building Systems Monitor provides specialized capabilities supporting the AI Facilities Manager within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
