...
import { HardHat, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function ContractorCoordinatorPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const stats = [{label:'Tasks',value:'1,567',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.8s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.5%',icon:Target,color:'#558B2F'}];
  const capabilities = ['Contractor Management','Bid Coordination','Performance Tracking','Payment Management','Compliance Verification','Scheduling'];
  const responsibilities = ['General contractor coordination & management','Bid coordination & comparison','Contractor performance tracking & rating','Payment management & approval workflow','Compliance verification & insurance tracking','Contractor scheduling & resource planning'];
  const activities = [{time:'3 min ago',text:'Coordinated 12 contractors across 5 projects',icon:CircleCheckBig},{time:'15 min ago',text:'Managed bid process for $50M construction contract',icon:Clock},{time:'30 min ago',text:'Verified compliance for 8 contractor renewals',icon:Zap}];

  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#558B2F20' }]}><HardHat size={56} color="#558B2F" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Contractor Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Development Coordinator</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Contractor Coordinator provides specialized capabilities supporting the AI Development Coordinator within the Real Estate division, delivering automated analysis, tracking, and optimization.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/realestate/development-coordinator')} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><HardHat size={24} color="#33691E" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Development Coordinator</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Main Agent</Text></View><ArrowRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
      <AgentFeatures agentId="contractor-coordinator" agentName="AI Contractor Coordinator" />
