...
import { Building2, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'3,966',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.0s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.6%',icon:Target,color:'#33691E'}];
  const capabilities = ['Development Strategy','Pipeline Mgmt','Capital Deployment','Risk Assessment','Stakeholder Mgmt','Project Oversight'];
  const responsibilities = ['Real estate development strategy','Development pipeline management','Capital deployment & investment decisions','Development risk assessment','Stakeholder & community relations','Major project oversight & governance'];
  const activities = [{time:'3 min ago',text:'Reviewed development pipeline of $500M',icon: CircleCheckBig},{time:'6 min ago',text:'Approved 2 new development projects',icon:Clock},{time:'9 min ago',text:'Managed stakeholder communications',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#33691E20' }]}><Building2 size={48} color="#33691E" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Real Estate Development</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Development Division</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Real Estate Development AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Real Estate division.</Text></View>
      ...
      <AgentFeatures agentId="vp-real-estate-development" agentName="VP Real Estate Development" />
    </ScrollView>
  );
}
