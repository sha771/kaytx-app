...
import { Mail, Activity, Star, CircleCheckBig, Clock, Target, ChartBarBig, MessageSquare, Calendar, Shield, ArrowRight, Users, Zap, TrendingUp, BarChart3, Brain, Briefcase, Settings, Layout, ShieldCheck } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function aiEmailMarketingAgentPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks/Day', value: '188', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.8s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '97.1%', icon: Target, color: '#0D47A1' },
  ];

  const capabilities = ['Email list segmentation & audience targeting', 'Email template design & A/B testing', 'Email deliverability monitoring & optimization'];

  const responsibilities = [
    'Email list segmentation & audience targeting & execution',
    'Email template design & A/B testing & execution',
    'Email deliverability monitoring & optimization & execution'
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#0D47A118' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0D47A125' }]}>
          <Mail size={48} color="#0D47A1" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Email Marketing Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Email Marketing Agent provides specialist-level oversight and coordination within the organization. This agent manages 3 sub-agents, driving operational excellence, strategic alignment, and continuous improvement across all assigned domains.
        </Text>
      </View>
        ...
        {subAgentsList.map((sub, index) => {
          ...
          return (
            <TouchableOpacity key={sub.id} onPress={() => router.push('/ai-agent/marketing/sub-agents/' + sub.id)} style={[styles.subAgentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <View style={[styles.subAgentIcon, { backgroundColor: '#0D47A115' }]}>
                <SubIcon size={20} color="#0D47A1" />
              </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#0D47A112' }]}>
              <action.icon size={24} color="#0D47A1" />
      ...
      <AgentFeatures agentId="ai-email-marketing-agent" agentName="AI Email Marketing Agent" />
