...
import { Search, Activity, Star, CircleCheckBig, Clock, Target, ChartBarBig, MessageSquare, Calendar, Shield, ArrowRight, Users, Zap, TrendingUp, BarChart3, Brain, Briefcase, Settings, FileCode, Link } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function aiSeoSpecialistAgentPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks/Day', value: '163', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '2.3s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.1%', icon: Target, color: '#2E7D32' },
  ];

  const capabilities = ['Keyword research & search trend analysis', 'On-page SEO optimization & meta management', 'Backlink analysis & link building strategy'];

  const responsibilities = [
    'Keyword research & search trend analysis & execution',
    'On-page SEO optimization & meta management & execution',
    'Backlink analysis & link building strategy & execution'
  ];

  const activities = [
    { time: '2 min ago', text: 'Processed 34 tasks autonomously', icon: Zap },
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#2E7D3218' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#2E7D3225' }]}>
          <Search size={48} color="#2E7D32" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI SEO Specialist</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI SEO Specialist provides specialist-level oversight and coordination within the organization. This agent manages 3 sub-agents, driving operational excellence, strategic alignment, and continuous improvement across all assigned domains.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {activities.map((act, index) => (
          <View key={index} style={styles.activityRow}>
            <View style={[styles.activityIcon, { backgroundColor: '#2E7D3215' }]}>
              <act.icon size={14} color="#2E7D32" />
            </View>
        ...
        {subAgentsList.map((sub, index) => {
          ...
          return (
            <TouchableOpacity key={sub.id} onPress={() => router.push('/ai-agent/marketing/sub-agents/' + sub.id)} style={[styles.subAgentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <View style={[styles.subAgentIcon, { backgroundColor: '#2E7D3215' }]}>
                <SubIcon size={20} color="#2E7D32" />
              </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#2E7D3212' }]}>
              <action.icon size={24} color="#2E7D32" />
              <Text style={[styles.actionText, { color: '#2E7D32' }]}>{action.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>

      <AgentFeatures agentId="ai-seo-specialist-agent" agentName="AI SEO Specialist" />
