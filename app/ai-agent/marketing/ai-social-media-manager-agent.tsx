...
import { Share2, Activity, Star, CircleCheckBig, Clock, Target, ChartBarBig, MessageSquare, Calendar, Shield, ArrowRight, Users, Zap, TrendingUp, BarChart3, Brain, Briefcase, Settings, MessageCircle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function aiSocialMediaManagerAgentPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks/Day', value: '110', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '2.2s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '97.8%', icon: Target, color: '#1DA1F2' },
  ];

  const capabilities = ['Social media post scheduling & timing optimization', 'Social engagement response & community management', 'Social trend monitoring & viral detection'];

  const responsibilities = [
    'Social media post scheduling & timing optimization & execution',
    'Social engagement response & community management & execution',
    'Social trend monitoring & viral detection & execution'
  ];

  const activities = [
    { time: '2 min ago', text: 'Processed 57 tasks autonomously', icon: Zap },
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#1DA1F218' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#1DA1F225' }]}>
          <Share2 size={48} color="#1DA1F2" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Social Media Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Social Media Manager provides specialist-level oversight and coordination within the organization. This agent manages 3 sub-agents, driving operational excellence, strategic alignment, and continuous improvement across all assigned domains.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.tagsContainer}>
          {capabilities.map((cap, index) => (
            <View key={index} style={[styles.tag, { backgroundColor: '#1DA1F218' }]}>
              <Text style={[styles.tagText, { color: '#1DA1F2' }]}>{cap}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {responsibilities.map((item, index) => (
          <View key={index} style={styles.responsibilityRow}>
            <ArrowRight size={14} color="#1DA1F2" />
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {activities.map((act, index) => (
          <View key={index} style={styles.activityRow}>
            <View style={[styles.activityIcon, { backgroundColor: '#1DA1F215' }]}>
              <act.icon size={14} color="#1DA1F2" />
            </View>
        ...
        {subAgentsList.map((sub, index) => {
          ...
          return (
            <TouchableOpacity key={sub.id} onPress={() => router.push('/ai-agent/marketing/sub-agents/' + sub.id)} style={[styles.subAgentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <View style={[styles.subAgentIcon, { backgroundColor: '#1DA1F215' }]}>
                <SubIcon size={20} color="#1DA1F2" />
              </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#1DA1F212' }]}>
              <action.icon size={24} color="#1DA1F2" />
              <Text style={[styles.actionText, { color: '#1DA1F2' }]}>{action.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>

      <AgentFeatures agentId="ai-social-media-manager-agent" agentName="AI Social Media Manager" />
