...
import { FileText, Activity, Star, CircleCheckBig, Clock, Target, ChartBarBig, MessageSquare, Calendar, Shield, ArrowRight, Users, Zap, TrendingUp, BarChart3, Brain, Briefcase, Settings, PenLine, FileEdit, Share2 } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function aiContentMarketingAgentPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Tasks/Day', value: '374', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '2.1s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.4%', icon: Target, color: '#6A1B9A' },
  ];

  const capabilities = ['Blog content creation & SEO writing', 'Copy editing & quality assurance', 'Content distribution & syndication'];

  const responsibilities = [
    'Blog content creation & SEO writing & execution',
    'Copy editing & quality assurance & execution',
    'Content distribution & syndication & execution'
  ];

  const activities = [
    { time: '2 min ago', text: 'Processed 44 tasks autonomously', icon: Zap },
    { time: '15 min ago', text: 'Updated performance metrics dashboard', icon: BarChart3 },
    { time: '1 hour ago', text: 'Coordinated with 6 sub-agents', icon: Users },
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#6A1B9A18' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#6A1B9A25' }]}>
          <FileText size={48} color="#6A1B9A" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Content Marketing Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Content Marketing Agent provides specialist-level oversight and coordination within the organization. This agent manages 3 sub-agents, driving operational excellence, strategic alignment, and continuous improvement across all assigned domains.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.tagsContainer}>
          {capabilities.map((cap, index) => (
            <View key={index} style={[styles.tag, { backgroundColor: '#6A1B9A18' }]}>
        ...
        {subAgentsList.map((sub, index) => {
          ...
          return (
            <TouchableOpacity key={sub.id} onPress={() => router.push('/ai-agent/marketing/sub-agents/' + sub.id)} style={[styles.subAgentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <View style={[styles.subAgentIcon, { backgroundColor: '#6A1B9A15' }]}>
                <SubIcon size={20} color="#6A1B9A" />
              </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          {quickActions.map((action, index) => (
            <TouchableOpacity key={index} style={[styles.actionButton, { backgroundColor: '#6A1B9A12' }]}>
      ...
      <AgentFeatures agentId="ai-content-marketing-agent" agentName="AI Content Marketing Agent" />
