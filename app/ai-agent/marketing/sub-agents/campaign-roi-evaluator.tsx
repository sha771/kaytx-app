...
import { Activity, ChartBarBig, Clock, Target, Zap, ArrowRight, Briefcase, TrendingUp, DollarSign } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function CampaignRoiEvaluatorPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'cmo',icon:ChartBarBig,color:'#007AFF'}
  ];
  const capabilities = ["ROI Analysis","Campaign Performance","Attribution Modeling","Revenue Tracking","Cost-per-Acquisition","LTV Analysis"];
  const responsibilities = ["Evaluate campaign ROI across all channels","Build and maintain attribution models","Track revenue impact per campaign","Calculate cost-per-acquisition metrics","Analyze customer lifetime value by campaign","Compare actual vs projected ROI","Generate campaign performance dashboards","Recommend budget adjustments based on ROI data"];
  const activities = [{time:"3 min ago",text:"Completed ROI analysis for 8 active campaigns",icon:ChartBarBig},{time:"15 min ago",text:"Updated multi-touch attribution model",icon:Target},{time:"1 hour ago",text:"Identified top 3 campaigns by LTV",icon:TrendingUp},{time:"4 hours ago",text:"Generated monthly ROI dashboard",icon:Activity}];
  const endpoints = ["/consult/campaign-roi-evaluator","/campaign-roi-evaluator/execute","/campaign-roi-evaluator/analyze"];
  const quickActions = [{label:"ROI Report",icon:ChartBarBig},{label:"Attribution Model",icon:Target},{label:"LTV Analysis",icon:TrendingUp},{label:"CPA Tracker",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#C6282818'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#C6282825'}]}><ChartBarBig size={56} color="#C62828"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Campaign ROI Evaluator</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Measures and evaluates ROI for every marketing campaign. Provides attribution modeling, tracks revenue impact, and delivers insights to optimize future campaign spending.</Text></View>
