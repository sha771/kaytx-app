...
import { Activity, UserPlus, Clock, Target, Zap, ArrowRight, Briefcase, Timer, DollarSign } from 'lucide-react-native';
...
export default function TaskAssignerPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'marketing-manager',icon:UserPlus,color:'#007AFF'}
  ];
  const capabilities = ["Task Assignment","Workload Balancing","Skill Matching","Priority Routing","Team Capacity","Auto-delegation"];
  const responsibilities = ["Assign tasks based on team member skills and capacity","Balance workloads across the marketing team","Route high-priority tasks to appropriate specialists","Monitor team capacity and availability","Auto-delegate routine tasks to available resources","Track task completion rates and bottlenecks","Recommend team structure adjustments","Report task assignment metrics to Marketing Manager"];
  const activities = [{time:"1 min ago",text:"Assigned 5 content tasks to writing team",icon:UserPlus},{time:"10 min ago",text:"Rebalanced workload: moved 3 tasks to design",icon:Target},{time:"1 hour ago",text:"Auto-delegated 12 routine tasks",icon:Zap},{time:"2 hours ago",text:"Flagged capacity issue in social team",icon:Activity}];
  const endpoints = ["/consult/task-assigner","/task-assigner/execute","/task-assigner/analyze"];
  const quickActions = [{label:"Assign Task",icon:UserPlus},{label:"Workload",icon:Target},{label:"Auto-delegate",icon:Zap},{label:"Capacity",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#E6510018'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#E6510025'}]}><UserPlus size={56} color="#E65100"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Task Assigner</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Intelligently assigns marketing tasks to team members based on skills, capacity, and priority, ensuring balanced workloads and optimal resource utilization.</Text></View>
