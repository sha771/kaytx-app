...
import { Activity, Calendar, Clock, Target, Zap, ArrowRight, Briefcase, CheckCircle, RefreshCw } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function EditorialCalendarPlannerPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'vp-content',icon:Calendar,color:'#007AFF'}
  ];
  const capabilities = ["Editorial Planning","Content Scheduling","Topic Clustering","Publication Timing","Content Gaps","Seasonal Planning"];
  const responsibilities = ["Plan and maintain editorial calendar across channels","Schedule content for optimal publication timing","Identify and fill content gaps in coverage","Coordinate seasonal and event-driven content","Cluster topics for thematic consistency","Align content schedule with campaign timelines","Manage content pipeline and workflow status","Report calendar status and upcoming priorities"];
  const activities = [{time:"2 min ago",text:"Scheduled 18 blog posts for September",icon:Calendar},{time:"20 min ago",text:"Identified 4 content gaps in product category",icon:Target},{time:"1 hour ago",text:"Aligned content schedule with Q3 campaigns",icon:Zap},{time:"3 hours ago",text:"Generated seasonal content plan",icon:Activity}];
  const endpoints = ["/consult/editorial-calendar-planner","/editorial-calendar-planner/execute","/editorial-calendar-planner/analyze"];
  const quickActions = [{label:"View Calendar",icon:Calendar},{label:"Gap Analysis",icon:Target},{label:"Seasonal Plan",icon:Zap},{label:"Pipeline",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#00897B18'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#00897B25'}]}><Calendar size={56} color="#00897B"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Editorial Calendar Planner</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Structures and optimizes the content publishing schedule, ensuring topic coverage, seasonal relevance, and consistent output across all channels.</Text></View>
