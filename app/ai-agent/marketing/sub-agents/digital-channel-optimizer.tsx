...
import { Activity, Globe, Clock, Target, Zap, ArrowRight, Briefcase, Gauge, TrendingUp } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function DigitalChannelOptimizerPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'vp-digital',icon:Globe,color:'#007AFF'}
  ];
  const capabilities = ["Channel Optimization","Performance Tuning","Spend Allocation","Channel Analytics","Cross-channel","Audience Targeting"];
  const responsibilities = ["Optimize digital channel performance and efficiency","Tune spend allocation across paid digital channels","Adjust targeting parameters for maximum ROI","Monitor channel analytics and performance trends","Identify underperforming channels for reallocation","Coordinate cross-channel optimization strategies","A/B test channel configurations and settings","Report channel optimization results to VP Digital"];
  const activities = [{time:"1 min ago",text:"Reallocated $50K from Display to Search",icon:Globe},{time:"15 min ago",text:"Optimized targeting on 4 ad platforms",icon:Target},{time:"1 hour ago",text:"Identified LinkedIn as top-performing B2B channel",icon:Zap},{time:"3 hours ago",text:"Generated channel optimization report",icon:Activity}];
  const endpoints = ["/consult/digital-channel-optimizer","/digital-channel-optimizer/execute","/digital-channel-optimizer/analyze"];
  const quickActions = [{label:"Optimize Spend",icon:Globe},{label:"Targeting",icon:Target},{label:"Channel Report",icon:Zap},{label:"Reallocate",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#1565C018'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#1565C025'}]}><Globe size={56} color="#1565C0"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Digital Channel Optimizer</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Optimizes digital channel performance by continuously tuning spend allocation, targeting parameters, and campaign settings across all digital platforms.</Text></View>
