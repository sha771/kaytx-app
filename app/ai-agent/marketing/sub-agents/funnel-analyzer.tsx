...
import { Activity, Filter, Clock, Target, Zap, ArrowRight, Briefcase, FlaskConical, GitBranch } from 'lucide-react-native';
...
export default function FunnelAnalyzerPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'vp-growth',icon:Filter,color:'#007AFF'}
  ];
  const capabilities = ["Funnel Analysis","Drop-off Detection","Conversion Paths","User Journey","Bottleneck ID","Stage Optimization"];
  const responsibilities = ["Analyze conversion funnels across all touchpoints","Identify drop-off points and bottlenecks","Map user journey paths and friction points","Quantify conversion rate impact per stage","Recommend optimizations for each funnel stage","Track funnel performance trends over time","Compare funnel performance across segments","Report funnel insights to VP Growth"];
  const activities = [{time:"1 min ago",text:"Identified 34% drop-off at payment step",icon:Filter},{time:"15 min ago",text:"Mobile funnel conversion improved 12%",icon:Target},{time:"1 hour ago",text:"Mapped 5 user journey paths",icon:Zap},{time:"2 hours ago",text:"Generated funnel comparison report",icon:Activity}];
  const endpoints = ["/consult/funnel-analyzer","/funnel-analyzer/execute","/funnel-analyzer/analyze"];
  const quickActions = [{label:"Funnel Report",icon:Filter},{label:"Drop-off Map",icon:Target},{label:"Journey Paths",icon:Zap},{label:"Optimize",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#FF6B3518'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#FF6B3525'}]}><Filter size={56} color="#FF6B35"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Funnel Analyzer</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Identifies drop-off points and bottlenecks across conversion funnels, providing actionable recommendations to improve flow-through rates at every customer journey stage.</Text></View>
