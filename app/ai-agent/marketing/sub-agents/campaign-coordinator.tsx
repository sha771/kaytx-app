...
import { Activity, GitMerge, Clock, Target, Zap, ArrowRight, Briefcase, Layers, Calendar } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function CampaignCoordinatorPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'vp-marketing',icon:GitMerge,color:'#007AFF'}
  ];
  const capabilities = ["Campaign Orchestration","Cross-team Coordination","Milestone Tracking","Status Reporting","Issue Escalation","Launch Management"];
  const responsibilities = ["Coordinate end-to-end campaign execution","Track campaign milestones and deliverables","Manage cross-team dependencies and handoffs","Escalate issues and blockers proactively","Ensure all campaign assets are ready for launch","Run pre-launch checklists and quality gates","Monitor campaign health during execution","Generate campaign status reports for leadership"];
  const activities = [{time:"2 min ago",text:"Cleared 3 campaigns for launch readiness",icon:GitMerge},{time:"20 min ago",text:"Escalated design asset delay for Campaign X",icon:Target},{time:"1 hour ago",text:"Completed pre-launch checklist for Q3 push",icon:Zap},{time:"3 hours ago",text:"Generated campaign status dashboard",icon:Activity}];
  const endpoints = ["/consult/campaign-coordinator","/campaign-coordinator/execute","/campaign-coordinator/analyze"];
  const quickActions = [{label:"Launch Check",icon:GitMerge},{label:"Milestones",icon:Target},{label:"Escalations",icon:Zap},{label:"Status Report",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#D81B6018'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#D81B6025'}]}><GitMerge size={56} color="#D81B60"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Campaign Coordinator</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Manages end-to-end campaign execution, coordinating across teams, tracking milestones, and ensuring every campaign launches on time with all components in place.</Text></View>
