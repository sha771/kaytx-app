...
import { Activity, Timer, Clock, Target, Zap, ArrowRight, Briefcase, UserPlus, DollarSign } from 'lucide-react-native';
...
export default function DeadlineTrackerPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'marketing-manager',icon:Timer,color:'#007AFF'}
  ];
  const capabilities = ["Deadline Monitoring","Risk Escalation","Timeline Tracking","Milestone Alerts","Dependency Mapping","Progress Tracking"];
  const responsibilities = ["Monitor all marketing project deadlines","Identify and flag at-risk deliverables early","Escalate deadline risks to appropriate stakeholders","Map task dependencies and critical path items","Send milestone alerts and deadline reminders","Track progress against planned timelines","Calculate schedule variance and slippage","Report deadline status and risk assessment to Marketing Manager"];
  const activities = [{time:"30 sec ago",text:"Alert: Campaign X assets due in 2 hours (at risk)",icon:Timer},{time:"10 min ago",text:"Escalated 2 overdue deliverables to VP",icon:Target},{time:"1 hour ago",text:"Updated critical path for Q3 launch",icon:Zap},{time:"3 hours ago",text:"Generated deadline compliance report: 94%",icon:Activity}];
  const endpoints = ["/consult/deadline-tracker","/deadline-tracker/execute","/deadline-tracker/analyze"];
  const quickActions = [{label:"Deadlines",icon:Timer},{label:"Escalations",icon:Target},{label:"Critical Path",icon:Zap},{label:"Compliance",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#E6510018'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#E6510025'}]}><Timer size={56} color="#E65100"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Deadline Tracker</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Monitors all marketing deadlines, identifies at-risk deliverables, and escalates issues proactively to ensure every project is delivered on time.</Text></View>
