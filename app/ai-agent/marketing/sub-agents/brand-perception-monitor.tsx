...
import { Activity, Eye, Clock, Target, Zap, ArrowRight, Briefcase, Shield, Image as ImageIcon } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function BrandPerceptionMonitorPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'vp-brand',icon:Eye,color:'#007AFF'}
  ];
  const capabilities = ["Sentiment Analysis","Perception Tracking","Social Listening","Brand Health","Competitor Monitoring","Reputation Alerts"];
  const responsibilities = ["Monitor brand sentiment across social and review platforms","Track perception shifts and alert on significant changes","Analyze competitor brand positioning in real-time","Generate brand health scorecards","Identify reputation risks before they escalate","Measure brand awareness and recall metrics","Correlate perception data with marketing activities","Report brand health insights to VP Brand"];
  const activities = [{time:"2 min ago",text:"Detected positive sentiment spike on Twitter",icon:Eye},{time:"20 min ago",text:"Brand health score: 87/100 (+3)",icon:Target},{time:"1 hour ago",text:"Flagged competitor rebranding activity",icon:Zap},{time:"3 hours ago",text:"Generated weekly perception report",icon:Activity}];
  const endpoints = ["/consult/brand-perception-monitor","/brand-perception-monitor/execute","/brand-perception-monitor/analyze"];
  const quickActions = [{label:"Sentiment Scan",icon:Eye},{label:"Brand Health",icon:Target},{label:"Competitor Alert",icon:Zap},{label:"Weekly Report",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#9C27B018'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#9C27B025'}]}><Eye size={56} color="#9C27B0"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Brand Perception Monitor</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Continuously tracks brand sentiment across all channels, providing real-time alerts on perception shifts, competitive positioning changes, and reputation risks.</Text></View>
