...
import { Activity, RefreshCw, Clock, Target, Zap, ArrowRight, Briefcase, Calendar, CheckCircle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function RepurposingStrategistPage() {
  ...
  const stats = [
    ...
    {label:'Parent',value:'vp-content',icon:RefreshCw,color:'#007AFF'}
  ];
  const capabilities = ["Content Repurposing","Multi-format Strategy","Channel Adaptation","Content Atomization","Lifecycle Extension","Cross-platform"];
  const responsibilities = ["Identify repurposing opportunities across content library","Develop multi-format strategies for key content assets","Atomize long-form content into social, email, and video","Adapt content for different channel requirements","Extend content lifecycle through strategic repurposing","Track repurposing ROI and performance metrics","Maintain repurposing playbook and templates","Report repurposing impact to VP Content"];
  const activities = [{time:"4 min ago",text:"Repurposed whitepaper into 8 social posts",icon:RefreshCw},{time:"20 min ago",text:"Created video script from top blog post",icon:Target},{time:"1 hour ago",text:"Atomized webinar into 6 email snippets",icon:Zap},{time:"3 hours ago",text:"Generated repurposing ROI report: 340% lift",icon:Activity}];
  const endpoints = ["/consult/repurposing-strategist","/repurposing-strategist/execute","/repurposing-strategist/analyze"];
  const quickActions = [{label:"Repurpose Content",icon:RefreshCw},{label:"Atomize",icon:Target},{label:"Multi-format",icon:Zap},{label:"ROI Report",icon:Activity}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{backgroundColor:'#00897B18'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#00897B25'}]}><RefreshCw size={56} color="#00897B"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Repurposing Strategist</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>Develops strategies to repurpose and atomize content across formats and channels, maximizing the ROI of every content asset by extending its lifecycle and reach.</Text></View>
