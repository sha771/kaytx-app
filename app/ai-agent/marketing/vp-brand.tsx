...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'2111',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'2.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.7%',icon:Target,color:'#C62828'}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#C6282820' }]}><Palette size={48} color="#C62828" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Brand</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Brand AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Marketing division.</Text></View>
      ...
      <AgentFeatures agentId="vp-brand" agentName="VP Brand" />
    </ScrollView>
  );
}
