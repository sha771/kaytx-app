...
import { Briefcase, Activity, Star, CircleCheckBig, Clock, Target, ChartBarBig, MessageSquare, Calendar, Shield, ArrowRight, Users, Zap, Eye, BarChart3, MessageCircle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function AIBrandManagerPage() {
  const { theme } = useTheme();
  const router = useRouter();

  const stats = [
    { label: 'Brands', value: '24', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.8%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '1.5s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '97.5%', icon: Target, color: '#F43F5E' },
  ];

  const capabilities = [
    'Brand Management', 'Competitor Tracking', 'Brand Health', 'Messaging Alignment',
    'Brand Strategy', 'Market Positioning', 'Brand Equity', 'Visual Consistency'
  ];

  const responsibilities = [
    'Competitor brand tracking & benchmarking',
    'Brand health surveying & sentiment analysis',
    'Messaging alignment across all channels',
    'Brand strategy development & execution',
    'Market positioning & competitive differentiation',
    'Brand equity measurement & growth',
    'Visual consistency enforcement & auditing',
    'Brand crisis management & response'
  ];

  const activities = [
    { time: '5 min ago', text: 'Tracked 6 competitor brand movements', icon: Eye },
    { time: '20 min ago', text: 'Surveyed brand health across 4 markets', icon: BarChart3 },
    { time: '1 hour ago', text: 'Aligned messaging for Q3 campaign launch', icon: MessageCircle },
    { time: '3 hours ago', text: 'Updated brand positioning deck', icon: Briefcase },
    { time: '5 hours ago', text: 'Reviewed brand equity scorecard', icon: Zap },
  ];
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: '#F43F5E18' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#F43F5E25' }]}>
          <Briefcase size={48} color="#F43F5E" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Brand Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Brand Manager oversees brand strategy, competitor tracking, and messaging alignment across all channels. This specialist agent monitors brand health, tracks competitor movements, and ensures consistent brand messaging to maintain and grow brand equity.
        </Text>
      </View>
      ...
      <AgentFeatures agentId="ai-brand-manager" agentName="AI Brand Manager" />
