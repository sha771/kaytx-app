...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'4752',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.4s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.2%',icon:Target,color:'#1565C0'}];
  const capabilities = ['API Development','Service Integration','Data Validation','Microservices Architecture','Database Design','Performance Optimization'];
  const responsibilities = ['Design & develop scalable API endpoints','Validate data integrity across services','Integrate third-party & internal services','Architect microservices communication patterns','Optimize database queries & schema design','Monitor & improve backend performance metrics'];
  const activities = [{time:'3 min ago',text:'Built 18 new REST API endpoints',icon:CircleCheckBig},{time:'6 min ago',text:'Validated 2,400 data records across 6 services',icon:Clock},{time:'9 min ago',text:'Integrated 3 new third-party APIs',icon:Zap}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{borderBottomColor:theme.colors.border||'#E5E5EA'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#1565C020'}]}><Code size={48} color="#1565C0"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI Backend Developer</Text>
        <Text style={[styles.heroSubtitle,{color:theme.colors.textSecondary}]}>Engineering</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge,{backgroundColor:'#34C75922'}]}><Activity size={12} color="#34C759"/><Text style={[styles.badgeText,{color:'#34C759'}]}>Active</Text></View>
          <View style={[styles.badge,{backgroundColor:'#1565C022'}]}><Star size={12} color="#1565C0"/><Text style={[styles.badgeText,{color:'#1565C0'}]}>Agent</Text></View>
        </View>
      </View>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI Backend Developer designs, builds, and maintains scalable backend services, APIs, and data pipelines. It ensures service integration, data validation, and performance optimization across the entire engineering stack.</Text></View>
      ...
      <AgentFeatures agentId="ai-backend-developer" agentName="AI Backend Developer"/>
    </ScrollView>
  );
}
