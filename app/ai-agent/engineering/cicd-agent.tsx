...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'205',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.0%',icon:Target,color:'#1565C0'}];
  const capabilities = ['CI/CD Management','Build Automation','Deployment Assurance'];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI CI/CD Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Manages CI/CD pipelines, automates builds, and ensures smooth deployments.</Text></View>
      ...
      <AgentFeatures agentId="cicd-agent" agentName="AI CI/CD Agent" />
    </ScrollView>
  );
}
