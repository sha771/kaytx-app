...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'2216',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.8s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.3%',icon:Target,color:'#1565C0'}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#1565C020' }]}><Shield size={48} color="#1565C0" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Security Engineer</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Security Engineer AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Engineering department.</Text></View>
      ...
      <AgentFeatures agentId="security-engineer-1" agentName="Security Engineer" />
    </ScrollView>
  );
}
