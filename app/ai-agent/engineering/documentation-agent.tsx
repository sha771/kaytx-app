...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'233',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'2.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.5%',icon:Target,color:'#1565C0'}];
  const capabilities = ['Documentation Creation','API Documentation','User Guide Generation'];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#1565C020' }]}><User size={48} color="#1565C0" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Documentation Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Creates and maintains technical documentation, API docs, and user guides.</Text></View>
      ...
      <AgentFeatures agentId="documentation-agent" agentName="AI Documentation Agent" />
    </ScrollView>
  );
}
