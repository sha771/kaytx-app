...
export default function AgentPage() {
  ...
  const stats = [{label:'Endpoints',value:'312',icon:CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Latency',value:'89ms',icon:Clock,color:'#FF9500'},{label:'Coverage',value:'98.5%',icon:Target,color:'#1565C0'}];
  const capabilities = ['REST API Design','GraphQL Schema','API Versioning','Rate Limiting','Authentication Flows','API Documentation'];
  const responsibilities = ['Design RESTful & GraphQL API endpoints','Implement API versioning strategies','Configure rate limiting & throttling','Build authentication & authorization flows','Generate OpenAPI/Swagger documentation','Test API endpoints with automated suites'];
  const activities = [{time:'3 min ago',text:'Created 24 new API endpoints',icon:CircleCheckBig},{time:'6 min ago',text:'Updated API docs for v3.2 release',icon:Clock},{time:'9 min ago',text:'Implemented OAuth2 flow for partner APIs',icon:Zap}];
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.colors.background}]}>
      <View style={[styles.hero,{borderBottomColor:theme.colors.border||'#E5E5EA'}]}>
        <View style={[styles.heroIconWrap,{backgroundColor:'#1565C020'}]}><Code size={48} color="#1565C0"/></View>
        <Text style={[styles.heroTitle,{color:theme.colors.text}]}>AI API Endpoint Developer</Text>
      ...
      <View style={[styles.section,{backgroundColor:theme.colors.card||'#F2F2F7'}]}><Text style={[styles.sectionTitle,{color:theme.colors.text}]}>Overview</Text><Text style={[styles.description,{color:theme.colors.textSecondary}]}>The AI API Endpoint Developer specializes in designing, building, and maintaining RESTful and GraphQL API endpoints with robust versioning, rate limiting, authentication, and documentation.</Text></View>
      ...
      <AgentFeatures agentId="ai-api-endpoint-developer" agentName="AI API Endpoint Developer"/>
    </ScrollView>
  );
}
