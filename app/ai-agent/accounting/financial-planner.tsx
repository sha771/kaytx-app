...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, User, Shield, TrendingUp, PieChart } from 'lucide-react-native';
...
export default function FinancialPlannerPage() {
  ...
  const [scenarioPlanning, setScenarioPlanning] = useState(true);
  const [riskAnalysis, setRiskAnalysis] = useState(true);

  const stats = [{label:'Tasks/Day',value:'208',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.3%',icon:Target,color:'#0D47A1'}];
  const capabilities = ['Financial Planning','Budget Forecasting','Strategic Guidance'];
  ...
  const activities = [{time:'3 min ago',text:'Created Q2 financial forecast',icon: CircleCheckBig},{time:'15 min ago',text:'Analyzed budget variances',icon:Clock},{time:'1 hour ago',text:'Generated strategic report',icon:Zap}];

  const plans = [
    { id: 'FP-001', name: 'Q2 2026 Budget', status: 'Active', amount: '$1.2M', date: 'May 1, 2026' },
    { id: 'FP-002', name: 'Capital Expenditure', status: 'Review', amount: '$500K', date: 'Apr 15, 2026' },
    { id: 'FP-003', name: 'Cash Flow Optimization', status: 'Active', amount: '$850K', date: 'Apr 1, 2026' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Financial Planner</Text>
      ...
      <View style={styles.tabContainer}>
        {['overview', 'plans', 'capabilities', 'settings'].map((tab) => (
      ...
      {activeTab === 'overview' && (
        <>
          <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Creates financial plans, forecasts budgets, and provides strategic financial guidance.</Text></View>
      ...
      {activeTab === 'plans' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Financial Plans</Text>
          {plans.map((plan, i) => (
            <View key={i} style={styles.txnCard}>
              <View style={styles.txnHeader}>
                <PieChart size={18} color="#0D47A1" />
                <Text style={styles.txnId}>{plan.id}</Text>
                <View style={[styles.statusBadge, { backgroundColor: plan.status === 'Active' ? '#34C75920' : '#FF950020' }]}>
                  <Text style={[styles.statusText, { color: plan.status === 'Active' ? '#34C759' : '#FF9500' }]}>{plan.status}</Text>
                </View>
              </View>
              <Text style={styles.txnDesc}>{plan.name}</Text>
              <View style={styles.txnFooter}>
                <Text style={styles.txnDate}>{plan.date}</Text>
                <Text style={styles.txnAmount}>{plan.amount}</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          {[
            { endpoint: '/consult/financial-planner', desc: 'Consult on financial planning' },
            { endpoint: '/financial-planner/forecast', desc: 'Create budget forecast' },
            { endpoint: '/financial-planner/analyze', desc: 'Analyze financial data' },
            { endpoint: '/financial-planner/strategy', desc: 'Generate strategy' },
          ].map((item, i) => (
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Auto Forecast</Text>
              <Text style={styles.featureToggleDesc}>Auto-generate forecasts</Text>
            </View>
            <Switch value={autoForecast} onValueChange={setAutoForecast} trackColor={{ false: '#767577', true: '#0D47A1' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Scenario Planning</Text>
              <Text style={styles.featureToggleDesc}>Enable scenario analysis</Text>
            </View>
            <Switch value={scenarioPlanning} onValueChange={setScenarioPlanning} trackColor={{ false: '#767577', true: '#0D47A1' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Risk Analysis</Text>
              <Text style={styles.featureToggleDesc}>Enable risk assessment</Text>
            </View>
            <Switch value={riskAnalysis} onValueChange={setRiskAnalysis} trackColor={{ false: '#767577', true: '#0D47A1' }} />
          </View>
        </View>
      )}

      <AgentFeatures agentId="financial-planner" agentName="AI Financial Planner" />
