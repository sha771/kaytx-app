...
import { DollarSign, Activity, CheckCircle2, Clock, Target, BarChart3, MessageSquare, Calendar, Shield, ArrowRight, Users, Zap, Star, PieChart, Landmark } from 'lucide-react-native';

export default function VPFinancePage() {
  const { theme } = useTheme();

  const stats = [
    { label: 'Transactions', value: '45,231', icon: CheckCircle2, color: '#34C759' },
    { label: 'Uptime', value: '99.99%', icon: Activity, color: '#007AFF' },
    { label: 'Response', value: '0.6s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.7%', icon: Target, color: '#AF52DE' },
  ];

  const capabilities = [
    'Financial Planning', 'Budget Control', 'Forecasting', 'Treasury Mgmt',
    'Audit & Compliance', 'Investor Relations', 'Tax Strategy', 'Risk Analysis'
  ];

  const responsibilities = [
    'Financial Strategy & Capital Allocation',
    'Budget Planning, Control & Variance Analysis',
    'Revenue Forecasting & Financial Modeling',
    'Treasury, Cash Flow & Liquidity Management',
    'Audit Coordination & Internal Controls',
    'Investor Relations & Board Reporting',
    'Tax Strategy & Regulatory Compliance',
    'Risk Assessment & Financial Governance'
  ];
  ...
  const activities = [
    ...
    { time: '40 min ago', text: 'Published cash flow forecast update', icon: BarChart3 },
    { time: '2 hours ago', text: 'Reviewed audit findings with team', icon: Shield },
    { time: '5 hours ago', text: 'Updated investor presentation deck', icon: PieChart },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { backgroundColor: theme.colors.primary + '18' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Finance</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Finance & Accounting Department</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The VP Finance AI oversees all financial operations, strategic planning, and capital allocation. This agent manages budgeting, forecasting, treasury, and ensures financial compliance while optimizing organizational value.
        </Text>
      </View>
      ...
      <AgentFeatures agentId="vp-finance" agentName="VP Finance" />
    </ScrollView>
  );
}
