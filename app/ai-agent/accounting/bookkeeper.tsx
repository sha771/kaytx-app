...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, User, Shield, FileText } from 'lucide-react-native';
...
export default function BookkeeperPage() {
  ...
  const [invoiceProcessing, setInvoiceProcessing] = useState(true);
  const [financialAlerts, setFinancialAlerts] = useState(true);

  const stats = [{label:'Tasks/Day',value:'99',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.0%',icon:Target,color:'#0D47A1'}];
  const capabilities = ['Record Maintenance','Account Reconciliation','Accurate Bookkeeping'];
  ...
  const transactions = [
    { id: 'TXN-001', date: 'Today', description: 'Invoice Payment - Client A', amount: '$5,000', status: 'Completed' },
    { id: 'TXN-002', date: 'Today', description: 'Expense Report - John D', amount: '$450', status: 'Pending' },
    { id: 'TXN-003', date: 'Yesterday', description: 'Vendor Payment - Supplier X', amount: '$12,500', status: 'Completed' },
    { id: 'TXN-004', date: 'Yesterday', description: 'Reconciliation - Bank', amount: '$0', status: 'Completed' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Bookkeeper</Text>
      ...
      <View style={styles.tabContainer}>
        {['overview', 'transactions', 'capabilities', 'settings'].map((tab) => (
      ...
      {activeTab === 'overview' && (
        <>
          <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Maintains financial records, reconciles accounts, and ensures accurate bookkeeping.</Text></View>
          ...
          {transactions.map((txn, i) => (
            <View key={i} style={styles.txnCard}>
              <View style={styles.txnHeader}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: txn.status === 'Completed' ? '#34C75920' : '#FF950020' }]}>
                  <Text style={[styles.statusText, { color: txn.status === 'Completed' ? '#34C759' : '#FF9500' }]}>{txn.status}</Text>
                </View>
              </View>
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          {[
            { endpoint: '/consult/bookkeeper', desc: 'Consult on bookkeeping' },
            { endpoint: '/bookkeeper/reconcile', desc: 'Reconcile accounts' },
            { endpoint: '/bookkeeper/process', desc: 'Process transactions' },
            { endpoint: '/bookkeeper/reports', desc: 'Generate reports' },
          ].map((item, i) => (
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Auto Reconcile</Text>
              <Text style={styles.featureToggleDesc}>Automatically reconcile accounts</Text>
            </View>
            <Switch value={autoReconcile} onValueChange={setAutoReconcile} trackColor={{ false: '#767577', true: '#0D47A1' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Invoice Processing</Text>
              <Text style={styles.featureToggleDesc}>Auto-process invoices</Text>
            </View>
            <Switch value={invoiceProcessing} onValueChange={setInvoiceProcessing} trackColor={{ false: '#767577', true: '#0D47A1' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Financial Alerts</Text>
              <Text style={styles.featureToggleDesc}>Alert on anomalies</Text>
            </View>
            <Switch value={financialAlerts} onValueChange={setFinancialAlerts} trackColor={{ false: '#767577', true: '#0D47A1' }} />
          </View>
        </View>
      )}

      <AgentFeatures agentId="bookkeeper" agentName="AI Bookkeeper" />
