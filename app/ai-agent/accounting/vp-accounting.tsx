...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'1997',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'100.0%',icon:Target,color:'#0D47A1'}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#0D47A120' }]}><Briefcase size={48} color="#0D47A1" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Accounting</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Accounting AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Accounting & Finance division.</Text></View>
      ...
      <AgentFeatures agentId="vp-accounting" agentName="VP Accounting" />
    </ScrollView>
  );
}
