...
import { Settings, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Users, ChevronRight, Shield, Gauge, BarChart3, TrendingUp, AlertTriangle, CheckCircle2, Eye, Zap, ChartBarBig } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function AIOperationsManagerSubPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState('overview');

  const stats = [
    { label: 'Workflows Monitored', value: '1,540', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response Time', value: '0.8s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '97.8%', icon: Target, color: '#4E342E' },
  ];

  const subOpsMetrics = [
    { label: 'Workflow Monitoring', value: '99.2%', trend: '+2.4%', icon: Eye, color: '#007AFF' },
    { label: 'Bottleneck Detection', value: '94%', trend: '+6.1%', icon: Zap, color: '#FF9500' },
    { label: 'Efficiency Reporting', value: '98%', trend: '+3.3%', icon: ChartBarBig, color: '#34C759' },
    { label: 'Ops Savings', value: '$640K', trend: '+9%', icon: TrendingUp, color: '#4E342E' },
  ];

  const capabilities = [
    'Workflow Health Monitoring', 'Bottleneck Detection & Analysis', 'Efficiency Reporting & Dashboards',
    'Real-Time Performance Tracking', 'Process Flow Analysis', 'Anomaly Detection',
    'Capacity Utilization Monitoring', 'SLA Compliance Tracking', 'Resource Throttling Alerts',
    'Workflow Optimization Recommendations', 'Cross-System Monitoring', 'Automated Status Reporting'
  ];

  const responsibilities = [
    'Monitor all operational workflows in real-time for health and performance',
    'Detect bottlenecks and performance degradation before they impact operations',
    'Generate efficiency reports with actionable insights and recommendations',
    'Track SLA compliance across all monitored workflows and processes',
    'Analyze process flows for optimization opportunities and waste reduction',
    'Detect anomalies and trigger automated alerts for immediate attention',
    'Provide cross-system monitoring for end-to-end workflow visibility',
    'Deliver automated status reporting to stakeholders and management'
  ];

  const activities = [
    { time: '1 min ago', text: 'Workflow health check: 142 workflows monitored — 99.2% healthy', icon: Eye, type: 'success' },
    { time: '6 min ago', text: 'Bottleneck detected: Invoice approval queue at 94% capacity — auto-scaled', icon: Zap, type: 'warning' },
    { time: '14 min ago', text: 'Efficiency report generated: Operations throughput +8% vs last week', icon: ChartBarBig, type: 'info' },
    { time: '22 min ago', text: 'SLA alert: Customer response time approaching threshold — remediation triggered', icon: AlertTriangle, type: 'warning' },
    { time: '35 min ago', text: 'Anomaly detected: Unusual spike in API latency — root cause identified in 45s', icon: Eye, type: 'success' },
  ];

  const subAgents = [
    { id: 'workflow-monitor', name: 'AI Workflow Monitor', icon: Eye, desc: 'Real-time workflow health monitoring', color: '#007AFF', route: '/ai-agent/operations/sub-agents/workflow-monitor' },
    { id: 'bottleneck-detector', name: 'AI Bottleneck Detector', icon: Zap, desc: 'Detect & resolve operational bottlenecks', color: '#FF9500', route: '/ai-agent/operations/sub-agents/bottleneck-detector' },
    { id: 'efficiency-reporter', name: 'AI Efficiency Reporter', icon: ChartBarBig, desc: 'Generate efficiency reports & insights', color: '#34C759', route: '/ai-agent/operations/sub-agents/efficiency-reporter' },
  ];

  const kpiData = [
    { metric: 'Workflow Health Score', value: '99.2%', target: '98%', status: 'exceeding' },
    { metric: 'Bottleneck Detection Rate', value: '94%', target: '85%', status: 'exceeding' },
    { metric: 'Efficiency Improvement Rate', value: '+8.2%', target: '+5%', status: 'exceeding' },
    { metric: 'Anomaly Detection Time', value: '45s', target: '120s', status: 'exceeding' },
    { metric: 'SLA Compliance', value: '99.4%', target: '99%', status: 'exceeding' },
    { metric: 'Report Generation Time', value: '12s', target: '30s', status: 'exceeding' },
  ];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Sub-Operations Metrics</Text><View style={styles.metricsGrid}>{subOpsMetrics.map((m, i) => (<View key={i} style={[styles.metricCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><View style={styles.metricHeader}><m.icon size={18} color={m.color} /><Text style={[styles.metricTrend, { color: '#34C759' }]}>{m.trend}</Text></View><Text style={[styles.metricValue, { color: theme.colors.text }]}>{m.value}</Text><Text style={[styles.metricLabel, { color: theme.colors.textSecondary }]}>{m.label}</Text></View>))}</View></View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Operations Manager (Sub) focuses on real-time operational monitoring, bottleneck detection, and efficiency reporting. It provides continuous visibility into workflow health and performance, enabling proactive intervention before issues impact business operations.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="ai-operations-manager-sub" agentName="AI Operations Manager (Sub)" />
