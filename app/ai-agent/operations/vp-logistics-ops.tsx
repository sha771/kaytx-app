...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'4157',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.0%',icon:Target,color:'#4E342E'}];
  const capabilities = ['Logistics Management','Supply Chain Operations','Quality Assurance','Process Optimization','Vendor Management','Resource Planning'];
  ...
  const activities = [{time:'3 min ago',text:'Optimized 8 shipping routes',icon: CircleCheckBig},{time:'6 min ago',text:'Managed 120 warehouse transactions',icon:Clock},{time:'9 min ago',text:'Coordinated 45 supplier deliveries',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#4E342E20' }]}><Truck size={48} color="#4E342E" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Logistics Operations</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Logistics Operations AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Operations division.</Text></View>
      ...
      <AgentFeatures agentId="vp-logistics-ops" agentName="VP Logistics Operations" />
    </ScrollView>
  );
}
