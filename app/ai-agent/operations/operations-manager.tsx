...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, User, Shield, ClipboardList, Package } from 'lucide-react-native';
...
export default function OperationsManagerPage() {
  ...
  const [teamCoordination, setTeamCoordination] = useState(true);
  const [resourceAllocation, setResourceAllocation] = useState(true);

  const stats = [{label:'Tasks/Day',value:'143',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.4%',icon:Target,color:'#4E342E'}];
  const capabilities = ['Operations Oversight','Team Coordination','Operational Excellence'];
  ...
  const activities = [{time:'3 min ago',text:'Optimized 8 shipping routes',icon: CircleCheckBig},{time:'10 min ago',text:'Coordinated team tasks',icon:Clock},{time:'30 min ago',text:'Reviewed operations metrics',icon:Zap}];

  const operations = [
    { id: 'OPS-001', type: 'Shipping', status: 'On Track', tasks: 45, date: 'Today' },
    { id: 'OPS-002', type: 'Warehouse', status: 'Delayed', tasks: 12, date: 'Today' },
    { id: 'OPS-003', type: 'Supply Chain', status: 'On Track', tasks: 28, date: 'Yesterday' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.tabContainer}>
        {['overview', 'operations', 'capabilities', 'settings'].map((tab) => (
      ...
      {activeTab === 'overview' && (
        <>
          <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Oversees all operations, coordinates teams, and ensures operational excellence.</Text></View>
      ...
      {activeTab === 'operations' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Operations Status</Text>
          {operations.map((op, i) => (
            <View key={i} style={styles.txnCard}>
              <View style={styles.txnHeader}>
                <Package size={18} color="#4E342E" />
                <Text style={styles.txnId}>{op.id}</Text>
                <View style={[styles.statusBadge, { backgroundColor: op.status === 'On Track' ? '#34C75920' : '#FF950020' }]}>
                  <Text style={[styles.statusText, { color: op.status === 'On Track' ? '#34C759' : '#FF9500' }]}>{op.status}</Text>
                </View>
              </View>
              <Text style={styles.txnDesc}>{op.type}</Text>
              <View style={styles.txnFooter}>
                <Text style={styles.txnDate}>{op.date}</Text>
                <Text style={styles.txnAmount}>{op.tasks} tasks</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          {[
            { endpoint: '/consult/operations', desc: 'Consult on operations' },
            { endpoint: '/operations/manage', desc: 'Manage operations' },
            { endpoint: '/operations/optimize', desc: 'Optimize processes' },
            { endpoint: '/operations/report', desc: 'Generate reports' },
          ].map((item, i) => (
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Auto Optimize</Text>
              <Text style={styles.featureToggleDesc}>Auto-optimize operations</Text>
            </View>
            <Switch value={autoOptimize} onValueChange={setAutoOptimize} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Team Coordination</Text>
              <Text style={styles.featureToggleDesc}>Enable team sync</Text>
            </View>
            <Switch value={teamCoordination} onValueChange={setTeamCoordination} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Resource Allocation</Text>
              <Text style={styles.featureToggleDesc}>Auto-allocate resources</Text>
            </View>
            <Switch value={resourceAllocation} onValueChange={setResourceAllocation} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
        </View>
      )}

      <AgentFeatures agentId="operations-manager" agentName="AI Operations Manager" />
