...
import { Building, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Settings, Thermometer, LayoutGrid, BarChart3, Shield, Users, ChevronRight, Wrench, Gauge, MapPin, Lightbulb, TrendingUp, AlertTriangle, CheckCircle2 } from 'lucide-react-native';
...
export default function VPFacilitiesPage() {
  ...
  const stats = [
    { label: 'Tasks Completed', value: '2,324', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response Time', value: '0.5s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.3%', icon: Target, color: '#4E342E' },
  ];

  const facilitiesMetrics = [
    { label: 'Space Utilization', value: '87%', trend: '+3.2%', icon: LayoutGrid, color: '#007AFF' },
    { label: 'Energy Efficiency', value: '94%', trend: '+1.8%', icon: Thermometer, color: '#34C759' },
    { label: 'Maintenance SLA', value: '99.2%', trend: '+0.5%', icon: Wrench, color: '#FF9500' },
    { label: 'Cost Savings', value: '$1.2M', trend: '+12%', icon: TrendingUp, color: '#4E342E' },
  ];

  const capabilities = [
    'Space Planning & Optimization', 'Preventive Maintenance Scheduling', 'Energy Management & Monitoring',
    'Facility Budget Management', 'Safety & Compliance Oversight', 'Vendor & Contractor Management',
    'Capital Improvement Planning', 'Environmental Sustainability', 'Building Systems Integration',
    'Occupancy Analytics', 'Emergency Response Coordination', 'Lease Administration'
  ];

  const responsibilities = [
    'Develop and execute facility management strategy across all locations',
    'Monitor and optimize space utilization with AI-driven analytics',
    'Schedule and track preventive and corrective maintenance programs',
    'Manage energy consumption and drive sustainability initiatives',
    'Oversee facility budgets and capital improvement projects',
    'Ensure safety compliance and emergency preparedness protocols',
    'Coordinate vendor relationships and contractor performance',
    'Drive environmental sustainability and green building certifications'
  ];

  const activities = [
    { time: '2 min ago', text: 'Optimized HVAC scheduling across 3 buildings — 18% energy reduction', icon: Thermometer, type: 'success' },
    { time: '8 min ago', text: 'Auto-scheduled 12 preventive maintenance tasks for Building A', icon: Wrench, type: 'info' },
    { time: '15 min ago', text: 'Detected space utilization anomaly in Floor 3 — reallocating 4,200 sq ft', icon: LayoutGrid, type: 'warning' },
    { time: '22 min ago', text: 'Completed quarterly facility compliance audit — 100% pass rate', icon: Shield, type: 'success' },
    { time: '35 min ago', text: 'Generated capital improvement forecast for FY2026 — $2.4M planned', icon: TrendingUp, type: 'info' },
  ];

  const subAgents = [
    { id: 'space-utilization-analyst', name: 'AI Space Utilization Analyst', icon: LayoutGrid, desc: 'Real-time space analytics & optimization', color: '#007AFF', route: '/ai-agent/operations/sub-agents/space-utilization-analyst' },
    { id: 'maintenance-scheduler', name: 'AI Maintenance Scheduler', icon: Wrench, desc: 'Preventive & corrective maintenance automation', color: '#FF9500', route: '/ai-agent/operations/sub-agents/maintenance-scheduler' },
    { id: 'energy-efficiency-monitor', name: 'AI Energy Efficiency Monitor', icon: Thermometer, desc: 'Energy monitoring & sustainability tracking', color: '#34C759', route: '/ai-agent/operations/sub-agents/energy-efficiency-monitor' },
  ];

  const kpiData = [
    { metric: 'Building Occupancy Rate', value: '82%', target: '85%', status: 'on-track' },
    { metric: 'Work Order Completion', value: '97%', target: '95%', status: 'exceeding' },
    { metric: 'Energy Cost per Sq Ft', value: '$2.18', target: '$2.50', status: 'exceeding' },
    { metric: 'Tenant Satisfaction', value: '4.6/5', target: '4.5/5', status: 'exceeding' },
    { metric: 'Emergency Response Time', value: '2.1 min', target: '3 min', status: 'exceeding' },
    { metric: 'Green Certification Score', value: '78/100', target: '80/100', status: 'on-track' },
  ];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Facilities Metrics</Text>
        <View style={styles.metricsGrid}>
          {facilitiesMetrics.map((m, i) => (
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI VP Facilities leads enterprise facility management strategy, optimizing space utilization, maintenance operations, and energy efficiency across all organizational locations. Leveraging AI-driven analytics, it ensures maximum operational efficiency while reducing costs and maintaining full regulatory compliance.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.sectionSubtitle, { color: theme.colors.textSecondary }]}>3 specialized agents under VP Facilities</Text>
        {subAgents.map((agent) => (
  ...
  const renderSubAgents = () => (
    <View style={styles.section}>
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>VP Facilities — Sub-Agent Workforce</Text>
      <Text style={[styles.sectionSubtitle, { color: theme.colors.textSecondary }]}>3 specialized AI agents executing facilities operations</Text>
      {subAgents.map((agent) => (
        <TouchableOpacity key={agent.id} style={[styles.subAgentFullCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]} onPress={() => router.push(agent.route as any)}>
          ...
          <View style={styles.subAgentFullStats}>
            <View style={[styles.subAgentStat, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <Activity size={14} color="#34C759" />
              <Text style={[styles.subAgentStatText, { color: theme.colors.text }]}>Active</Text>
            </View>
            <View style={[styles.subAgentStat, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <Gauge size={14} color="#007AFF" />
              <Text style={[styles.subAgentStatText, { color: theme.colors.text }]}>20x Efficiency</Text>
            </View>
            <View style={[styles.subAgentStat, { backgroundColor: theme.colors.background || '#F2F2F7' }]}>
              <Shield size={14} color="#FF9500" />
              <Text style={[styles.subAgentStatText, { color: theme.colors.text }]}>Enterprise</Text>
            </View>
          </View>
          <View style={styles.subAgentFullActions}>
            <TouchableOpacity style={[styles.subAgentAction, { backgroundColor: agent.color }]} onPress={() => router.push(agent.route as any)}>
              ...
              <Text style={styles.subAgentActionText}>Configure</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.subAgentActionOutline, { borderColor: agent.color }]} onPress={() => router.push(`/ai-agent/agent-configuration?id=${agent.id}` as any)}>
              <BarChart3 size={14} color={agent.color} />
              <Text style={[styles.subAgentActionTextOutline, { color: agent.color }]}>Analytics</Text>
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      ))}
  ...
  const renderKPIs = () => (
    <View style={styles.section}>
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Facilities KPI Dashboard</Text>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#4E342E20' }]}>
          <Building size={48} color="#4E342E" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI VP Facilities</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Operations — VP Level</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#4E342E22' }]}><Star size={12} color="#4E342E" /><Text style={[styles.badgeText, { color: '#4E342E' }]}>VP Level</Text></View>
      ...
      <View style={styles.tabBar}>
        {tabs.map((tab) => (
          <TouchableOpacity
            key={tab.id}
            style={[styles.tab, activeTab === tab.id && styles.tabActive, { borderBottomColor: activeTab === tab.id ? '#4E342E' : 'transparent' }]}
