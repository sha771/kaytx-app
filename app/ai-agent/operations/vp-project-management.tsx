...
import { ClipboardList, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, Settings, Users, ChevronRight, Shield, Gauge, BarChart3, TrendingUp, AlertTriangle, CheckCircle2, Flag, Calendar, Box, CircleAlert } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function VPProjectManagementPage() {
  const { theme } = useTheme();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState('overview');

  const stats = [
    { label: 'Projects Managed', value: '2,900', icon: ClipboardList, color: '#34C759' },
    { label: 'On-Time Delivery', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response Time', value: '0.5s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '99.7%', icon: Target, color: '#4E342E' },
  ];

  const projectMetrics = [
    { label: 'Milestone Completion', value: '96%', trend: '+4.1%', icon: Flag, color: '#007AFF' },
    { label: 'Resource Utilization', value: '91%', trend: '+2.8%', icon: Box, color: '#34C759' },
    { label: 'Risk Mitigation', value: '94%', trend: '+5.2%', icon: CircleAlert, color: '#FF9500' },
    { label: 'Budget Adherence', value: '98%', trend: '+1.5%', icon: TrendingUp, color: '#4E342E' },
  ];

  const capabilities = [
    'Project Portfolio Management', 'Milestone Tracking & Reporting', 'Resource Allocation & Optimization',
    'Risk Identification & Mitigation', 'Budget & Cost Management', 'Stakeholder Communication',
    'Gantt Chart & Timeline Management', 'Agile & Waterfall Methodologies', 'Cross-Project Dependency Management',
    'Scope Change Management', 'Quality Gate Enforcement', 'Post-Project Review & Lessons Learned'
  ];

  const responsibilities = [
    'Oversee enterprise project portfolio and strategic initiative delivery',
    'Track milestones across all active projects with AI-driven forecasting',
    'Optimize resource allocation across teams and departments',
    'Identify and mitigate project risks before they impact delivery',
    'Ensure budget adherence and cost optimization across projects',
    'Coordinate stakeholder communication and status reporting',
    'Enforce quality gates and project governance standards',
    'Drive continuous improvement through post-project analysis'
  ];

  const activities = [
    { time: '1 min ago', text: 'Milestone "API Integration" completed 2 days ahead of schedule', icon: Flag, type: 'success' },
    { time: '5 min ago', text: 'Reallocated 3 engineers from Project Alpha to Project Beta — 15% efficiency gain', icon: Box, type: 'info' },
    { time: '12 min ago', text: 'Risk alert: Project Gamma vendor delay — auto-generated mitigation plan', icon: CircleAlert, type: 'warning' },
    { time: '18 min ago', text: 'Budget forecast updated for Q2 — $340K under projection', icon: TrendingUp, type: 'success' },
    { time: '30 min ago', text: 'Generated stakeholder status report for 8 active projects', icon: ClipboardList, type: 'info' },
  ];

  const subAgents = [
    { id: 'milestone-tracker', name: 'AI Milestone Tracker', icon: Flag, desc: 'Track & forecast project milestones', color: '#007AFF', route: '/ai-agent/operations/sub-agents/milestone-tracker' },
    { id: 'resource-allocator', name: 'AI Resource Allocator', icon: Box, desc: 'Optimize resource allocation across projects', color: '#34C759', route: '/ai-agent/operations/sub-agents/resource-allocator' },
    { id: 'risk-identifier', name: 'AI Risk Identifier', icon: CircleAlert, desc: 'Proactive risk detection & mitigation', color: '#FF9500', route: '/ai-agent/operations/sub-agents/risk-identifier' },
  ];

  const kpiData = [
    { metric: 'Project On-Time Rate', value: '94%', target: '90%', status: 'exceeding' },
    { metric: 'Resource Utilization', value: '91%', target: '85%', status: 'exceeding' },
    { metric: 'Budget Variance', value: '+2.1%', target: '±5%', status: 'exceeding' },
    { metric: 'Scope Change Frequency', value: '1.2/week', target: '<2/week', status: 'exceeding' },
    { metric: 'Stakeholder Satisfaction', value: '4.7/5', target: '4.5/5', status: 'exceeding' },
    { metric: 'Risk Mitigation Rate', value: '89%', target: '85%', status: 'exceeding' },
  ];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Project Metrics</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI VP Project Management orchestrates the enterprise project portfolio, ensuring strategic initiatives are delivered on time, within budget, and to specification. Leveraging AI-driven forecasting and risk analysis, it proactively identifies issues and optimizes resource allocation across all active projects.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.sectionSubtitle, { color: theme.colors.textSecondary }]}>3 specialized agents under VP Project Management</Text>
        {subAgents.map((agent) => (
  ...
  const renderSubAgents = () => (
    <View style={styles.section}>
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>VP Project Management — Sub-Agent Workforce</Text>
      <Text style={[styles.sectionSubtitle, { color: theme.colors.textSecondary }]}>3 specialized AI agents executing project operations</Text>
      {subAgents.map((agent) => (
  ...
  const renderKPIs = () => (
    <View style={styles.section}>
      <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Project Management KPI Dashboard</Text>
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#4E342E20' }]}>
          <ClipboardList size={48} color="#4E342E" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI VP Project Management</Text>
      ...
      <AgentFeatures agentId="vp-project-management" agentName="AI VP Project Management" />
