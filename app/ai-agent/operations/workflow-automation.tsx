...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, User, Shield, GitBranch, PlayCircle } from 'lucide-react-native';
...
export default function WorkflowAutomationPage() {
  ...
  const [errorRecovery, setErrorRecovery] = useState(true);
  const [analytics, setAnalytics] = useState(true);

  const stats = [{label:'Tasks/Day',value:'140',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.1s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.0%',icon:Target,color:'#4E342E'}];
  const capabilities = ['Workflow Automation','Task Reduction','Efficiency Improvement'];
  ...
  const activities = [{time:'3 min ago',text:'Automated 15 workflows',icon: CircleCheckBig},{time:'10 min ago',text:'Reduced manual tasks',icon:Clock},{time:'25 min ago',text:'Optimized process flow',icon:Zap}];

  const workflows = [
    { id: 'WF-001', name: 'Invoice Processing', runs: 45, success: '98%', status: 'Active', date: 'Today' },
    { id: 'WF-002', name: 'Onboarding Flow', runs: 12, success: '100%', status: 'Active', date: 'Today' },
    { id: 'WF-003', name: 'Approval Chain', runs: 28, success: '95%', status: 'Paused', date: 'Yesterday' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Workflow Automation Agent</Text>
      ...
      {activeTab === 'overview' && (
        <>
          <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Automates business workflows, reduces manual tasks, and improves efficiency.</Text></View>
      ...
      {activeTab === 'workflows' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Workflow Status</Text>
          {workflows.map((wf, i) => (
            <View key={i} style={styles.txnCard}>
              <View style={styles.txnHeader}>
                <GitBranch size={18} color="#4E342E" />
                <Text style={styles.txnId}>{wf.id}</Text>
                <View style={[styles.statusBadge, { backgroundColor: wf.status === 'Active' ? '#34C75920' : '#FF950020' }]}>
                  <Text style={[styles.statusText, { color: wf.status === 'Active' ? '#34C759' : '#FF9500' }]}>{wf.status}</Text>
                </View>
              </View>
              <Text style={styles.txnDesc}>{wf.name}</Text>
              <View style={styles.txnFooter}>
                <Text style={styles.txnDate}>{wf.date}</Text>
                <Text style={styles.txnAmount}>{wf.runs} runs</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          {[
            { endpoint: '/consult/workflow', desc: 'Consult on workflows' },
            { endpoint: '/workflow/automate', desc: 'Automate workflows' },
            { endpoint: '/workflow/trigger', desc: 'Trigger workflows' },
            { endpoint: '/workflow/report', desc: 'Generate reports' },
          ].map((item, i) => (
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Auto Trigger</Text>
              <Text style={styles.featureToggleDesc}>Auto-trigger workflows</Text>
            </View>
            <Switch value={autoTrigger} onValueChange={setAutoTrigger} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Error Recovery</Text>
              <Text style={styles.featureToggleDesc}>Auto-recover from errors</Text>
            </View>
            <Switch value={errorRecovery} onValueChange={setErrorRecovery} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Analytics</Text>
              <Text style={styles.featureToggleDesc}>Track workflow analytics</Text>
            </View>
            <Switch value={analytics} onValueChange={setAnalytics} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
        </View>
      )}

      <AgentFeatures agentId="workflow-automation" agentName="AI Workflow Automation Agent" />
