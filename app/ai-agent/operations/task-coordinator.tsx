...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, User, Shield, CheckSquare, ListTodo } from 'lucide-react-native';
...
export default function TaskCoordinatorPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'75',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.6%',icon:Target,color:'#4E342E'}];
  const capabilities = ['Task Coordination','Priority Management','Timely Completion'];
  ...
  const activities = [{time:'3 min ago',text:'Coordinated 12 tasks',icon: CircleCheckBig},{time:'10 min ago',text:'Prioritized workload',icon:Clock},{time:'25 min ago',text:'Updated task status',icon:Zap}];

  const tasks = [
    { id: 'TSK-001', name: 'Complete quarterly report', priority: 'High', status: 'In Progress', due: 'Today' },
    { id: 'TSK-002', name: 'Review vendor contracts', priority: 'Medium', status: 'Pending', due: 'Tomorrow' },
    { id: 'TSK-003', name: 'Team meeting preparation', priority: 'Low', status: 'Completed', due: 'Yesterday' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Task Coordinator</Text>
      ...
      <View style={styles.tabContainer}>
        {['overview', 'tasks', 'capabilities', 'settings'].map((tab) => (
      ...
      {activeTab === 'overview' && (
        <>
          <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Coordinates tasks across teams, manages priorities, and ensures timely completion.</Text></View>
      ...
      {activeTab === 'tasks' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Task Status</Text>
          {tasks.map((task, i) => (
            <View key={i} style={styles.txnCard}>
              <View style={styles.txnHeader}>
                <CheckSquare size={18} color="#4E342E" />
                <Text style={styles.txnId}>{task.id}</Text>
                <View style={[styles.statusBadge, { backgroundColor: task.status === 'Completed' ? '#34C75920' : task.status === 'In Progress' ? '#007AFF20' : '#FF950020' }]}>
                  <Text style={[styles.statusText, { color: task.status === 'Completed' ? '#34C759' : task.status === 'In Progress' ? '#007AFF' : '#FF9500' }]}>{task.status}</Text>
                </View>
              </View>
              <Text style={styles.txnDesc}>{task.name}</Text>
              <View style={styles.txnFooter}>
                <Text style={styles.txnDate}>{task.due}</Text>
                <Text style={[styles.txnAmount, { color: task.priority === 'High' ? '#FF3B30' : task.priority === 'Medium' ? '#FF9500' : '#34C759' }]}>{task.priority}</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          {[
            { endpoint: '/consult/task', desc: 'Consult on tasks' },
            { endpoint: '/task/coordinate', desc: 'Coordinate tasks' },
            { endpoint: '/task/prioritize', desc: 'Prioritize tasks' },
            { endpoint: '/task/report', desc: 'Generate reports' },
          ].map((item, i) => (
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Auto Assign</Text>
              <Text style={styles.featureToggleDesc}>Auto-assign tasks</Text>
            </View>
            <Switch value={autoAssign} onValueChange={setAutoAssign} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Priority Routing</Text>
              <Text style={styles.featureToggleDesc}>Route by priority</Text>
            </View>
            <Switch value={priorityRouting} onValueChange={setPriorityRouting} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Deadline Tracking</Text>
              <Text style={styles.featureToggleDesc}>Track deadlines</Text>
            </View>
            <Switch value={deadlineTracking} onValueChange={setDeadlineTracking} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
        </View>
      )}

      <AgentFeatures agentId="task-coordinator" agentName="AI Task Coordinator" />
