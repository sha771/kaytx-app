...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, ChartBarBig, CheckCircle2, AlertTriangle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function EfficiencyReporterPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Operations Manager (Sub)', icon: ChartBarBig, color: '#007AFF' },
  ];

  const capabilities = ["Efficiency Score Calculation","Throughput Analysis","Resource Utilization Reporting","Cost-Per-Unit Tracking","Benchmark Comparison","Improvement Trend Analysis","Automated Insights Generation","ROI Calculation"];
  const kpiData = [{"metric":"Efficiency Score","value":"92/100","target":"85/100","status":"exceeding"},{"metric":"Throughput Gain","value":"+8.2%","target":"+5%","status":"exceeding"},{"metric":"Cost Reduction","value":"14%","target":"10%","status":"exceeding"},{"metric":"Insight Speed","value":"6s","target":"20s","status":"exceeding"}];
  const activities = [{"time":"3 min ago","text":"Efficiency report: Operations throughput +8.2%","icon":"ChartBarBig","type":"success"},{"time":"12 min ago","text":"Cost-per-unit: 14% reduction in customer onboarding","icon":"TrendingUp","type":"info"},{"time":"25 min ago","text":"Benchmark: 4 processes above industry median","icon":"BarChart3","type":"warning"}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}><View style={[styles.heroIconWrap, { backgroundColor: '#34C75920' }]}><ChartBarBig size={56} color="#34C759" /></View><Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Efficiency Reporter</Text><Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Sub-Agent of AI Operations Manager (Sub)</Text><View style={styles.badgesRow}><View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View><View style={[styles.badge, { backgroundColor: '#607D8B22' }]}><Briefcase size={12} color="#607D8B" /><Text style={[styles.badgeText, { color: '#607D8B' }]}>Specialist</Text></View></View></View>
      <View style={styles.tabBar}>{tabs.map((tab) => (<TouchableOpacity key={tab.id} style={[styles.tab, activeTab === tab.id && styles.tabActive, { borderBottomColor: activeTab === tab.id ? '#34C759' : 'transparent' }]} onPress={() => setActiveTab(tab.id)}><Text style={[styles.tabText, { color: activeTab === tab.id ? '#34C759' : theme.colors.textSecondary }]}>{tab.label}</Text></TouchableOpacity>))}</View>
      {activeTab === 'overview' && renderOverview()}
      {activeTab === 'kpi' && renderKPIs()}
      {activeTab === 'activity' && renderActivity()}
      <AgentFeatures agentId="efficiency-reporter" agentName="AI Efficiency Reporter" />
