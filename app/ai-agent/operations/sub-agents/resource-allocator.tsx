...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, Users, CheckCircle2, AlertTriangle } from 'lucide-react-native';
...
export default function ResourceAllocatorPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI VP Project Management', icon: Users, color: '#007AFF' },
  ];

  const capabilities = ["Smart Resource Matching","Capacity Planning","Skill-Based Assignment","Workload Balancing","Cross-Project Allocation","Availability Forecasting","Cost-Optimized Staffing","Conflict Resolution"];
  const kpiData = [{"metric":"Utilization Rate","value":"89%","target":"85%","status":"exceeding"},{"metric":"Assignment Accuracy","value":"93%","target":"88%","status":"exceeding"},{"metric":"Workload Balance","value":"4.3/5","target":"3.5/5","status":"exceeding"},{"metric":"Allocation Speed","value":"12min","target":"30min","status":"exceeding"}];
  const activities = [{"time":"3 min ago","text":"Allocated 8 engineers to Sprint 15 — 93% skill match","icon":"Users","type":"success"},{"time":"10 min ago","text":"Workload imbalance: Team Alpha at 110% — rebalanced","icon":"AlertTriangle","type":"warning"},{"time":"22 min ago","text":"Capacity forecast: Q3 needs 12 additional developers","icon":"TrendingUp","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Resource Allocator is a specialized sub-agent supporting AI VP Project Management. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="resource-allocator" agentName="AI Resource Allocator" />
