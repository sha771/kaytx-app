...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, FileCode, CheckCircle2, AlertTriangle } from 'lucide-react-native';
...
export default function AutomationRuleBuilderPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Workflow Automation Agent', icon: FileCode, color: '#007AFF' },
  ];

  const capabilities = ["Visual Rule Builder","Conditional Logic Engine","Template Library","Rule Testing & Validation","Version Control","Conflict Detection","Performance Optimization","Audit Trail Generation"];
  const kpiData = [{"metric":"Rules Built","value":"1,240","target":"800","status":"exceeding"},{"metric":"Execution Accuracy","value":"99.4%","target":"99%","status":"exceeding"},{"metric":"Conflict Detection","value":"100%","target":"95%","status":"exceeding"},{"metric":"Build Time Reduction","value":"68%","target":"50%","status":"exceeding"}];
  const activities = [{"time":"3 min ago","text":"Built 18 automation rules for invoice processing","icon":"FileCode","type":"success"},{"time":"12 min ago","text":"Rule conflict detected in HR workflow — auto-resolved","icon":"AlertTriangle","type":"warning"},{"time":"25 min ago","text":"Template published: 5 new reusable automation templates","icon":"CheckCircle2","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Automation Rule Builder is a specialized sub-agent supporting AI Workflow Automation Agent. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="automation-rule-builder" agentName="AI Automation Rule Builder" />
