...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, Calendar, CheckCircle2, AlertTriangle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function DailyOperationsCoordinatorPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Operations Manager', icon: Calendar, color: '#007AFF' },
  ];

  const capabilities = ["Daily Task Scheduling","Shift Coordination","Operational Handoff Management","Priority Queue Management","Cross-Team Synchronization","Status Board Management","Morning Briefing Generation","End-of-Day Reporting"];
  const kpiData = [{"metric":"Task Completion","value":"97%","target":"93%","status":"exceeding"},{"metric":"Handoff Accuracy","value":"99%","target":"95%","status":"exceeding"},{"metric":"Sync Score","value":"4.5/5","target":"4.0/5","status":"exceeding"},{"metric":"Briefing Speed","value":"8s","target":"30s","status":"exceeding"}];
  const activities = [{"time":"1 min ago","text":"Morning briefing: 42 tasks, 6 critical, 3 cross-team","icon":"Calendar","type":"success"},{"time":"15 min ago","text":"Shift handoff completed: 0 items dropped","icon":"CheckCircle2","type":"success"},{"time":"30 min ago","text":"Priority queue: 3 urgent items escalated","icon":"ArrowRight","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Daily Operations Coordinator is a specialized sub-agent supporting AI Operations Manager. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="daily-operations-coordinator" agentName="AI Daily Operations Coordinator" />
