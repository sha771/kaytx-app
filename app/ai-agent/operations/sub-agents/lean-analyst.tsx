...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, Lightbulb, CheckCircle2, AlertTriangle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function LeanAnalystPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Process Optimization Agent', icon: Lightbulb, color: '#007AFF' },
  ];

  const capabilities = ["Lean Methodology Analysis","Value Stream Mapping","Non-Value-Add Detection","Process Flow Scoring","Kaizen Event Support","Muda Classification","Standard Work Documentation","Lean Maturity Assessment"];
  const kpiData = [{"metric":"Lean Score","value":"92/100","target":"80/100","status":"exceeding"},{"metric":"NVA Detection","value":"96%","target":"88%","status":"exceeding"},{"metric":"Flow Improvement","value":"+22%","target":"+15%","status":"exceeding"},{"metric":"Maturity Score","value":"4.2/5","target":"3.5/5","status":"exceeding"}];
  const activities = [{"time":"1 min ago","text":"Lean analysis: Procurement cycle — 34% waste identified","icon":"Lightbulb","type":"success"},{"time":"8 min ago","text":"Value stream mapped: 5 non-value-add steps found","icon":"ArrowRight","type":"info"},{"time":"18 min ago","text":"Muda classification: 8 wastes catalogued","icon":"BarChart3","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Lean Analyst is a specialized sub-agent supporting AI Process Optimization Agent. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="lean-analyst" agentName="AI Lean Analyst" />
