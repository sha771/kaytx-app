...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, CheckCircle2, AlertTriangle } from 'lucide-react-native';
...
export default function BottleneckDetectorPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Operations Manager (Sub)', icon: Zap, color: '#007AFF' },
  ];

  const capabilities = ["Real-Time Bottleneck Detection","Queue Depth Analysis","Resource Saturation Monitoring","Auto-Scaling Recommendations","Process Flow Analysis","Capacity Threshold Alerts","Root Cause Identification","Predictive Forecasting"];
  const kpiData = [{"metric":"Detection Accuracy","value":"94%","target":"85%","status":"exceeding"},{"metric":"Time to Detection","value":"8s","target":"30s","status":"exceeding"},{"metric":"Auto-Resolution","value":"72%","target":"60%","status":"exceeding"},{"metric":"False Positive","value":"3%","target":"8%","status":"exceeding"}];
  const activities = [{"time":"2 min ago","text":"Bottleneck: Invoice approval queue at 94% — auto-scaled","icon":"Zap","type":"warning"},{"time":"10 min ago","text":"Queue normalized: 12 agents added to support tickets","icon":"CheckCircle2","type":"success"},{"time":"22 min ago","text":"Predictive: API gateway saturation in 4h — pre-scaling initiated","icon":"TrendingUp","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Bottleneck Detector is a specialized sub-agent supporting AI Operations Manager (Sub). It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="bottleneck-detector" agentName="AI Bottleneck Detector" />
