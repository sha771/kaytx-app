...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, Sparkles, CheckCircle2, AlertTriangle } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function ImprovementRecommenderPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Process Optimization Agent', icon: Sparkles, color: '#007AFF' },
  ];

  const capabilities = ["AI-Driven Recommendations","Impact Projection","ROI Calculation","Implementation Roadmap","Priority Ranking","Quick Win Identification","Best Practice Matching","Change Impact Assessment"];
  const kpiData = [{"metric":"Acceptance Rate","value":"89%","target":"75%","status":"exceeding"},{"metric":"Implementation Success","value":"84%","target":"70%","status":"exceeding"},{"metric":"Average ROI","value":"340%","target":"200%","status":"exceeding"},{"metric":"Quick Wins","value":"42/quarter","target":"20/quarter","status":"exceeding"}];
  const activities = [{"time":"3 min ago","text":"Generated 8 recommendations for supply chain — $420K savings","icon":"Sparkles","type":"success"},{"time":"12 min ago","text":"Quick win: Automate email triage — 120h/month saved","icon":"Zap","type":"info"},{"time":"25 min ago","text":"Roadmap: 6 improvements prioritized for Q3","icon":"ArrowRight","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Improvement Recommender is a specialized sub-agent supporting AI Process Optimization Agent. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="improvement-recommender" agentName="AI Improvement Recommender" />
