...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, Trash2, CheckCircle2, AlertTriangle } from 'lucide-react-native';
...
export default function WasteIdentifierPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Process Optimization Agent', icon: Trash2, color: '#007AFF' },
  ];

  const capabilities = ["Waste Detection & Classification","Overprocessing Identification","Waiting Time Analysis","Defect Rate Tracking","Inventory Waste Detection","Motion Waste Analysis","Transport Waste Mapping","Overproduction Alerting"];
  const kpiData = [{"metric":"Reduction Rate","value":"34%","target":"25%","status":"exceeding"},{"metric":"Detection Accuracy","value":"95%","target":"88%","status":"exceeding"},{"metric":"Elimination Rate","value":"87%","target":"75%","status":"exceeding"},{"metric":"Cost Impact","value":"$890K saved","target":"$500K saved","status":"exceeding"}];
  const activities = [{"time":"2 min ago","text":"Eliminated 12 non-value-add steps from onboarding","icon":"Trash2","type":"success"},{"time":"10 min ago","text":"Overprocessing: 3 redundant approval steps in procurement","icon":"AlertTriangle","type":"warning"},{"time":"22 min ago","text":"Waiting time reduced: Customer support queue 45% improvement","icon":"TrendingUp","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Waste Identifier is a specialized sub-agent supporting AI Process Optimization Agent. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>A2A Endpoints</Text>{['/consult/waste-identifier', '/waste-identifier/execute', '/waste-identifier/analyze'].map((ep, i) => (<View key={i} style={styles.endpointRow}><Zap size={14} color="#8B5CF6" /><Text style={[styles.endpointText, { color: theme.colors.textSecondary }]}>{ep}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Parent Agent</Text><TouchableOpacity onPress={() => router.push('/ai-agent/operations/ai-process-optimization' as any)} style={[styles.parentCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><Trash2 size={24} color="#34C759" /><View style={styles.parentInfo}><Text style={[styles.parentName, { color: theme.colors.text }]}>AI Process Optimization Agent</Text><Text style={[styles.parentDesc, { color: theme.colors.textSecondary }]}>Parent Agent</Text></View><ChevronRight size={20} color={theme.colors.textSecondary} /></TouchableOpacity></View>
    </>
  );
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="waste-identifier" agentName="AI Waste Identifier" />
