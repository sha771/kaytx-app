...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, Eye, CheckCircle2, AlertTriangle } from 'lucide-react-native';
...
export default function WorkflowMonitorPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Operations Manager (Sub)', icon: Eye, color: '#007AFF' },
  ];

  const capabilities = ["Real-Time Workflow Monitoring","Health Score Calculation","Anomaly Detection","Process Flow Visualization","SLA Compliance Tracking","Alert Generation","Cross-System Monitoring","Automated Status Reporting"];
  const kpiData = [{"metric":"Health Score","value":"99.2%","target":"98%","status":"exceeding"},{"metric":"Anomaly Detection","value":"96%","target":"90%","status":"exceeding"},{"metric":"Alert Accuracy","value":"93%","target":"85%","status":"exceeding"},{"metric":"Coverage","value":"100%","target":"98%","status":"exceeding"}];
  const activities = [{"time":"1 min ago","text":"Health check: 142 workflows — 99.2% healthy","icon":"Eye","type":"success"},{"time":"6 min ago","text":"Anomaly: Unusual API latency spike — root cause in 45s","icon":"AlertTriangle","type":"warning"},{"time":"15 min ago","text":"SLA compliance: 99.4% across all monitored workflows","icon":"BarChart3","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Workflow Monitor is a specialized sub-agent supporting AI Operations Manager (Sub). It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="workflow-monitor" agentName="AI Workflow Monitor" />
