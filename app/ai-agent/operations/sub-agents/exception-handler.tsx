...
import { Activity, Clock, Target, Zap, ArrowRight, Briefcase, ChevronRight, BarChart3, TrendingUp, CircleAlert, CheckCircle2, AlertTriangle } from 'lucide-react-native';
...
export default function ExceptionHandlerPage() {
  ...
  const stats = [
    ...
    { label: 'Parent', value: 'AI Workflow Automation Agent', icon: CircleAlert, color: '#007AFF' },
  ];

  const capabilities = ["Exception Detection & Classification","Auto-Remediation Engine","Fallback Workflow Triggering","Error Recovery & Retry","Human Escalation Routing","Exception Pattern Analysis","Root Cause Identification","Resilience Testing"];
  const kpiData = [{"metric":"Resolution Rate","value":"99.4%","target":"98%","status":"exceeding"},{"metric":"Auto-Remediation","value":"82%","target":"70%","status":"exceeding"},{"metric":"Recovery Time","value":"2.8min","target":"10min","status":"exceeding"},{"metric":"Pattern Detection","value":"91%","target":"80%","status":"exceeding"}];
  const activities = [{"time":"1 min ago","text":"Exception resolved: Payment gateway timeout — auto-retry succeeded","icon":"CircleAlert","type":"success"},{"time":"8 min ago","text":"Fallback triggered: Primary API unavailable","icon":"Zap","type":"warning"},{"time":"18 min ago","text":"Pattern: 3 similar exceptions — root cause: DNS cache expiry","icon":"Eye","type":"info"}];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>AI Exception Handler is a specialized sub-agent supporting AI Workflow Automation Agent. It provides enterprise-level automation and intelligence for its domain, delivering consistent high-quality performance as part of the Kaytx AI Workforce.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="exception-handler" agentName="AI Exception Handler" />
