...
import { Zap, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Settings, Users, ChevronRight, Shield, Gauge, BarChart3, TrendingUp, AlertTriangle, CheckCircle2, Lightbulb, Trash2, Sparkles } from 'lucide-react-native';
...
export default function AIProcessOptimizationPage() {
  ...
  const stats = [
    { label: 'Processes Optimized', value: '2,077', icon: CircleCheckBig, color: '#34C759' },
    { label: 'Uptime', value: '99.9%', icon: Activity, color: '#007AFF' },
    { label: 'Response Time', value: '0.6s', icon: Clock, color: '#FF9500' },
    { label: 'Accuracy', value: '98.5%', icon: Target, color: '#4E342E' },
  ];

  const processMetrics = [
    { label: 'Lean Analysis Score', value: '92/100', trend: '+8', icon: Lightbulb, color: '#007AFF' },
    { label: 'Waste Reduction', value: '34%', trend: '+5.2%', icon: Trash2, color: '#34C759' },
    { label: 'Improvements Made', value: '847', trend: '+14%', icon: Sparkles, color: '#FF9500' },
    { label: 'Cost Savings', value: '$2.1M', trend: '+18%', icon: TrendingUp, color: '#4E342E' },
  ];

  const capabilities = [
    'Lean Methodology Analysis', 'Waste Identification & Elimination', 'Improvement Recommendation Engine',
    'Process Benchmarking', 'Value Stream Mapping', 'Kaizen Event Coordination',
    'Six Sigma Integration', 'Root Cause Analysis', 'Statistical Process Control',
    'Continuous Improvement Framework', 'Change Impact Assessment', 'ROI Calculation & Tracking'
  ];

  const responsibilities = [
    'Analyze processes using lean methodology to identify inefficiencies',
    'Identify and eliminate waste across all operational processes',
    'Generate AI-driven improvement recommendations with impact projections',
    'Benchmark processes against industry standards and best practices',
    'Map value streams and optimize flow across the enterprise',
    'Coordinate kaizen events and continuous improvement initiatives',
    'Perform root cause analysis for recurring operational issues',
    'Track ROI of all process improvements and optimization initiatives'
  ];

  const activities = [
    { time: '1 min ago', text: 'Lean analysis complete: Procurement cycle — 34% waste identified', icon: Lightbulb, type: 'success' },
    { time: '5 min ago', text: 'Eliminated 12 non-value-add steps from customer onboarding flow', icon: Trash2, type: 'info' },
    { time: '12 min ago', text: 'Generated 8 improvement recommendations for supply chain — $420K projected savings', icon: Sparkles, type: 'success' },
    { time: '18 min ago', text: 'Benchmark report: 6 processes below industry median — action plans created', icon: BarChart3, type: 'warning' },
    { time: '28 min ago', text: 'Kaizen event completed: Manufacturing line throughput +22%', icon: Zap, type: 'success' },
  ];

  const subAgents = [
    { id: 'lean-analyst', name: 'AI Lean Analyst', icon: Lightbulb, desc: 'Lean methodology analysis & scoring', color: '#007AFF', route: '/ai-agent/operations/sub-agents/lean-analyst' },
    { id: 'waste-identifier', name: 'AI Waste Identifier', icon: Trash2, desc: 'Identify & eliminate process waste', color: '#34C759', route: '/ai-agent/operations/sub-agents/waste-identifier' },
    { id: 'improvement-recommender', name: 'AI Improvement Recommender', icon: Sparkles, desc: 'AI-driven improvement recommendations', color: '#FF9500', route: '/ai-agent/operations/sub-agents/improvement-recommender' },
  ];

  const kpiData = [
    { metric: 'Process Efficiency Score', value: '92/100', target: '85/100', status: 'exceeding' },
    { metric: 'Waste Reduction Rate', value: '34%', target: '25%', status: 'exceeding' },
    { metric: 'Improvement Implementation Rate', value: '89%', target: '80%', status: 'exceeding' },
    { metric: 'ROI on Improvements', value: '340%', target: '200%', status: 'exceeding' },
    { metric: 'Lean Maturity Score', value: '4.2/5', target: '3.5/5', status: 'exceeding' },
    { metric: 'Continuous Improvement Velocity', value: '12/quarter', target: '8/quarter', status: 'exceeding' },
  ];
  ...
  const renderOverview = () => (
    <>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Process Optimization Metrics</Text><View style={styles.metricsGrid}>{processMetrics.map((m, i) => (<View key={i} style={[styles.metricCard, { backgroundColor: theme.colors.background || '#F2F2F7' }]}><View style={styles.metricHeader}><m.icon size={18} color={m.color} /><Text style={[styles.metricTrend, { color: '#34C759' }]}>{m.trend}</Text></View><Text style={[styles.metricValue, { color: theme.colors.text }]}>{m.value}</Text><Text style={[styles.metricLabel, { color: theme.colors.textSecondary }]}>{m.label}</Text></View>))}</View></View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The AI Process Optimization Agent drives continuous improvement across the enterprise using lean methodology, waste elimination, and AI-driven improvement recommendations. It benchmarks processes, maps value streams, and delivers measurable cost savings with quantified ROI tracking.</Text></View>
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <AgentFeatures agentId="ai-process-optimization" agentName="AI Process Optimization" />
