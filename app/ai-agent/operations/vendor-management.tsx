...
import { Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap, User, Shield, Building2, FileText } from 'lucide-react-native';
...
export default function VendorManagementPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'231',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.2%',icon:Target,color:'#4E342E'}];
  const capabilities = ['Vendor Management','Contract Administration','Performance Evaluation'];
  ...
  const activities = [{time:'3 min ago',text:'Reviewed 12 vendor contracts',icon: CircleCheckBig},{time:'10 min ago',text:'Evaluated vendor performance',icon:Clock},{time:'25 min ago',text:'Processed vendor payments',icon:Zap}];

  const vendors = [
    { id: 'VEN-001', name: 'Acme Supplies', contract: 'Active', value: '$150K', renewal: 'Dec 2026', date: 'Today' },
    { id: 'VEN-002', name: 'Tech Solutions', contract: 'Expiring', value: '$85K', renewal: 'Jun 2026', date: 'Today' },
    { id: 'VEN-003', name: 'Global Logistics', contract: 'Active', value: '$220K', renewal: 'Mar 2027', date: 'Yesterday' },
  ];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Vendor Management Agent</Text>
      ...
      <View style={styles.tabContainer}>
        {['overview', 'vendors', 'capabilities', 'settings'].map((tab) => (
      ...
      {activeTab === 'overview' && (
        <>
          <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Manages vendor relationships, contracts, and performance evaluations.</Text></View>
      ...
      {activeTab === 'vendors' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Vendor Contracts</Text>
          {vendors.map((v, i) => (
            <View key={i} style={styles.txnCard}>
              <View style={styles.txnHeader}>
                <Building2 size={18} color="#4E342E" />
                <Text style={styles.txnId}>{v.id}</Text>
                <View style={[styles.statusBadge, { backgroundColor: v.contract === 'Active' ? '#34C75920' : '#FF950020' }]}>
                  <Text style={[styles.statusText, { color: v.contract === 'Active' ? '#34C759' : '#FF9500' }]}>{v.contract}</Text>
                </View>
              </View>
              <Text style={styles.txnDesc}>{v.name}</Text>
              <View style={styles.txnFooter}>
                <Text style={styles.txnDate}>Renewal: {v.renewal}</Text>
                <Text style={styles.txnAmount}>{v.value}</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      {activeTab === 'capabilities' && (
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          {[
            { endpoint: '/consult/vendor', desc: 'Consult on vendors' },
            { endpoint: '/vendor/manage', desc: 'Manage vendor relations' },
            { endpoint: '/vendor/contract', desc: 'Handle contracts' },
            { endpoint: '/vendor/report', desc: 'Generate reports' },
          ].map((item, i) => (
        ...
        <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
          ...
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Auto Renewal</Text>
              <Text style={styles.featureToggleDesc}>Auto-renew contracts</Text>
            </View>
            <Switch value={autoRenewal} onValueChange={setAutoRenewal} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Performance Tracking</Text>
              <Text style={styles.featureToggleDesc}>Track vendor performance</Text>
            </View>
            <Switch value={performanceTracking} onValueChange={setPerformanceTracking} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
          <View style={styles.featureToggle}>
            <View style={styles.featureToggleInfo}>
              <Text style={styles.featureToggleTitle}>Risk Assessment</Text>
              <Text style={styles.featureToggleDesc}>Assess vendor risks</Text>
            </View>
            <Switch value={riskAssessment} onValueChange={setRiskAssessment} trackColor={{ false: '#767577', true: '#4E342E' }} />
          </View>
        </View>
      )}

      <AgentFeatures agentId="vendor-management" agentName="AI Vendor Management Agent" />
