...
import { 
  ...
  ArrowUpRight, AlertTriangle, Zap, Percent, BarChart3, LineChart
} from 'lucide-react-native';
import { TRAVEL_COLORS, PRICING_DATA } from '@/constants/travelTourism';
...
export default function PricingRevenue() {
  const PRICING_METRICS = [
    ...
    { label: 'Profit Margin', value: '28.4%', icon: Percent, color: TRAVEL_COLORS.purple, trend: '+2.4%', trendUp: true },
    { label: 'Forecast Precision', value: '94.8%', icon: BarChart3, color: TRAVEL_COLORS.oceanBlue, trend: '+2.6%', trendUp: true },
    { label: 'Avg Response Time', value: '0.8s', icon: Clock, color: TRAVEL_COLORS.magenta, trend: '-0.4s', trendUp: true },
  ];

  const PRICING_STRATEGIES = [
    { strategy: 'Dynamic Pricing', adjustments: '1.2M', revenue: '+$420M', accuracy: '96.4%', color: TRAVEL_COLORS.emeraldGreen },
    { strategy: 'Seasonal Pricing', periods: '24', revenue: '+$280M', accuracy: '94.2%', color: TRAVEL_COLORS.neonCyan },
    { strategy: 'Package Pricing', packages: '8,400', revenue: '+$180M', accuracy: '92.8%', color: TRAVEL_COLORS.purple },
    { strategy: 'Promotional Pricing', campaigns: '486', revenue: '+$30M', accuracy: '88.4%', color: TRAVEL_COLORS.amber },
  ];

  const PRICING_ALERTS = [
  ...
  const renderMetrics = () => (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>Pricing & Revenue Metrics</Text>
    ...
    <View style={styles.section}>
      ...
      <View style={styles.pricingList}>
        {PRICING_DATA.map((category) => (
          <View key={category.category} style={[styles.pricingCard, { backgroundColor: '#0A0F1A', borderLeftWidth: 3, borderLeftColor: TRAVEL_COLORS.emeraldGreen }]}>
            <View style={styles.pricingHeader}>
              <Zap size={20} color={TRAVEL_COLORS.emeraldGreen} />
              <Text style={styles.pricingCategory}>{category.category}</Text>
            </View>
            <View style={styles.pricingMetrics}>
              <View style={styles.pricingMetric}>
                <Text style={styles.pricingMetricLabel}>Adjustments</Text>
                <Text style={styles.pricingMetricValue}>{category.adjustments}</Text>
              </View>
              <View style={styles.pricingMetric}>
                <Text style={styles.pricingMetricLabel}>Revenue</Text>
                <Text style={[styles.pricingMetricValue, { color: '#10B981' }]}>{category.revenueImpact}</Text>
              </View>
              <View style={styles.pricingMetric}>
                <Text style={styles.pricingMetricLabel}>Accuracy</Text>
                <Text style={styles.pricingMetricValue}>{category.accuracy}</Text>
              </View>
            </View>
          </View>
        ))}
  ...
  const renderPricingStrategies = () => (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>Pricing Strategies</Text>
      <View style={styles.strategiesList}>
        {PRICING_STRATEGIES.map((strategy) => (
          <View key={strategy.strategy} style={[styles.strategyCard, { backgroundColor: '#0A0F1A', borderLeftWidth: 3, borderLeftColor: strategy.color }]}>
            ...
            <View style={styles.strategyMetrics}>
              <View style={styles.strategyMetric}>
                <Text style={styles.strategyMetricLabel}>Volume</Text>
                <Text style={styles.strategyMetricValue}>{strategy.adjustments || strategy.periods || strategy.packages || strategy.campaigns}</Text>
              </View>
              <View style={styles.strategyMetric}>
                <Text style={styles.strategyMetricLabel}>Revenue</Text>
                <Text style={[styles.strategyMetricValue, { color: '#10B981' }]}>{strategy.revenue}</Text>
              </View>
              <View style={styles.strategyMetric}>
                <Text style={styles.strategyMetricLabel}>Accuracy</Text>
                <Text style={styles.strategyMetricValue}>{strategy.accuracy}</Text>
              </View>
            </View>
          </View>
        ))}
  ...
  const renderAlerts = () => (
    <View style={styles.section}>
      <Text style={styles.sectionTitle}>Pricing Alerts</Text>
      {PRICING_ALERTS.map((alert, index) => (
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.headerTitle}>Pricing & Revenue Engine</Text>
          <Text style={styles.headerSubtitle}>Dynamic pricing and revenue optimization</Text>
        </View>
      </View>

      {renderMetrics()}
      {renderPricingData()}
      {renderPricingStrategies()}
      {renderAlerts()}
    </ScrollView>
  );
}
