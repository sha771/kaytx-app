...
export default function DataIntelligenceAgentsScreen() {
  ...
  const filteredAgents = dataIntelligenceSubAgents.filter(agent =>
  ...
  const stats = {
    total: dataIntelligenceSubAgents.length,
    ...
    <Animated.View entering={FadeInUp.delay(index * 50)} style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={styles.cardHeader}>
        <View style={[styles.iconContainer, { backgroundColor: '#9B59B615' }]}>
          <agent.icon size={24} color="#9B59B6" />
        </View>
        <View style={styles.agentInfo}>
          <Text style={[styles.agentName, { color: colors.text }]}>{agent.name}</Text>
          <Text style={[styles.agentTitle, { color: colors.text + '80' }]} numberOfLines={1}>{agent.title}</Text>
        </View>
        <View style={[styles.statusBadge, { backgroundColor: agent.status === 'active' ? '#9B59B6' : '#F59E0B' }]}>
      ...
      <View style={styles.configRow}>
        {agent.configuration?.model && (
          <View style={styles.configBadge}>
            <Brain size={12} color="#9B59B6" />
            <Text style={[styles.configText, { color: colors.text }]}>{agent.configuration.model.primary}</Text>
          </View>
        )}
        {agent.configuration?.personality && (
          <View style={styles.configBadge}>
            <User size={12} color="#9B59B6" />
            <Text style={[styles.configText, { color: colors.text }]}>{(agent.configuration.personality as any).expertise}</Text>
          </View>
        )}
      </View>

      <View style={styles.actionRow}>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#9B59B6' }]} onPress={() => handleConfigure(agent.id)}>
        ...
        <TouchableOpacity style={[styles.actionBtnOutline, { borderColor: colors.border }]} onPress={() => handleDataUpload(agent.id)}>
          ...
          <Text style={[styles.actionBtnTextOutline, { color: colors.text }]}>Data Sources</Text>
        </TouchableOpacity>
      </View>
    </Animated.View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        ...
        {mainAgent && (
          <Animated.View entering={FadeInRight} style={[styles.mainCard, { backgroundColor: '#9B59B615' }]}>
            <mainAgent.icon size={28} color="#9B59B6" />
            ...
            <TouchableOpacity style={[styles.manageBtn, { backgroundColor: '#9B59B6' }]} onPress={() => handleConfigure(mainAgent.id)}>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statPill, { backgroundColor: '#9B59B615' }]}>
            <Mic size={14} color="#9B59B6" />
            <Text style={[styles.statValue, { color: '#9B59B6' }]}>{stats.withVoice}</Text>
          </View>
