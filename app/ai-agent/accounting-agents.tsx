...
export default function AccountingAgentsScreen() {
  ...
  const mainAgent = getMainAgentByCategory('accounting-finance');
  
  const filteredAgents = accountingFinanceSubAgents.filter(agent =>
  ...
  const stats = {
    total: accountingFinanceSubAgents.length,
    active: accountingFinanceSubAgents.filter(a => a.status === 'active').length,
    withVoice: accountingFinanceSubAgents.filter(a => a.configuration?.voice.enabled).length,
    withTraining: accountingFinanceSubAgents.filter(a => a.configuration?.training.enabled).length,
  };
