...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3120',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.2s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.2%',icon:Target,color:'#3E2723'}];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#3E272320' }]}><BookOpen size={48} color="#3E2723" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Policy Manager</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Policy Manager AI coordinates team operations, streamlines workflows, and ensures quality delivery within the Legal & Governance department.</Text></View>
      ...
      <AgentFeatures agentId="policy-manager" agentName="Policy Manager" />
    </ScrollView>
  );
}
