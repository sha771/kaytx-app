...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks/Day',value:'156',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.7s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.7%',icon:Target,color:'#3E2723'}];
  const capabilities = ['Regulatory Management','Change Tracking','Compliance Adherence'];
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        ...
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Regulatory Agent</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>Manages regulatory compliance, tracks regulatory changes, and ensures adherence.</Text></View>
      ...
      <AgentFeatures agentId="regulatory-agent" agentName="AI Regulatory Agent" />
    </ScrollView>
  );
}
