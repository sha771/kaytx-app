...
import { Activity, BarChart3, Clock, Target, Zap, ArrowRight, Briefcase, DollarSign, TrendingUp } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function CapitalAllocationOptimizerPage() {
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#007AFF20' }]}>
          <BarChart3 size={56} color="#007AFF" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI Capital Allocation Optimizer</Text>
      ...
      <View style={styles.statsContainer}>
        {[
          {label:'Cost Equiv',value:'$75K/yr',icon: BarChart3, color: '#34C759'},
          {label:'AI Cost',value:'$3.8K/yr',icon: Clock, color: '#007AFF'},
          {label:'Efficiency',value:'20x',icon: Target, color: '#FF9500'},
          {label:'Optimization',value:'94.2%',icon: Activity, color: '#007AFF'}
        ].map((stat,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI Capital Allocation Optimizer deploys enterprise capital across business units, projects, and
          investments using advanced optimization algorithms. It maximizes return on deployed capital while
          maintaining risk thresholds and ensuring strategic alignment with corporate objectives.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {[
          'Capital Deployment Strategy & Execution',
          'Portfolio Optimization & Rebalancing',
          'Investment Opportunity Scoring',
          'Budget-to-Capital Mapping',
          'ROI Maximization Across Business Units',
          'Capital Efficiency Monitoring & Reporting'
        ].map((item,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.tagsContainer}>
          {['Capital Optimization','Portfolio Rebalancing','ROI Analysis','Investment Scoring','Budget Mapping','Efficiency Monitoring','Risk Thresholds','Strategic Alignment'].map((cap,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {[
          { label: 'Capital Efficiency', value: '94.2% deployed' },
          { label: 'Avg ROI', value: '23.4% return' },
          { label: 'Rebalance Time', value: '< 30 min' },
          { label: 'Coverage', value: 'All business units' }
        ].map((metric,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {['/consult/capital-allocation-optimizer', '/capital-allocation-optimizer/optimize', '/capital-allocation-optimizer/rebalance'].map((endpoint,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.actionsGrid}>
          ...
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: '#007AFF12' }]} onPress={() => router.push('/ai-agent/finance')}>
            <BarChart3 size={24} color="#007AFF" />
            <Text style={[styles.actionText, { color: '#007AFF' }]}>Finance</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <AgentFeatures agentId="capital-allocation-optimizer" agentName="AI Capital Allocation Optimizer" />
