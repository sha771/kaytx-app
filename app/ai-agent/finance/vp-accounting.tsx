...
import { Calculator, Activity, Users, Clock, Target, Zap, ArrowRight, Receipt, FileText, Briefcase, Shield, BookOpen } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';
import { useRouter } from 'expo-router';

export default function VPAccountingPage() {
    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#388E3C20' }]}>
          <Calculator size={56} color="#388E3C" />
        </View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>AI VP of Accounting</Text>
        ...
        <View style={styles.badgesRow}>
          ...
          <View style={[styles.badge, { backgroundColor: '#388E3C22' }]}><Briefcase size={12} color="#388E3C" /><Text style={[styles.badgeText, { color: '#388E3C' }]}>VP Level</Text></View>
      ...
      <View style={styles.statsContainer}>
        {[
          {label:'Cost Equiv',value:'$170K/yr',icon: Receipt, color: '#34C759'},
          {label:'AI Cost',value:'$8.5K/yr',icon: Clock, color: '#007AFF'},
          {label:'Efficiency',value:'20x',icon: Target, color: '#FF9500'},
          {label:'Reports To',value:'CFO',icon: Users, color: '#388E3C'}
        ].map((stat,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <Text style={[styles.description, { color: theme.colors.textSecondary }]}>
          The AI VP of Accounting oversees all accounting operations, financial reporting, and controllership
          functions. It ensures GAAP/IFRS compliance, manages the financial close process, and coordinates
          with internal and external auditors to maintain the highest standards of financial integrity.
        </Text>
      </View>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {[
          ...
          'Financial Close Process Oversight',
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        <View style={styles.tagsContainer}>
          {['Financial Reporting','GAAP Compliance','Audit Coordination','Close Management','Internal Controls','Reconciliation','Compliance Management','Process Improvement'].map((cap,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {[
          { label: 'Close Cycle Time', value: '2 days (vs 10)' },
          { label: 'Reporting Accuracy', value: '99.97%' },
          { label: 'Audit Findings', value: '0 material' },
          { label: 'Compliance Score', value: '100% adherence' }
        ].map((metric,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {[
          { name: 'AI Ledger Reconciler', route: '/ai-agent/finance/sub-agents/ledger-reconciler', icon: Receipt, color: '#2E7D32', desc: 'General ledger reconciliation' },
          { name: 'AI Accounting Standards Enforcer', route: '/ai-agent/finance/sub-agents/accounting-standards-enforcer', icon: Shield, color: '#007AFF', desc: 'GAAP/IFRS compliance enforcement' },
          { name: 'AI Close Process Coordinator', route: '/ai-agent/finance/sub-agents/close-process-coordinator', icon: BookOpen, color: '#FF9500', desc: 'Financial close orchestration' }
        ].map((sub,index)=>(
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}>
        ...
        {['/consult/vp-accounting', '/accounting/close', '/accounting/compliance'].map((endpoint,index)=>(
      ...
      <AgentFeatures agentId="vp-accounting" agentName="VP of Accounting" />
