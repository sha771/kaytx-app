...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'1,205',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.0s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'100.0%',icon:Target,color:'#B71C1C'}];
  const capabilities = ['Patient Scheduling','Registration','Communication','Insurance Coord','Medical Records','Patient Experience'];
  const responsibilities = ['Patient scheduling & appointment management','Patient registration & demographics','Patient communication & follow-up','Insurance coordination & verification','Medical records coordination','Patient experience improvement'];
  const activities = [{time:'3 min ago',text:'Scheduled 75 appointments',icon: CircleCheckBig},{time:'6 min ago',text:'Registered 34 new patients',icon:Clock},{time:'9 min ago',text:'Followed up with 45 patients',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#B71C1C20' }]}><Heart size={48} color="#B71C1C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Patient Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Patient Services</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Patient Coordinator AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Healthcare department.</Text></View>
      ...
      <AgentFeatures agentId="patient-coordinator" agentName="Patient Coordinator" />
    </ScrollView>
  );
}
