...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'1,389',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.9s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.4%',icon:Target,color:'#B71C1C'}];
  const capabilities = ['Operations Strategy','Facility Mgmt','Budget Oversight','Regulatory Compliance','Staff Planning','Performance Improvement'];
  const responsibilities = ['Healthcare operations strategy & planning','Facility management oversight','Budget development & oversight','Regulatory compliance management','Staff planning & workforce optimization','Performance improvement programs'];
  const activities = [{time:'3 min ago',text:'Approved Q3 operational budget',icon: CircleCheckBig},{time:'6 min ago',text:'Reviewed facility compliance reports',icon:Clock},{time:'9 min ago',text:'Led performance improvement initiative',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#B71C1C20' }]}><Building size={48} color="#B71C1C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>VP Healthcare Operations</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Healthcare Operations</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#B71C1C22' }]}><Star size={12} color="#B71C1C" /><Text style={[styles.badgeText, { color: '#B71C1C' }]}>VP Level</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The VP Healthcare Operations AI leads departmental strategy, manages cross-functional initiatives, and drives performance optimization within the Healthcare division.</Text></View>
      ...
      <AgentFeatures agentId="vp-healthcare-operations" agentName="VP Healthcare Operations" />
    </ScrollView>
  );
}
