...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'4,060',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.6s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'98.0%',icon:Target,color:'#BF360C'}];
  const capabilities = ['Production Strategy','Quality Systems','Safety Leadership','Capacity Planning','Continuous Improvement','Supply Chain'];
  const responsibilities = ['Production strategy & planning','Quality management system oversight','Workplace safety program leadership','Capacity planning & resource allocation','Continuous improvement program management','Supply chain coordination'];
  const activities = [{time:'3 min ago',text:'Approved Q3 production plan',icon: CircleCheckBig},{time:'6 min ago',text:'Reviewed quality metrics dashboard',icon:Clock},{time:'9 min ago',text:'Led safety committee meeting',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}><HardHat size={48} color="#BF360C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>CPO</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Manufacturing Leadership</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#BF360C22' }]}><Star size={12} color="#BF360C" /><Text style={[styles.badgeText, { color: '#BF360C' }]}>C-Suite</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The CPO AI provides executive-level strategic oversight, drives organizational alignment, and ensures operational excellence across the Manufacturing division.</Text></View>
      ...
      <AgentFeatures agentId="cpo" agentName="CPO" />
    </ScrollView>
  );
}
