...
import { Factory, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'1,590',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.5s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.6%',icon:Target,color:'#BF360C'}];
  const capabilities = ['Production Planning','Scheduling','Quality Control','Team Management','KPI Tracking','Safety'];
  const responsibilities = ['Production planning & scheduling','Quality control oversight','Production team management','KPI tracking & performance improvement','Safety compliance & enforcement','Production reporting & analytics'];
  const activities = [{time:'3 min ago',text:'Managed 3 production lines',icon: CircleCheckBig},{time:'6 min ago',text:'Achieved 98.5% OEE target',icon:Clock},{time:'9 min ago',text:'Reduced scrap rate by 15%',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}><Factory size={48} color="#BF360C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Production Manager</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Production Operations</Text>
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Production Manager AI coordinates team operations, streamlines workflows, and ensures quality delivery within the Manufacturing department.</Text></View>
      ...
      <AgentFeatures agentId="production-manager" agentName="Production Manager" />
    </ScrollView>
  );
}
