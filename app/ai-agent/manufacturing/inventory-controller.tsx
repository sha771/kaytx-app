...
import { Package, Activity, Star, CircleCheckBig, Clock, Target, ArrowRight, Zap } from 'lucide-react-native';
import AgentFeatures from '@/components/ai-agent/AgentFeatures';

export default function AgentPage() {
  const { theme } = useTheme();
  const stats = [{label:'Tasks',value:'3,701',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.8s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.5%',icon:Target,color:'#BF360C'}];
  const capabilities = ['Inventory Mgmt','Stock Optimization','Cycle Counting','Demand Planning','Warehouse Ops','ERP Systems'];
  const responsibilities = ['Inventory level monitoring & control','Stock optimization & reorder management','Cycle counting & accuracy programs','Demand planning & forecasting','Warehouse operations coordination','ERP inventory module management'];
  const activities = [{time:'3 min ago',text:'Completed cycle count for zone A',icon: CircleCheckBig},{time:'6 min ago',text:'Optimized safety stock levels',icon:Clock},{time:'9 min ago',text:'Reduced inventory holding by 12%',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}><Package size={48} color="#BF360C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Inventory Controller</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Inventory Management</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#BF360C22' }]}><Star size={12} color="#BF360C" /><Text style={[styles.badgeText, { color: '#BF360C' }]}>Controller</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Inventory Controller AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Manufacturing department.</Text></View>
      ...
      <AgentFeatures agentId="inventory-controller" agentName="Inventory Controller" />
    </ScrollView>
  );
}
