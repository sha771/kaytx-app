...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'1,603',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'1.0s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'99.0%',icon:Target,color:'#BF360C'}];
  const capabilities = ['Production Scheduling','MRP','Capacity Planning','BOM Management','Lead Time Analysis','Order Management'];
  const responsibilities = ['Production schedule development & maintenance','MRP system management & optimization','Capacity planning & constraint resolution','Bill of materials management','Lead time analysis & reduction','Production order management & tracking'];
  const activities = [{time:'3 min ago',text:'Created weekly production schedule',icon: CircleCheckBig},{time:'6 min ago',text:'Resolved capacity bottleneck on line 2',icon:Clock},{time:'9 min ago',text:'Updated BOM for 8 products',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}><Calendar size={48} color="#BF360C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Production Planner</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Production Planning</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#BF360C22' }]}><Star size={12} color="#BF360C" /><Text style={[styles.badgeText, { color: '#BF360C' }]}>Planner</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Production Planner AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Manufacturing department.</Text></View>
      ...
      <AgentFeatures agentId="production-planner" agentName="Production Planner" />
    </ScrollView>
  );
}
