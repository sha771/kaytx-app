...
export default function AgentPage() {
  ...
  const stats = [{label:'Tasks',value:'3,485',icon: CircleCheckBig,color:'#34C759'},{label:'Uptime',value:'99.9%',icon:Activity,color:'#007AFF'},{label:'Response',value:'0.8s',icon:Clock,color:'#FF9500'},{label:'Accuracy',value:'97.0%',icon:Target,color:'#BF360C'}];
  const capabilities = ['Shipping Coord','Route Optimization','Carrier Mgmt','Freight Analysis','Customs','Tracking'];
  const responsibilities = ['Shipping coordination & scheduling','Route optimization & cost reduction','Carrier management & negotiation','Freight cost analysis & optimization','Customs documentation & compliance','Shipment tracking & exception management'];
  const activities = [{time:'3 min ago',text:'Coordinated 120 shipments',icon: CircleCheckBig},{time:'6 min ago',text:'Optimized routes saving $8K',icon:Clock},{time:'9 min ago',text:'Managed 5 carrier relationships',icon:Zap}];
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.hero, { borderBottomColor: theme.colors.border || '#E5E5EA' }]}>
        <View style={[styles.heroIconWrap, { backgroundColor: '#BF360C20' }]}><Truck size={48} color="#BF360C" /></View>
        <Text style={[styles.heroTitle, { color: theme.colors.text }]}>Logistics Coordinator</Text>
        <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>Supply Chain Logistics</Text>
        <View style={styles.badgesRow}>
          <View style={[styles.badge, { backgroundColor: '#34C75922' }]}><Activity size={12} color="#34C759" /><Text style={[styles.badgeText, { color: '#34C759' }]}>Active</Text></View>
          <View style={[styles.badge, { backgroundColor: '#BF360C22' }]}><Star size={12} color="#BF360C" /><Text style={[styles.badgeText, { color: '#BF360C' }]}>Coordinator</Text></View>
        </View>
      </View>
      <View style={styles.statsContainer}>{stats.map((stat,i)=>(<View key={i} style={[styles.statCard, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><stat.icon size={22} color={stat.color} /><Text style={[styles.statValue, { color: theme.colors.text }]}>{stat.value}</Text><Text style={[styles.statLabel, { color: theme.colors.textSecondary }]}>{stat.label}</Text></View>))}</View>
      <View style={[styles.section, { backgroundColor: theme.colors.card || '#F2F2F7' }]}><Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Overview</Text><Text style={[styles.description, { color: theme.colors.textSecondary }]}>The Logistics Coordinator AI executes specialized tasks, automates workflows, and delivers consistent high-quality performance within the Manufacturing department.</Text></View>
      ...
      <AgentFeatures agentId="logistics-coordinator" agentName="Logistics Coordinator" />
    </ScrollView>
  );
}
