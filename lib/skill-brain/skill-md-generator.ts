...
export class SkillMdGenerator {
  ...
  private generateFrontmatter(skillMd: SkillMd): string {
    const fm = {
      title: skillMd.title,
  ...
  private generateBody(skillMd: SkillMd): string {
    const parts: string[] = [];
  ...
  private addMetadataSection(skillMd: SkillMd, parts: string[]): void {
    parts.push('## Metadata');
    parts.push('');
  ...
  private addSkillsSection(skills: SkillEntry[], parts: string[]): void {
    parts.push('## Skills');
    parts.push('');
  ...
  private addPeopleSection(people: SkillPerson[], parts: string[]): void {
    parts.push('## People');
    parts.push('');
  ...
  private addRelationshipsSection(
    ...
    const grouped = new Map<string, Array<{ target: string; type: string; strength: number }>>();
  ...
  private addTransferReadinessSection(skillMd: SkillMd, parts: string[]): void {
  ...
  private parseBody(raw: string): Partial<SkillMd> {
  ...
  private calculateOverallReadiness(skills: SkillEntry[]): TransferReadiness {
    if (skills.length === 0) return TransferReadiness.NOT_READY;

    const readyCount = skills.filter(s =>
      s.sources.length > 0 &&
      s.evidence.length > 0 &&
    ...
    if (ratio >= 0.9) return TransferReadiness.FULLY_READY;
    if (ratio >= 0.7) return TransferReadiness.READY;
    if (ratio >= 0.4) return TransferReadiness.PARTIALLY_READY;
    return TransferReadiness.NOT_READY;
  }

  private mergeSkills(existing: SkillEntry[], incoming: SkillEntry[]): SkillEntry[] {
    ...
    for (const s of incoming) {
      ...
      if (e) {
        map.set(key, {
          ...e,
          sources: [...e.sources, ...s.sources].filter(
  ...
  private mergePeople(existing: SkillPerson[], incoming: SkillPerson[]): SkillPerson[] {
    const map = new Map<string, SkillPerson>();

    for (const p of existing) map.set(p.email.toLowerCase(), p);
  ...
  private mergeRelationships(
    ...
    const map = new Map<string, SkillRelation>();

    for (const r of existing) map.set(`${r.sourceSkillId}-${r.targetSkillId}-${r.type}`, r);
...
export const skillMdGenerator = new SkillMdGenerator();
