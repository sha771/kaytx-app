/**
 * Script to add comprehensive features to all sub-agents (831 agents)
...
// Function to find all sub-agent files
function findSubAgentFiles(dir, fileList = []) {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      // Process all directories, including sub-agents
      findSubAgentFiles(filePath, fileList);
    } else if (file.endsWith('.tsx') && file !== '_layout.tsx' && file !== 'index.tsx') {
      // Check if this file is in a sub-agents directory
      if (filePath.includes('sub-agents')) {
...
// Function to find agent files within a sub-agents directory
function findAgentFilesInSubDir(dir, fileList = []) {
  const files = fs.readdirSync(dir);
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      findAgentFilesInSubDir(filePath, fileList);
    } else if (file.endsWith('.tsx') && file !== '_layout.tsx' && file !== 'index.tsx') {
      fileList.push(filePath);
    }
  });
...
console.log('Scanning for sub-agent directories...');
const subAgentFiles = findSubAgentFiles(aiAgentDir);
console.log(`Found ${subAgentFiles.length} sub-agent files`);

console.log('Adding comprehensive features to sub-agents...');
subAgentFiles.forEach(addFeaturesToAgent);

console.log('Sub-agent feature addition complete!');
